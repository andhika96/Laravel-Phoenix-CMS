...
function uploadFormData(path, data, onProgress = () => {}, bindAbort = () => {}) {
  return new Promise((resolve, reject) => {
    ...
    request.addEventListener('load', () => {
      ...
      reject(new Error(body.message || body.errors?.[Object.keys(body.errors)[0]]?.[0] || 'Upload File Manager V2 gagal.'));
    });
...
export async function uploadFile(file, {
  ...
  if (file.size < uploadOptions.chunkThreshold) {
    ...
    const asset = await uploadFormData('/assets/upload', data, onProgress, (abort) => {
  ...
  for (let part = 0; part < parts; part += 1) {
    ...
    await uploadFormData(`/uploads/${session.id}/chunks/${part}`, data, (progress) => {

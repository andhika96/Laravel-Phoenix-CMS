...
import { assets, createFolder as createFolderApi, deleteAsset, folders, initializeWorkspace, loadAssets, loadFolderDetails as loadFolderDetailsApi, loadFolders, moveAssets, navItems, renameAsset, replaceAssets, replaceFolders, storageProfiles, toggleAssetStar, upsertAsset } from './data/live';
...
async function submitActionModal() {
  ...
  try {
    if (type === 'create') {
      ...
      if (asset.type === 'folder' && (activeFolder.value === asset.path || activeFolder.value.startsWith(`${asset.path}/`))) {
      ...
      if (!selected.length) throw new Error('Select at least one asset to delete.');
      const count = selected.length;
      await Promise.all(selected.map((asset) => deleteAsset(activeStorage.value, asset.path)));
      clearSelection();
      await refreshAssets({ refreshFolders: true });
      successMessage = `${count} asset${count === 1 ? '' : 's'} deleted.`;
      ...
      if (uploadItem.storage === activeStorage.value) await refreshAssets({ refreshFolders: true });
        ...
        <section class="action-modal" :class="{ 'is-danger': actionModal === 'delete' || actionModal === 'delete-upload' }" role="dialog" aria-modal="true" aria-labelledby="action-modal-title">
          <header>
            <div>
              ...
              <h2 id="action-modal-title">
                ...
                <template v-else-if="actionModal === 'rename'">Rename {{ actionModalItem?.type === 'folder' ? 'folder' : 'file' }}</template>
                <template v-else>Delete {{ actionModalCount }} selected asset<span v-if="actionModalCount !== 1">s</span></template>
              </h2>
            </div>
          ...
          <form @submit.prevent="submitActionModal">
            <div class="action-modal-body">
              ...
              <template v-else>
                <div class="action-modal-alert">
                  ...
                  <div>
                    <strong><template v-if="actionModal === 'delete-upload'">Delete {{ actionModalItem?.name }}?</template><template v-else>Delete selected assets?</template></strong>
            ...
            <footer>
              ...
              <button v-if="actionSubmitState === 'idle'" type="submit" class="btn" :class="actionModal === 'delete' || actionModal === 'delete-upload' ? 'btn-danger' : 'btn-primary'" :disabled="actionModal === 'move' && moveDestination === activeFolder">
                <template v-if="actionModal === 'create'">Create folder</template>
                <template v-else-if="actionModal === 'move'">Move assets</template>
                 <template v-else-if="actionModal === 'rename'">Rename</template>
                <template v-else>Delete assets</template>
              </button>

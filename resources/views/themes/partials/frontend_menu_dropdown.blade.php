			...
			<ul class="navbar-nav ph-fe-navbar ms-auto">
				@foreach ($frontendMenuItems as $item)
					@php
						...
						$toggleIconType = $dropdownBox['toggle_icon_type'] ?? '';
						$toggleIconUrl = $dropdownBox['toggle_icon_url'] ?? '';
					...
					@if ($hasDropdown && $dropdownType == 'bootstrap')
						<li class="nav-item dropdown ph-fe-menu-item" style="--ph-fe-dropdown-margin: {{ (int) $dropdownBox['margin_top'] }}px;">
							<a class="nav-link dropdown-toggle ph-fe-link{{ $toggleIconClass }}" href="{{ $item['parent_url'] }}" role="button" data-bs-toggle="dropdown" aria-expanded="false">
								...
								@if ($hasToggleIcon)
									<span class="ph-fe-toggle-icon" aria-hidden="true">
										@if ($toggleIconUrl !== '')
											<img src="{{ $toggleIconUrl }}" alt="">
										@else
											{!! $toggleIconHtml !!}
										@endif
									</span>
								...
								@foreach ($visibleSubmenus as $submenu)
									...
									<li>
										<a class="dropdown-item ph-fe-bootstrap-item" href="{{ $submenu['submenu_url'] }}">
											@if ($submenuHasIcon)
												<span class="ph-fe-bootstrap-icon-slot">
								...
								@if ($dropdownConfig['mega_layout'] == 'featured' && count($visibleSubmenus) > 0)
									...
									<a class="ph-fe-featured-menu-item" href="{{ $featuredItem['submenu_url'] }}">
										...
										<span class="ph-fe-featured-body">
											...
											@if ($megaBox['show_description'] && $featuredItem['submenu_description'] !== '')
												<span class="ph-fe-submenu-desc ph-fe-text-{{ $megaBox['description_position'] }}">{{ $featuredItem['submenu_description'] }}</span>
											@endif
										</span>
									</a>
								@endif
									...
									@foreach ($visibleSubmenus as $submenu)
										<a class="ph-fe-mega-item ph-fe-image-{{ $megaBox['image_position'] }}" href="{{ $submenu['submenu_url'] }}">
											...
											<span class="ph-fe-mega-body">
												...
												@if ($megaBox['show_description'] && $submenu['submenu_description'] !== '')
													<span class="ph-fe-submenu-desc ph-fe-text-{{ $megaBox['description_position'] }}">{{ $submenu['submenu_description'] }}</span>
												@endif
											</span>
										</a>
									@endforeach

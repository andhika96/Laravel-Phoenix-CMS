...
<head>
    ...
    <style>body{margin:0;background:#fff}.article-preview-fixture-note{position:fixed;top:12px;right:14px;z-index:10;padding:4px 8px;border:1px solid var(--article-accent-ring,#ded5ff);border-radius:999px;background:rgb(255 255 255 / .92);color:var(--article-accent,#6542d7);font:700 11px/1 var(--ph-font-family,Arial,sans-serif);letter-spacing:.04em;text-transform:uppercase}.article-page .article-shell,.article-detail__shell{padding-top:2rem;padding-bottom:2rem}</style>
</head>
<body @if ($isPreviewFixture ?? false) data-preview-fixture="true" @endif>
    ...
    @if ($surface === 'archive')
        @include($templateView, ['articles' => $articles, 'templateSettings' => $templateSettings, 'previousArticle' => $previousArticle, 'nextArticle' => $nextArticle])
    @elseif ($article)
        @include($templateView, ['article' => $article, 'templateSettings' => $templateSettings, 'previousArticle' => $previousArticle, 'nextArticle' => $nextArticle])
    @else

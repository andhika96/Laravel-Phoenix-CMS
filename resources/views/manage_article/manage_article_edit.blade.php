...
@section('content')
	<div id="ph-app-manage-article">
		<div class="mb-3">
			{{ Breadcrumbs::render('manage_article.edit') }}
		</div>

		<div class="ph-content rounded p-3 mb-3">
			<h4 class="mb-0"><i class="fad fa-plus fa-fw me-1"></i> {{ t('Edit Post') }}</h4>
		</div>

		<div>
			<div id="ph-form-submit-data">
				<form action="{{ route('cms.core.manage_article.update', $article->id) }}" method="post" ref="formHTML" @submit.prevent="submitData" auto-refresh="false" auto-reset="true" auto-lock-button="false" auto-block-button-mobile="false" custom-button-value="{{ t('Save') }}">
					...
					<div class="row g-3 ph-fetch-detail-data" data-url="{{ url('manage_article/detaildata/'.$article->id) }}">
						<div class="col-md-8">
							<div class="ph-content rounded mb-3">
								<div class="p-4">
									<div class="row g-3">
										<div class="col-12">
											...
											<input type="text" name="title" class="form-control font-size-inherit" value="{{ $article->title }}">
										</div>
								...
								<div class="row g-0 mt-1">
									<div class="col-12 position-relative">
										<textarea name="content" id="editor" rows="15" class="form-control font-size-inherit">{{ $article->content }}</textarea>
									</div>
								</div>
												...
												<div class="collapse" id="collapseEditStatus">
													<div class="card card-body border-0 bg-body-secondary my-2">
														<select name="status" class="form-select mb-3" aria-label="Select status">
															<option value="publish" @if ($article->status == 'publish') selected @endif>{{ t('Publish') }}</option>
															<option value="draft" @if ($article->status == 'draft') selected @endif>{{ t('Draft') }}</option>
															<option value="pending" @if ($article->status == 'pending') selected @endif>{{ t('Pending') }}</option>
														</select>
												...
												<div class="collapse" id="collapseEditVisibility">
													<div class="card card-body border-0 bg-body-secondary my-2">
														<select name="visibility" class="form-select mb-3" aria-label="Select visibility" v-on:change="selectVisibility($event)">
															<option value="public" @if ($article->visibility == 'public') selected @endif>{{ t('Public') }}</option>
															<option value="private" @if ($article->visibility == 'private') selected @endif>{{ t('Private') }}</option>
															<option value="password_protected" @if ($article->visibility == 'password_protected') selected @endif>{{ t('Password Protected') }}</option>
														</select>
												...
												<div class="row g-1">
													<div class="col-8 d-flex align-items-center">	
														<span>
															...
															<span v-if="article.created_at == '' || article.scheduled == 'false'" class="fw-bold" v-cloak>{{ t('Immediately') }}</span>
															<span v-else-if="article.created_at !== '' || article.scheduled == 'true'" class="fw-bold" v-cloak>@{{ article.created_at }} @{{ article.hours }}:@{{ article.minutes }}</span>
														</span>
													</div>
													...
													<div class="card card-body border-0 bg-body-secondary my-2">
														<div class="row g-1 mb-3">
															...
															<div class="col-12 col-md-6">
																<input type="date" name="created_at" class="form-control font-size-inherit" value="@if ($article->scheduled == 'true'){{ $article->created_at->format('Y-m-d') }}@endif">
															</div>

															<div class="col-6 col-md-3">
																<select name="hours" class="form-select" aria-label="Select hours">
																	@foreach (range(0, 23) as $hour)
																		<option value="{{ sprintf('%02d', $hour) }}" @if ($article->scheduled == 'true' && sprintf('%02d', $hour) == $article->created_at->format('H')) selected @endif>{{ sprintf('%02d', $hour) }}</option>
																	@endforeach	
																</select>															
															</div>
															...
															<div class="col-6 col-md-3">
																<select name="minutes" class="form-select" aria-label="Select minutes">
																	@foreach (range(0, 59) as $minute)
																		<option value="{{ sprintf('%02d', $minute) }}" @if ($article->scheduled == 'true' && sprintf('%02d', $minute) == $article->created_at->format('i')) selected @endif>{{ sprintf('%02d', $minute) }}</option>
																	@endforeach	
																</select>		
															</div>
														</div>

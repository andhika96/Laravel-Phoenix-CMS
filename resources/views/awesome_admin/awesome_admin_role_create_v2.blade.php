...
@section('content')
	...
	<div id="ph-app-manage-role">
		<div class="mb-3">
			{{ Breadcrumbs::render('awesome_admin.role.create') }}
		</div>

		<div id="ph-submit-data-role">
			<form action="{{ route('cms.admin.awesome_admin.role.storeV2') }}" method="post" ref="formHTML" auto-lock-button="false" auto-block-button-mobile="true" custom-button-value="Submit" @submit.prevent="submitData($event)">

				<div class="ph-content p-3 mb-3 rounded">
					<div class="row g-3">
						<div class="col-md-6 d-flex align-items-center">
							<h4 class="mb-0">{{ t('Create Role & Permissions') }}</h4>
						</div>
												...
												<td>
													<div class="fw-bold d-flex justify-content-center text-center">
														<div class="form-check p-0">
															<input class="form-check-input check-all-permission check-permission-{{ Str::replace(' ', '', $permission) }} m-0" type="checkbox" name="menu_vars[{{ $menu['index'] }}][parent_permissions][]" value="{{ $permission }}" id="flexCheckDefault" @if (isset(getDataCustomPermissions(0, $menu['parent_code'])[$permission])) checked @endif>
														</div>
													</div>
												</td>
											@endforeach
													...
													<tr>			
														...
														@foreach ($permissions as $permission)
															<td>
																<div class="fw-bold d-flex justify-content-center text-center">
																	<div class="form-check p-0">
																		<input class="form-check-input check-all-permission check-permission-{{ Str::replace(' ', '', $permission) }} m-0" type="checkbox" name="menu_vars[{{ $parent_index }}][parent_submenu][list][{{ $submenu['index'] }}][{{ $submenu['submenu_code'] }}][submenu_permissions][]" value="{{ $permission }}" id="flexCheckDefault" @if (isset(getDataCustomPermissions(0, $submenu['submenu_code'])[$permission])) checked @endif>
																	</div>
																</div>
															</td>
														@endforeach

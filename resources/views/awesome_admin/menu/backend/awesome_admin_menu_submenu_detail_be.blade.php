...
@section('title')
	{{ t('Manage Backend Submenu Details') }}
@endsection

@section('content')
	<div id="ph-app-manage-menu">
		<div class="mb-3">
			{{ Breadcrumbs::render('awesome_admin.menu.be.submenu.detail') }}
		</div>

		<div class="ph-content rounded p-3 mb-3">
			<div class="row g-3">
				<div class="col-md-6 d-flex align-items-center">
					<h4 class="mb-0">{{ t('Manage Backend Submenu Details') }}</h4>
				</div>
			</div>
		</div>
		...
		<div class="row g-4">
			<div class="col-md-6">
				<div class="ph-content rounded p-4 ph-fetch-listdata-submenu" id="ph-fetch-listdata-submenu" data-url="{{ url('awesome_admin/menu/be/listdata/submenu/detail/'.$idOrSlug) }}">
					...
					<div v-else class="ph-data-load-content" id="ph-form-manage-menu" style="display: none">
						<form action="{{ route('cms.admin.awesome_admin.menu.be.update_submenu_detail', ['idOrSlug' => $idOrSlug]) }}" method="post" ref="formHTML" custom-button-value="{{ t('Save') }}" @submit.prevent="submitData">
			...
			<div class="col-md-6">
				<div class="ph-content rounded p-4">
					...
					<div id="ph-submit-data-rp" class="ph-fetch-listdata-roles" data-url="{{ url('awesome_admin/menu/be/listdata/roles') }}">
						<form action="{{ route('cms.admin.awesome_admin.menu.be.checkSubmenuLink') }}" method="post" auto-refresh="false" auto-lock-button="false" custom-button-value="{{ t('Create') }}" ref="formHTMLCreateNewSubmenu" @submit.prevent="addSubmenu">
							<div class="ph-notice" v-cloak>
								<div aria-live="polite" aria-atomic="true" class="position-relative">
									<div class="toast-container position-fixed top-0 end-0 p-3">
										<div :class="'toast ph-notice-toast ph-callout-no-border '+responseStatusToast" role="alert" aria-live="assertive" aria-atomic="true" data-bs-delay="3000">
			...
			<div class="modal ph-modal-delete fade" id="modalDeleteSubmenuDetail" tabindex="-1" aria-labelledby="modalDeleteSubmenuDetailLabel" aria-hidden="true">
				<div class="modal-dialog ph-modal-dialog modal-dialog-centered">
					<div class="modal-content" id="ph-form-delete-submenu-detail">
						<form action="{{ url('awesome_admin/menu/delete_submenu_detail') }}" method="post" auto-refresh="true" auto-lock-button="true" custom-button-value="<i class='fas fa-trash-alt fa-fw me-1'></i> Delete" ref="formHTMLdelete" @submit="submitDataDeleteSubmenu($event)">
...
@pushonce('js')
	<script src="{{ url('assets/js/vue3/manage_menu/be/vueV3-manage-backend-submenu-2026.js?v=').time() }}"></script>
@endpushonce
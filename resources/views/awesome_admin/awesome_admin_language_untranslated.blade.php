...
@section('title')
	{{ t('Manage Untranslated') }}
@endsection

@section('content')
	<div>
		<div class="mb-3">
			{{ Breadcrumbs::render('awesome_admin.language.untranslated') }}
		</div>

		<div id="ph-app-data-language">
			<div class="ph-fetch-listdata" id="ph-form-submit-data-update" data-url="{{ url('awesome_admin/language/listdata/untranslated') }}">
				<form action="{{ route('cms.admin.awesome_admin.language.update.untranslated') }}" method="post" ref="formHTMLupdate" auto-refresh="true" @submit.prevent="submitDataLanguage($event, 'update')">

					<div class="ph-content rounded p-3 mb-3">
						<div class="row g-3">
							<div class="col-md-6 d-flex align-items-center">
								<h4 class="mb-0">{{ t('Manage Untranslated') }}</h4>
							</div>

							<div class="col-md-6 d-flex align-items-center justify-content-end">
								<input type="submit" class="btn ph-btn-theme btn-submit-data font-size-inherit" value="Save">							
							</div>
						</div>
					</div>
									...
									<table class="table table-striped table-hover mb-0">
										...
										<tbody is="transition-group" name="custom-classes-transition" enter-active-class="animate__animated animate__fadeIn animate__faster">
											<tr class="text-nowrap" v-for="(info, index) in responseData" v-bind:key="info.id">
												...
												<td class="align-middle">
													<div v-if="responseDataEditableStatus[index] == 'success'"><div class="badge text-bg-success">Sentences are saved and moved to translated</div></div>
												</td>

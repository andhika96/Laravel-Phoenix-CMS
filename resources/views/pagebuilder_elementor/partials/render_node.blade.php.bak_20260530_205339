...
@if(in_array($type, ['container', 'container_fluid'], true))
	@php
		...
		$targetCols = max(1, min(12, (int) ($s['gridColumns'] ?? 1)));
		$targetRows = $display === 'grid' ? $container_grid_rows_count($s['gridRows'] ?? null) : 1;
		$targetCells = max(1, min(144, $targetCols * $targetRows));

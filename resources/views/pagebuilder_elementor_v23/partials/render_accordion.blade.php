	...
	$advancedBackground = function (string $suffix = '') use ($accordionSettings, $gradient, $safeUrl): array {
		...
		if ($type === 'classic') {
			...
			if ($image !== '') {
				...
				$rules[] = 'background-position:' . ($accordionSettings['advancedBackgroundPosition' . $suffix] ?? 'center center');
				$rules[] = 'background-repeat:' . ($accordionSettings['advancedBackgroundRepeat' . $suffix] ?? 'no-repeat');
				$rules[] = 'background-size:' . ($accordionSettings['advancedBackgroundSize' . $suffix] ?? 'cover');
				$rules[] = 'background-attachment:' . ($accordionSettings['advancedBackgroundAttachment' . $suffix] ?? 'scroll');
			}
			return $rules;
		}
		...
		<details class="el-widget-accordion__item" data-accordion-item="{{ $safeItemToken }}" @if($safeItemId !== '') id="{{ $safeItemId }}" @endif @if($isOpen) open @endif>
			<summary id="{{ $summaryId }}" class="el-widget-accordion__summary" aria-expanded="{{ $isOpen ? 'true' : 'false' }}" aria-controls="{{ $panelId }}">
				@if($iconPosition === 'start')
					@include('pagebuilder_elementor_v23.partials.render_accordion_icon', ['expandIcon' => $expandIcon, 'collapseIcon' => $collapseIcon])
				@endif

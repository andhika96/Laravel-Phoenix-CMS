@php
	...
	$imageUrl = $imageUrl !== ''
		? app(\App\Support\PageBuilderElementorV23\ImageRenditionResolver::class)->resolve($imageUrl, $imageResolution, $imageResolution === 'custom' ? $customImageWidth : null, $imageResolution === 'custom' ? $customImageHeight : null)
		: '';
	...
	$imageStyle = implode(';', [
		'width:' . $cssLength($responsive('imageWidth', '', '30%'), '30%'),
		'max-width:100%',
		'display:block',
	...
	$advanced = app(\App\Support\PageBuilderElementorV23\WidgetAdvancedStyleResolver::class)->resolve($imageBoxSettings, $nodeId, request());
	...
	foreach (['Tablet' => 1024, 'Mobile' => 767] as $suffix => $breakpoint) {
		...
		$imageRules = [
			'width:' . $cssLength($responsive('imageWidth', $suffix, '30%'), '30%'),

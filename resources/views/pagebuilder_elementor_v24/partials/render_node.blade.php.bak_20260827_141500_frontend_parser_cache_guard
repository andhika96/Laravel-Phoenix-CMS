@php
	...
	$__pbConditionEvaluator = app(\App\Support\PageBuilderElementorV24\WidgetDisplayConditionEvaluator::class);
	if (!$__pbConditionEvaluator->allows($__pbConditionGroups, $__pbRequest, $__pbUser)) {
		return;
	}

	$__pbDiscoveredModule = app(\App\Support\PageBuilderElementorV24\ModuleCatalog::class)->find($type);
	$__pbDiscoveredView = is_array($__pbDiscoveredModule) ? ($__pbDiscoveredModule['assets']['view'] ?? '') : '';
	$__pbHasDiscoveredView = is_string($__pbDiscoveredView) && $__pbDiscoveredView !== '' && is_file($__pbDiscoveredView);

	if ($__pbHasDiscoveredView && ($settings['cacheMode'] ?? 'default') === 'active') {
		$__pbRoles = [];
		...
		$__pbFragmentCache = app(\App\Support\PageBuilderElementorV24\WidgetFragmentCache::class);
		$__pbFragmentData = ['node' => $node];
		if (isset($pageData)) $__pbFragmentData['pageData'] = $pageData;
		echo $__pbFragmentCache->remember($node, $__pbContext, fn () => view()->file($__pbDiscoveredView, $__pbFragmentData)->render());
		return;
	}
	...
	$shape_divider_shapes_path = resource_path('data/pagebuilder_elementor_v24_shapes.json');
	...
	$grid_column_style_defaults = function (): array {
		$defaults = [
			'borderType' => 'none', 'borderColor' => 'transparent',
			...
			'borderWidthLinked' => true, 'borderRadiusLinked' => true,
		...
		foreach (array_keys($defaults) as $key) {
			if (in_array($key, ['borderWidthLinked', 'borderRadiusLinked'], true)) continue;
			$defaults[$key . 'Tablet'] = '';
			$defaults[$key . 'Mobile'] = '';
		}
		return $defaults;
	...
	$normalize_grid_column_style = function ($source, bool $sparse = false) use ($grid_column_style_defaults): array {
		$input = is_array($source) ? $source : [];
		$defaults = $grid_column_style_defaults();
		$output = $sparse ? [] : $defaults;
		$borderTypes = ['none', 'solid', 'double', 'dotted', 'dashed', 'groove'];
		$backgroundTypes = ['none', 'color', 'gradient', 'image'];

		foreach ($defaults as $key => $default) {
			if (!array_key_exists($key, $input)) continue;
			$raw = $input[$key];
			if ($sparse && ($raw === '' || $raw === null)) continue;
			if (str_starts_with($key, 'borderType')) {
				$value = strtolower(trim((string) $raw));
				$output[$key] = in_array($value, $borderTypes, true) ? $value : ($sparse ? 'none' : $default);
				continue;
			}
			if (str_starts_with($key, 'bgType')) {
				$value = strtolower(trim((string) $raw));
				$output[$key] = in_array($value, $backgroundTypes, true) ? $value : ($sparse ? 'none' : $default);
				continue;
			}
			if (str_contains($key, 'Opacity')) {
				$output[$key] = is_numeric($raw) ? max(0, min(1, (float) $raw)) : ($sparse ? 1 : $default);
				continue;
			}
			if (in_array($key, ['bgGradientAngle', 'bgGradientPosition', 'bgGradientAngleTablet', 'bgGradientPositionTablet', 'bgGradientAngleMobile', 'bgGradientPositionMobile'], true)) {
				$output[$key] = is_numeric($raw) ? (float) $raw : ($sparse ? $raw : $default);
				continue;
			}
			$output[$key] = is_string($raw) ? trim($raw) : $raw;
		}

		return $output;
	};

	$grid_column_style_resolve = function ($trackStyle, $cellOverrides, string $deviceSuffix = '') use ($grid_column_style_defaults, $normalize_grid_column_style, $responsive_setting_value): array {
		$defaults = $grid_column_style_defaults();
		$track = $normalize_grid_column_style($trackStyle);
		$cell = $normalize_grid_column_style($cellOverrides, true);
		$resolved = [];
		foreach (array_keys($defaults) as $key) {
			if (str_ends_with($key, 'Tablet') || str_ends_with($key, 'Mobile')) continue;
			$cellValue = $responsive_setting_value($cell, $key, $deviceSuffix, null);
			$resolved[$key] = $cellValue !== null ? $cellValue : $responsive_setting_value($track, $key, $deviceSuffix, $defaults[$key]);
		}
		return $resolved;
	};

	$grid_column_style_safe_url = function ($value): string {
		$url = trim((string) ($value ?? ''));
		if ($url === '' || preg_match('/[\x00-\x1F\x7F"\'()]/', $url) || preg_match('/^(?:javascript|vbscript|data):/i', $url) || str_starts_with($url, '//')) return '';
		return preg_match('/^(?:https?:\/\/|\/)/i', $url) ? $url : '';
	};
	$grid_column_style_safe_color = function ($value, string $fallback = 'transparent'): string {
		$color = trim((string) ($value ?? ''));
		if ($color === '' || preg_match('/[;{}]/', $color)) return $fallback;
		return preg_match('/^(?:#[0-9a-f]{3,8}|(?:rgb|rgba|hsl|hsla)\([^(){};]+\)|[a-z]+)$/i', $color) ? $color : $fallback;
	};

	$grid_column_style_css = function (array $style) use ($css_value, $color_with_opacity, $grid_column_style_safe_url, $grid_column_style_safe_color): string {
		$type = in_array(($style['borderType'] ?? 'none'), ['none', 'solid', 'double', 'dotted', 'dashed', 'groove'], true) ? $style['borderType'] : 'none';
		$rules = [
			'box-sizing:border-box',
			'border-style:' . $type,
			'border-width:' . ($type === 'none' ? '0' : implode(' ', array_map(fn ($key) => $css_value($style[$key] ?? null, '0'), ['borderWidthTop', 'borderWidthRight', 'borderWidthBottom', 'borderWidthLeft']))),
			'border-color:' . $grid_column_style_safe_color($style['borderColor'] ?? null),
			'border-radius:' . implode(' ', array_map(fn ($key) => $css_value($style[$key] ?? null, '0'), ['borderRadiusTL', 'borderRadiusTR', 'borderRadiusBR', 'borderRadiusBL'])),
		];

		$backgroundType = $style['bgType'] ?? 'none';
		if ($backgroundType === 'color') {
			$color = $color_with_opacity($grid_column_style_safe_color($style['bgColor'] ?? null, '#ffffff'), $style['bgOpacity'] ?? 1);
			$rules[] = 'background-color:' . $color;
		} elseif ($backgroundType === 'gradient') {
			$start = $grid_column_style_safe_color($style['bgGradientStart'] ?? null, '#ffffff');
			$end = $grid_column_style_safe_color($style['bgGradientEnd'] ?? null, '#000000');
			$position = (float) ($style['bgGradientPosition'] ?? 50);
			$angle = (float) ($style['bgGradientAngle'] ?? 90);
			$rules[] = 'background-image:' . (($style['bgGradientType'] ?? 'linear') === 'radial'
				? 'radial-gradient(circle, ' . $start . ' 0%, ' . $end . ' ' . $position . '%)'
				: 'linear-gradient(' . $angle . 'deg, ' . $start . ' 0%, ' . $end . ' ' . $position . '%)');
		} elseif ($backgroundType === 'image') {
			$image = $grid_column_style_safe_url($style['bgImage'] ?? '');
			if ($image !== '') {
				$rules[] = 'background-image:url("' . $image . '")';
				$rules[] = 'background-size:' . (($style['bgSize'] ?? 'cover') === 'stretch' ? '100% 100%' : ($style['bgSize'] ?? 'cover'));
				$rules[] = 'background-position:' . ($style['bgPosition'] ?? 'center center');
				$rules[] = 'background-repeat:' . ($style['bgRepeat'] ?? 'no-repeat');
				$rules[] = 'background-attachment:' . ($style['bgAttachment'] ?? 'scroll');
			}
		}
	...
	$grid_column_style_has_responsive_values = function ($style): bool {
		if (!is_array($style)) return false;
		foreach ($style as $key => $value) {
			if ((str_ends_with((string) $key, 'Tablet') || str_ends_with((string) $key, 'Mobile')) && $value !== '' && $value !== null) return true;
		}
		return false;
	};
	...
	if ($__pbHasDiscoveredView) {
		$__pbWidgetData = get_defined_vars();
		$__pbWidgetData['node'] = $node;
		if (isset($pageData)) $__pbWidgetData['pageData'] = $pageData;
		echo view()->file($__pbDiscoveredView, $__pbWidgetData)->render();
		return;
	}

	echo '<!-- Inactive Page Builder v2.4 module: ' . e($type) . ' -->';
	return;
@endphp

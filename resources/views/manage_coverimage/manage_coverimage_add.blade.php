...
@section('content')
	<div id="ph-app-manage-coverimage">
		<div class="mb-3">
			{{ Breadcrumbs::render('manage_coverimage.add') }}
		</div>

		<div>
			<div id="ph-form-submit-data">
				<form action="{{ route('cms.core.manage_coverimage.store') }}" method="post" ref="formHTML" @submit.prevent="submitData" auto-refresh="false" auto-reset="true" auto-lock-button="false" auto-block-button-mobile="false" custom-button-value="{{ t('Save') }}">
					
					<div id="ph-content-button-submit" class="arv7-content rounded p-3 mb-3 sticky-top">
						<div class="row g-3">
							<div class="col-md-6 d-flex align-items-center">
								<h4 class="mb-0"><i class="fad fa-plus fa-fw me-1"></i> {{ t('Add Cover Image') }}</h4>
							</div>
					...
					<div class="arv7-content rounded p-4 mb-3">
						<div class="row g-3">
							<div class="col-md-4">
								...
								<input type="text" name="uri" class="form-control font-size-inherit">
							</div>

							<div class="col-md-4">
								<label class="form-label">{{ t('Cover Page Name') }}</label>
								<input type="text" name="cover_page_name" class="form-control font-size-inherit">
							</div>
							...
							<div class="col-md-4">
								...
								<select name="cover_type" class="form-select" aria-label="Default select example" v-on:change="changeCoverType($event)">
									<option value="background_image">{{ t('Only Background Image') }}</option>
									<option value="slideshow">{{ t('Slideshow') }}</option>
								</select>
							</div>

							<div class="col-md">
								<label class="form-label">{{ t('Slideshow Direction') }}</label>
								<select name="cover_slideshow_direction" class="form-select" aria-label="Default select example" :disabled="getFormCoverImage.cover_type == 'slideshow' ? false : true">
							...
							<div class="col-md">
								...
								<input type="text" name="cover_autoplay_slideshow_interval" class="form-control font-size-inherit" :disabled="getFormCoverImage.cover_type == 'slideshow' ? false : true">
							</div>
														...
														<h6 class="accordion-header rounded">
															<div class="input-group">
																...
																<button class="accordion-button accordion-button-background-image collapsed" type="button" data-bs-toggle="collapse" :data-bs-target="'#panelsSlideshow-collapse'+index" aria-expanded="true" :aria-controls="'panelsSlideshow-collapse'+index" v-on:click="accordionButton(index)">
																	...
																	<span v-if="getListFormBackgroundImage[index]['title'] !== '' && typeof getListFormBackgroundImage[index]['title'] !== 'undefined'" class="ms-2" v-cloak>( @{{ truncateText(removeHtmlTag(getListFormBackgroundImage[index]['title']), 50) }} )</span>
																	<span :class="getListFormBackgroundImage[index]['cover_is_active'] == 'active' ? 'text-success' : 'text-danger'" class="ms-2">@{{ ucFirst(getListFormBackgroundImage[index]['cover_is_active']) }}</span>
																</button>
															</div>
														</h6>
						...
						<div class="modal-content">
							...
							<div class="modal-footer pb-5 d-block border-0">
								<div class="row gx-2 justify-content-center">
									<div class="col-auto">	
										<button type="button" class="btn btn-secondary font-size-inherit" v-on:click="closeModal('ph-submit-data-coverimage-modalDelete')">{{ t('No, keep it') }}</button>
									</div>

									<div class="col-auto">
										<input type="hidden" class="index-form-list">
										<input type="hidden" class="type-form-list">
										<button type="button" class="btn btn-larapx btn-submit-data font-size-inherit" v-on:click="deleteFormCoverImage('ph-submit-data-coverimage-modalDelete', 'add')">{{ t('Yes, Delete') }}</button>
									</div>
								</div>
							</div>

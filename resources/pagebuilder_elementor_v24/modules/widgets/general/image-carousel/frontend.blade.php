@php
	...
	$resolver = app(\App\Support\PageBuilderElementorV24\ImageRenditionResolver::class);
	$images = [];
	...
	$sideValues = function (string $base, string $suffix = '', string $fallback = '0px') use ($responsive, $cssLength): string {
		return collect(['Top', 'Right', 'Bottom', 'Left'])
			->map(fn (string $side): string => $cssLength($responsive($base.$side, $suffix, $fallback), $fallback))
			->implode(' ');
	};
	$arrowPosition = fn (string $suffix = ''): string => $enum($responsive('arrowPosition', $suffix, 'inside'), ['inside', 'outside'], 'inside');
	$legacyArrowButtonSize = function (string $suffix = '') use ($responsive, $cssLength): string {
		$legacySize = $cssLength($responsive('arrowSize', $suffix, '16px'), '16px');
		return preg_match('/^(-?\d+(?:\.\d+)?)px$/', $legacySize, $match)
			? ((float) $match[1] + 12).'px'
			: '28px';
	};
	$arrowButtonSize = fn (string $suffix = ''): string => $cssLength($responsive('arrowButtonSize', $suffix, $legacyArrowButtonSize($suffix)), $legacyArrowButtonSize($suffix));
	$arrowEdgeOffset = fn (string $suffix = ''): string => $cssLength($responsive('arrowEdgeOffset', $suffix, $arrowPosition($suffix) === 'outside' ? '0px' : '8px'), $arrowPosition($suffix) === 'outside' ? '0px' : '8px');
	$arrowPadding = fn (string $suffix = ''): string => $arrowPosition($suffix) === 'outside' ? 'calc('.$arrowButtonSize($suffix).' + '.$arrowEdgeOffset($suffix).' + 4px)' : '0px';
	...
	$rootClasses = array_values(array_unique(array_merge(['el-widget-image-carousel', 'pb-image-carousel', 'is-arrows-'.$arrowPosition(), 'is-pagination-'.$enum($settings['paginationPosition'] ?? 'outside', ['inside', 'outside'], 'outside')], $advanced['classes'])));
	$rootStyle = implode(';', [
		'--pb-carousel-visible:'.$visibleCount(),
		'--pb-carousel-arrow-button-size:'.$arrowButtonSize(),
		'--pb-carousel-arrow-icon-size:'.$cssLength($responsive('arrowIconSize', '', $responsive('arrowSize', '', '16px')), '16px'),
		'--pb-carousel-arrow-edge-offset:'.$arrowEdgeOffset(),
		'--pb-carousel-arrow-padding:'.$arrowPadding(),
		'--pb-carousel-arrow-color:'.$cssColor($settings['arrowColor'] ?? '', '#69727d'),
		'--pb-carousel-arrow-background:'.$cssColor($settings['arrowBackground'] ?? '', 'rgba(255,255,255,.88)'),
		'--pb-carousel-arrow-hover-color:'.$cssColor($settings['arrowHoverColor'] ?? '', $cssColor($settings['arrowColor'] ?? '', '#69727d')),
		'--pb-carousel-arrow-hover-background:'.$cssColor($settings['arrowHoverBackground'] ?? '', $cssColor($settings['arrowBackground'] ?? '', 'rgba(255,255,255,.88)')),
		'--pb-carousel-arrow-radius:'.$sideValues('arrowRadius', '', '50%'),

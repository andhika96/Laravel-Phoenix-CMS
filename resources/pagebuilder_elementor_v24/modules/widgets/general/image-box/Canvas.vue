...
export default {
	...
	computed: {
		...
		imageWidth() {
			return this.cssSize(this.responsiveValue('imageWidth', '30%'), '30%');
		},
		mediaStyle() {
			return this.position === 'top'
				? { width: '100%', flex: '0 0 auto', maxWidth: '100%' }
				: { width: this.imageWidth, flex: `0 0 ${this.imageWidth}`, maxWidth: '100%' };
		},
		imageStyle() {
			const borderType = BORDER_TYPES.includes(this.settings.imageBorderType) ? this.settings.imageBorderType : 'none';
			return {
				width: this.position === 'top' ? this.imageWidth : '100%',
				maxWidth: '100%',
				display: 'block',
	...
	methods: {
		...
		async resolveImageRendition() {
			...
			const endpoint = String(window.PAGE_BUILDER_ELEMENTOR_V24_CONTEXT?.imageRenditionUrl || '');
...
.pb-image-box__image-link { display: flex; width: 100%; max-width: 100%; justify-content: var(--pb-image-box-media-justify, center); color: inherit; }

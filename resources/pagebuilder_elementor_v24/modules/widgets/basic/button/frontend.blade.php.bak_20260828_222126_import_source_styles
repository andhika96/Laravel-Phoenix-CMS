@php
	...
	$buttonStyle = function (string $suffix = '') use ($settings, $responsive, $cssLength, $cssColor, $background, $border, $shadow, $fontStyle, $duration, $buttonHasIcon, $buttonIconPosition): string {
	...
	$styleBlocks = ['#' . $nodeDomId . ' .el-widget-button:hover{' . $hoverRules . '}', '#' . $nodeDomId . ' .el-widget-button__icon{display:inline-flex;align-items:center;justify-content:center;line-height:1}', '#' . $nodeDomId . ' .el-widget-button__icon-svg{width:1em;height:1em;object-fit:contain}', '#' . $nodeDomId . ' .el-widget-button__text{white-space:nowrap}'];

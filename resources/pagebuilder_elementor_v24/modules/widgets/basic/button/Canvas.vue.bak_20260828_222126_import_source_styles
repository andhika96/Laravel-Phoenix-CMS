<template>
	<div class="pb-basic-button-canvas" :style="wrapperStyle">
		<a :href="href" :target="target" :rel="rel" :class="['el-widget-button', buttonClass]" :style="buttonStyle"><template v-if="hasIcon"><img v-if="iconSource === 'svg'" class="el-widget-button__icon-svg" :src="iconSvgDataUri" alt="" aria-hidden="true"><i v-else class="el-widget-button__icon" :class="iconClass" aria-hidden="true"></i></template><span class="el-widget-button__text">{{ text }}</span></a>
	</div>
</template>

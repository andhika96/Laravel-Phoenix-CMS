    ...
    >
        ...
        >
            ...
            <div class="pb-pro-video-playlist__body">
                ...
                <aside class="pb-pro-video-playlist__playlist-panel">
                    <button type="button" class="pb-pro-video-playlist__dropdown-toggle" :class="{ 'is-active': playlistDropdownExpanded }" :style="videoPlaylistDropdownStyle" data-playlist-dropdown-toggle data-pb-interactive="true" :aria-expanded="playlistDropdownExpanded ? 'true' : 'false'" :aria-controls="item.id + '-playlist-items'" aria-label="Toggle playlist" @click.stop="playlistDropdownExpanded = !playlistDropdownExpanded">
                        <span v-if="videoPlaylistDropdownIconSvg" class="pb-pro-icon-svg" v-html="videoPlaylistDropdownIconSvg"></span><i v-else-if="videoPlaylistDropdownIconClass" :class="videoPlaylistDropdownIconClass"></i>
                    </button>
                    <nav v-show="playlistDropdownExpanded" :id="item.id + '-playlist-items'" class="pb-pro-video-playlist__items" aria-label="Playlist videos">
                        <button v-for="(entry, index) in videoPlaylistItems" :key="entry.id || index" type="button" class="pb-pro-video-playlist__item" :class="{ 'is-active': index === activePlaylistIndex, 'is-watched': playlistWatched.includes(index) }" :data-playlist-index="index" :style="videoPlaylistItemStyle(index)" @click.stop="selectVideoPlaylistItem(index)">
                            <span v-if="s.showThumbnails !== false" class="pb-pro-video-playlist__thumbnail"><img v-if="videoPlaylistThumbnail(entry)" :src="videoPlaylistThumbnail(entry)" alt="" /><span v-else class="pb-pro-video-playlist__thumbnail-placeholder"><span v-if="videoPlaylistItemIconSvg(index)" class="pb-pro-icon-svg" v-html="videoPlaylistItemIconSvg(index)"></span><i v-else-if="videoPlaylistIconClass(index)" :class="videoPlaylistIconClass(index)"></i></span></span>
                            ...
                            <span class="pb-pro-video-playlist__item-icon" :style="videoPlaylistIconStyle(index)"><span v-if="videoPlaylistItemIconSvg(index)" class="pb-pro-icon-svg" v-html="videoPlaylistItemIconSvg(index)"></span><i v-else-if="videoPlaylistIconClass(index)" :class="videoPlaylistIconClass(index)"></i></span>
                        </button>
                    </nav>
                </aside>
            </div>
...
export default {
    ...
    computed: {
        ...
        videoPlaylistRootStyle() {
            return {
                ...
                "--video-playlist-dropdown-color": this.safeColor(this.s.dropdownIconColor, "#667085"),
                "--video-playlist-dropdown-hover": this.safeColor(this.s.dropdownIconColorHover, "#101828"),
                "--video-playlist-dropdown-active": this.safeColor(this.s.dropdownIconColorActive, "#6979f8"),
        ...
        videoPlaylistSectionStyle() {
            return { background: this.videoPlaylistSectionBackground, border: "var(--video-playlist-section-border-width) var(--video-playlist-section-border-type, solid) var(--video-playlist-section-border-color)", borderRadius: "var(--video-playlist-section-radius)", padding: "var(--video-playlist-section-padding)", boxShadow: this.safeTextShadow(this.s.sectionBoxShadow, "none") };
        },
        videoPlaylistSectionBackground() {
            const type = this.safeEnum(this.s.sectionBackgroundType, ["classic", "gradient", "none"], "classic");
            if (type === "none") return "transparent";
            if (type === "gradient") {
                const angle = Math.max(0, Math.min(360, Number(this.s.sectionGradientAngle) || 0));
                return `linear-gradient(${angle}deg, ${this.safeColor(this.s.sectionGradientColorOne, "#f8fafc")}, ${this.safeColor(this.s.sectionGradientColorTwo, "#eef2ff")})`;
            }
            return this.safeColor(this.s.sectionBackground, "#f8fafc");
        },
        ...
        videoPlaylistIconStyle() {
            return () => ({ color: this.safeColor(this.s.iconColor, "#6979f8"), background: "var(--video-playlist-icon-background)", boxShadow: this.safeTextShadow(this.s.iconShadow, "none"), fontSize: "var(--video-playlist-icon-size)" });
        },
        videoPlaylistDropdownStyle() {
            const alignment = this.safeEnum(this.s.dropdownAlignment, ["left", "center", "right"], "right");
            return { justifyContent: { left: "flex-start", center: "center", right: "flex-end" }[alignment] };
        },
        videoPlaylistDropdownIconClass() {
            return this.proIconClass(this.s, "dropdownIcon", "fas fa-chevron-down");
        },
        videoPlaylistDropdownIconSvg() {
            return this.proIconSvg(this.s, "dropdownIcon");
        },
    ...
    methods: {
        ...
        videoPlaylistItemIconSvg(index) {
            const watched = this.s.indicateWatched && this.playlistWatched.includes(index);
            return this.proIconSvg(this.s, watched ? "playedIcon" : "playIcon");
        },
...
.pb-pro-video-playlist.position-right .pb-pro-video-playlist__playlist-panel { order: -1; }
...
.pb-pro-video-playlist__playlist-panel { min-width: 0; background: var(--video-playlist-item-background, #fff); }
.pb-pro-video-playlist__dropdown-toggle {
    display: flex;
    width: 100%;
    min-height: 34px;
    align-items: center;
    ...
    background: transparent;
    color: var(--video-playlist-dropdown-color, #667085);
    cursor: pointer;
}
.pb-pro-video-playlist__dropdown-toggle:hover,
.pb-pro-video-playlist__dropdown-toggle:focus-visible { color: var(--video-playlist-dropdown-hover, #101828); }
.pb-pro-video-playlist__dropdown-toggle.is-active { color: var(--video-playlist-dropdown-active, #6979f8); }
...
@media (max-width: 700px) {
    ...
    .pb-pro-video-playlist.position-right .pb-pro-video-playlist__playlist-panel { order: 0; }

...
function initProCarousel(root) { bindProSlider(root, true); }
	...
	function init(scope) {
		...
		root.querySelectorAll("[data-pro-carousel].pb-pro-carousel:not(.pb-pro-reviews):not(.pb-pro-testimonial-carousel):not(.pb-pro-media-carousel)").forEach(initProCarousel);
	}

	const runtimes = window.PageBuilderElementorV24ModuleRuntimes ||= {};
	runtimes["carousel"] = Object.freeze({ init, initProCarousel });

@php
    ...
    $sliderTitleTagFontSizes = [
        'desktop' => ['h1' => '60px', 'h2' => '52px', 'h3' => '44px', 'h4' => '36px', 'h5' => '28px', 'h6' => '20px', 'div' => '52px'],
        'tablet' => ['h1' => '50px', 'h2' => '42px', 'h3' => '36px', 'h4' => '30px', 'h5' => '24px', 'h6' => '18px', 'div' => '42px'],
        'mobile' => ['h1' => '42px', 'h2' => '34px', 'h3' => '29px', 'h4' => '24px', 'h5' => '20px', 'h6' => '16px', 'div' => '34px'],
    ];
    $sliderTitleSize = static function (array $slide, $device = 'desktop') use ($settings, $responsiveLength, $cssLength, $slideResponsive, $sliderTitleTagFontSizes, $enum) {
        $device = in_array($device, ['desktop', 'tablet', 'mobile'], true) ? $device : 'desktop';
        $titleTag = $enum($slide['titleTag'] ?? 'h2', ['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'div'], 'h2');
        $automatic = $sliderTitleTagFontSizes[$device][$titleTag] ?? $sliderTitleTagFontSizes[$device]['h2'];
        $globalMode = strtolower(trim((string) ($settings['titleFontSizeMode'] ?? 'auto'))) === 'custom' ? 'custom' : 'auto';
        $globalSize = $globalMode === 'custom' ? $responsiveLength('titleFontSize', $device, $automatic) : $automatic;
        $slideMode = strtolower(trim((string) ($slide['slideTitleFontSizeMode'] ?? 'auto'))) === 'custom' ? 'custom' : 'auto';
        if (!empty($slide['styleOverride']) && $slideMode === 'custom') {
            $suffix = $device === 'mobile' ? 'Mobile' : ($device === 'tablet' ? 'Tablet' : '');
            return $cssLength($slideResponsive($slide, 'slideTitleFontSize', $suffix, $globalSize), $globalSize);
        }
        return $globalSize;
    };
    ...
    foreach ($slides as $index => $rawSlide) {
        ...
        $slideTitleSizeMode = strtolower(trim((string) ($slide['slideTitleFontSizeMode'] ?? 'auto')));
        if (!in_array($slideTitleSizeMode, ['auto', 'custom'], true)) {
            $hasLegacySlideTitleSize = false;
            foreach (['slideTitleFontSize', 'slideTitleFontSizeTablet', 'slideTitleFontSizeMobile'] as $sizeKey) {
                if (trim((string) ($slide[$sizeKey] ?? '')) !== '') {
                    $hasLegacySlideTitleSize = true;
                    break;
                }
            }
            $slideTitleSizeMode = $hasLegacySlideTitleSize ? 'custom' : 'auto';
        }
        $normalizedSlides[] = [
            ...$slide,
            ...
            'slideTitleFontSizeMode' => $slideTitleSizeMode,
        ];
    }
...
#{{ $heroId }} .pb-hero-slider__slide:nth-child({{ $index + 1 }}){--hero-slider-title-size:{{ $sliderTitleSize($slide, 'desktop') }} !important}
...
#{{ $heroId }}{min-height:{{ $tabletMinHeight }};--hero-slider-title-size:{{ $sliderTitleSize($normalizedSlides[0] ?? [], 'tablet') }};--hero-slider-subtitle-size:{{ $cssLength($settings['subtitleFontSizeTablet'] ?? '', '18px') }};--hero-slider-content-gap:{{ $cssLength($settings['contentGapTablet'] ?? '', '10px') }};--hero-slider-button-radius:{{ $buttonRadiusTablet }};--hero-slider-button-padding:{{ $buttonPaddingTablet }};--hero-slider-arrow-button-size:{{ $responsiveLength('arrowButtonSize', 'tablet', '38px') }};--hero-slider-arrow-icon-size:{{ $responsiveLength('arrowIconSize', 'tablet', '16px') }};--hero-slider-arrow-edge-position:{{ $arrowEdgePosition('tablet', 'Tablet') }};--hero-slider-arrow-overflow:{{ $arrowPlacementTablet === 'outside' ? 'visible' : 'hidden' }};--hero-slider-arrow-radius:{{ $arrowRadiusTablet }};{{ $heightMode === 'fixed' ? 'height:' . $fixedHeightTablet . ';' : '' }}}
@foreach($normalizedSlides as $index => $slide)

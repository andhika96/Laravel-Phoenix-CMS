<template>
    ...
    >
        ...
        >
            ...
            <div v-if="s.skin === 'slideshow'" class="pb-pro-media-carousel__thumbnails" :class="{ 'is-centered': s.centeredSlides }" :style="mediaCarouselThumbnailsStyle" role="tablist">
                <div class="pb-pro-media-carousel__thumbnail-track" :style="mediaCarouselThumbnailTrackStyle">
                    <button
                        v-for="(entry, index) in s.items"
                        :key="'thumb-' + entry.id"
                        type="button"
                        ...
                        :style="mediaCarouselThumbnailStyle"
...
export default {
    ...
    computed: {
        ...
        mediaCarouselThumbsVisible() {
            const value = Number(this.responsiveValue("thumbsSlidesToShow", 5)) || 5;
            return Math.max(1, Math.min(10, value));
        },
        mediaCarouselThumbsRatio() {
            return this.safeEnum(this.s.thumbsRatio, ["1:1", "4:3", "16:9", "21:9"], "21:9").replace(":", " / ");
        },
        mediaCarouselThumbnailsStyle() {
            return {
                "--media-thumbs-per-view": this.mediaCarouselThumbsVisible,
                "--media-thumbs-ratio": this.mediaCarouselThumbsRatio,
            };
        },
        mediaCarouselThumbnailTrackStyle() {
            const offset = this.s.centeredSlides
                ? 50 - ((this.activeIndex + 0.5) * 100 / this.mediaCarouselThumbsVisible)
                : 0;
            return { transform: `translate3d(${Number(offset.toFixed(4))}%,0,0)` };
        },
        mediaCarouselThumbnailStyle() {
            const count = this.mediaCarouselThumbsVisible;
            return {
                flexBasis: `calc((100% - ${(count - 1)} * var(--carousel-dot-gap, 8px)) / ${count})`,
                aspectRatio: this.mediaCarouselThumbsRatio,
            };
        },
...
.pb-pro-media-carousel__thumbnail-track {
    display: flex;
    width: 100%;
    gap: var(--carousel-dot-gap, 8px);
...
.pb-pro-media-carousel__thumbnail {
    flex: 0 0 calc((100% - (var(--media-thumbs-per-view, 5) - 1) * var(--carousel-dot-gap, 8px)) / var(--media-thumbs-per-view, 5));
    aspect-ratio: var(--media-thumbs-ratio, 21 / 9);
    padding: 2px;
    overflow: hidden;
...
.pb-pro-media-carousel__thumbnail img { width: 100%; height: 100%; object-fit: cover; }

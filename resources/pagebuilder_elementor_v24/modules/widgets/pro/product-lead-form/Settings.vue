<template>
    ...
    >
        ...
        <div v-if="editor.settingsTab === 'content'" class="pb-tab-content">
                <section-box title="Data Source" :open="true">
                    <div class="pb-product-dataset-summary">
                        <span><i class="fas fa-database" aria-hidden="true"></i><strong>{{ productDatasetLabel }}</strong></span>
                        <button type="button" class="pb-btn" @click="openProductDatasetModal">Manage Product Dataset</button>
                    </div>
                    <p class="pb-form-note">Level 1 uses root items. Add child items for Level 2, then grandchildren for Level 3. Use the + button beside a dataset node to build the hierarchy.</p>
                </section-box>
                <section-box title="Selector & Levels" :open="true">
                    <select-control label="Number of Levels" v-model.number="s.productLevelCount" :options="productLevelCountOptions" />
                    <text-control label="Selector Title" v-model="s.productSelectorTitle" />
                    <toggle-control label="Show Selector Title" v-model="s.showProductSelectorTitle" />
                    <div v-for="(item,index) in activeProductLevelSettings" :key="item.key" class="pb-product-level-settings">
                        <strong>Level {{ index + 1 }} · {{ item.label }}</strong>
                        <text-control label="Label" v-model="item.label" />
                        <text-control label="Submission Field ID" v-model="item.fieldId" />
                        <text-control label="Query Key" v-model="item.queryKey" />
                        <select-control label="Presentation" v-model="item.presentation" :options="productPresentationOptions" />
                        <toggle-control label="Required" v-model="item.required" />
                        <select-control label="Default Item" v-model="item.defaultNodeId" :options="productDefaultOptions(index)" />
                    </div>
                    <toggle-control label="Synchronize Query URL" v-model="s.syncProductQuery" />
                </section-box>
                <section-box title="Product Presentation">
                ...
                <section-box title="Form Structure" :open="true"
                    ><text-control
                        label="Form Name"
                        ...
                        ><template #step="{ step }"><pro-icon-picker
                            ...
                            ><text-control
                                ...
                                label="Type"
                                v-model="item.type"
                                :options="fieldTypes" /><text-control
                                label="ID"
                                v-model="item.id" /><text-control
                                v-if="
                            ...
                            ><summary>More field options</summary><div class="pb-form-field-more__body"><text-control
                                ...
                                v-if="
                                    formTypesWithAutocomplete.includes(
                                        item.type,
                                    )
                                "
                                label="Autocomplete"
                                v-model="item.autocomplete" /><textarea-control
                                v-if="
                                    ['checkbox', 'radio'].includes(
                                        item.type,
                                    )
                                "
                                label="Options (Label|value per line)"
                                v-model="item.optionsText" /><button
                                v-if="item.type === 'select'"
                                type="button"
                                class="pb-form-dataset-trigger"
                                @click="openFormDatasetModal(item)"
                            ><span class="pb-form-dataset-trigger__icon"><i class="fas fa-database"></i></span><span><strong>Manage dataset</strong><small>{{ item.datasetMode === 'dataset' && item.datasetId ? 'Shared dataset connected' : 'Options and conditional logic' }}</small></span><i class="fas fa-external-link-alt"></i></button><toggle-control
                                v-if="item.type === 'select'"
                                label="Multiple Selection"
                                v-model="item.multiple" /><toggle-control
                                v-if="['checkbox', 'radio'].includes(item.type)"
                                label="Inline List"
                                v-model="item.inlineList" /><number-control
                                ...
                                :max="20" /><text-control
                                v-if="
                                    ['number', 'date', 'time'].includes(
                                        item.type,
                                    )
                                "
                                label="Min"
                                v-model="item.min" /><text-control
                                v-if="
                                    ['number', 'date', 'time'].includes(
                                        item.type,
                                    )
                                "
                                label="Max"
                                v-model="item.max" /><text-control
                                v-if="item.type === 'number'"
                                label="Step"
                                v-model="item.step" /><textarea-control
                                v-if="item.type === 'acceptance'"
                                ...
                                v-if="item.type === 'html'"
                                label="HTML"
                                v-model="item.html" /><text-control
                                v-if="item.type === 'file'"
                    ...
                    ><select-control
                        label="Input Size"
                        v-model="s.inputSize"
                        :options="sizeOptions" /><toggle-control
                        label="Label"
                        v-model="s.showLabels" /><toggle-control
                        label="Required Mark"
                        v-model="s.markRequired"
                /></section-box>
                <section-box title="Submit Button"
                    ><select-control
                        label="Size"
                        ...
                        label="Button Text"
                        v-model="s.buttonText" /><pro-icon-picker
                        label="Icon"
                        prefix="buttonIcon"
                        target-key="formButton"
                        fallback-source="none"
                        :entry="s"
                        :node="node"
                        :editor="editor" /><select-control
                        label="Icon Position"
                        v-model="s.buttonIconPosition"
                        :options="iconPositionOptions" /><size-control
                        label="Icon Spacing"
                        base="buttonIconSpacing"
                        fallback="5px"
                        :node="node"
                        :editor="editor" /><text-control
                        label="Button ID"
                        v-model="s.buttonId"
                /></section-box>
                <section-box title="Submit Actions"
                    ><label
                        v-for="action in formActionOptions"
                        :key="action.value"
                        class="pb-pro-action-option"
                        ><input
                            type="checkbox"
                            :checked="formActionEnabled(action.value)"
                            @change="
                                toggleFormAction(
                                    action.value,
                            ...
                            "
                        /><span>{{ action.label }}</span></label
                    ></section-box
                >
                <section-box v-if="formActionEnabled('email')" title="Email Delivery"
                    ><text-control
                        label="To"
                        v-model="s.emailTo" /><text-control
                        label="Subject"
                        v-model="s.emailSubject" /><textarea-control
                        label="Message"
                        v-model="s.emailContent" /><text-control
                        label="From Email"
                        v-model="s.emailFrom" /><text-control
                        label="From Name"
                        v-model="s.emailFromName" /><text-control
                        label="Reply-To"
                        v-model="s.emailReplyTo" /><text-control
                        label="Cc"
                        v-model="s.emailCc" /><text-control
                        label="Bcc"
                        v-model="s.emailBcc" /><select-control
                        label="Send As"
                        v-model="s.emailContentType"
                        :options="[
                            { value: 'html', label: 'HTML' },
                            { value: 'plain', label: 'Plain' },
                        ]"
                /></section-box>
                <section-box v-if="formActionEnabled('email2')" title="Secondary Email Delivery"
                    ><text-control
                        label="To"
                        v-model="s.email2To" /><text-control
                        label="Subject"
                        v-model="s.email2Subject" /><textarea-control
                        label="Message"
                        v-model="s.email2Content" /><text-control
                        label="From Email"
                        v-model="s.email2From" /><text-control
                        label="From Name"
                        v-model="s.email2FromName" /><text-control
                        label="Reply-To"
                        v-model="s.email2ReplyTo" /><text-control
                        label="Cc"
                        v-model="s.email2Cc" /><text-control
                        label="Bcc"
                        v-model="s.email2Bcc" /><select-control
                        label="Send As"
                        v-model="s.email2ContentType"
                        :options="emailContentTypeOptions"
                /></section-box>
                <section-box
                    v-if="formActionEnabled('redirect')"
                    title="Redirect Action"
                    ><text-control label="Redirect To" v-model="s.redirectUrl"
                /></section-box>
                <section-box v-if="formActionEnabled('webhook')" title="Webhook Action"
                    ><text-control label="Webhook URL" v-model="s.webhookUrl"
                /></section-box>
                <section-box title="Multi-step"
                    ><select-control
                        label="Type"
                        v-model="s.stepType"
                ...
                <section-box title="Submission Messages" :open="s.customMessages"
                    ><toggle-control
                ...
                ></section-box>
                <section-box title="Form Identity & Validation"
                    ><text-control
                        label="Form ID"
        ...
        <div v-if="editor.settingsTab === 'style'" class="pb-tab-content">
                <section-box title="Overall Layout" :open="true">
                ...
                <section-box title="Product Detail Media">
                    <size-control label="Image Width" base="productImageWidth" fallback="100%" :node="node" :editor="editor" />
                    <size-control label="Image Height" base="productImageHeight" fallback="420px" :node="node" :editor="editor" />
                    <select-control label="Object Fit" v-model="s.productImageFit" :options="productImageFitOptions" />
                    <select-control label="Border Type" v-model="s.productImageBorderType" :options="productBorderTypeOptions" />
                    <size-control v-if="s.productImageBorderType !== 'none'" label="Border Width" base="productImageBorderWidth" fallback="1px" :node="node" :editor="editor" />
                ...
                <section-box title="Selector Cards">
                    <select-control
                        label="Editing Level"
                        :model-value="String(productStyleLevelIndexSafe)"
                        :options="productStyleLevelOptions"
                        @update:modelValue="setProductStyleLevel"
                    />
                    <div class="pb-product-level-style__group-label">State Appearance</div>
                    <div class="pb-state-tabs pb-state-tabs--three">
                        <button v-for="state in ['normal','hover','selected']" :key="state" type="button" :class="{active:productStyleState===state}" @click="setProductStyleState(state)">{{ state[0].toUpperCase()+state.slice(1) }}</button>
                    </div>
                    <details v-for="(item,index) in activeProductLevelStyles" v-show="index===productStyleLevelIndexSafe" :key="'style-'+index" class="pb-product-level-style" open>
                        <summary>{{ activeProductLevelSettings[index]?.label || ('Level '+(index+1)) }}</summary>
                        <div class="pb-product-level-style__body">
                            <div class="pb-product-level-style__group-label">Card Layout</div>
                            <number-control label="Desktop Columns" v-model="item.columns" :min="1" :max="8" />
                            <number-control label="Tablet Columns" v-model="item.columnsTablet" :min="1" :max="8" />
                            <number-control label="Mobile Columns" v-model="item.columnsMobile" :min="1" :max="4" />
                            <size-control label="Cards Gap" base="gap" :control-id="'product-level-'+index+'-gap'" fallback="16px" :target="item" :node="node" :editor="editor" :max="200" :allowed-units="['px','%','em','rem']" />
                            <responsive-select label="Card Alignment" base="cardAlign" :control-id="'product-level-'+index+'-alignment'" :target="item" :node="node" :editor="editor" :options="productCardAlignOptions" />
                            <div class="pb-product-level-style__group-label">Card Dimensions</div>
                            <size-control label="Card Width" base="cardWidth" :control-id="'product-level-'+index+'-width'" fallback="100%" :target="item" :node="node" :editor="editor" :max="1200" :allowed-units="['%','px','em','rem','vw']" />
                            <size-control label="Card Min Width" base="cardMinWidth" :control-id="'product-level-'+index+'-min-width'" fallback="0px" :target="item" :node="node" :editor="editor" :max="1200" :allowed-units="['px','em','rem','vw']" />
                            <select-control label="Card Height Mode" v-model="item.cardHeightMode" :options="productCardHeightModeOptions" />
                            <size-control v-if="item.cardHeightMode === 'fixed'" label="Card Height" base="cardHeight" :control-id="'product-level-'+index+'-height'" fallback="220px" :target="item" :node="node" :editor="editor" :max="1000" :allowed-units="['px','em','rem','vh']" />
                            <div class="pb-product-level-style__group-label">Card Spacing</div>
                            <sides-control label="Card Padding" base="cardPadding" :control-id="'product-level-'+index+'-padding'" kind="edges" :fallback-values="['16px','16px','16px','16px']" :target="item" :node="node" :editor="editor" />
                            <sides-control label="Card Margin" base="cardMargin" :control-id="'product-level-'+index+'-margin'" kind="edges" :fallback-values="['0px','0px','0px','0px']" :target="item" :node="node" :editor="editor" />
                            <size-control label="Content Gap" base="contentGap" :control-id="'product-level-'+index+'-content-gap'" fallback="12px" :target="item" :node="node" :editor="editor" :max="100" :allowed-units="['px','%','em','rem']" />
                            <div class="pb-product-level-style__group-label">Card Media</div>
                            <size-control label="Image Width" base="imageWidth" :control-id="'product-level-'+index+'-image-width'" fallback="120px" :target="item" :node="node" :editor="editor" :max="600" :allowed-units="['px','%','em','rem','vw']" />
                            <size-control label="Image Height" base="imageHeight" :control-id="'product-level-'+index+'-image-height'" fallback="120px" :target="item" :node="node" :editor="editor" :max="600" :allowed-units="['px','%','em','rem','vh']" />
                            <sides-control label="Thumbnail Padding" base="imagePadding" :control-id="'product-level-'+index+'-image-padding'" kind="edges" :fallback-values="['0px','0px','0px','0px']" :target="item" :node="node" :editor="editor" />
                            <select-control label="Image Fit" v-model="item.imageFit" :options="productImageFitOptions" />
                            <select-control label="Image Shape" v-model="item.imageShape" :options="productImageShapeOptions" />
                            <select-control label="Label Position" v-model="item.imageLabelPlacement" :options="productImageLabelPlacementOptions" />
                            <color-control label="Label Color" v-model="item.labelColor" />
                            <sides-control v-if="item.imageShape === 'custom'" label="Image Radius" base="imageRadius" :control-id="'product-level-'+index+'-image-radius'" kind="corners" :fallback-values="['8px','8px','8px','8px']" :target="item" :node="node" :editor="editor" />
                            <select-control label="Image Border Style" v-model="item.imageBorderType" :options="productBorderTypeOptions" />
                            <sides-control v-if="item.imageBorderType !== 'none'" label="Image Border Width" base="imageBorderWidth" :control-id="'product-level-'+index+'-image-border-width'" :fallback-values="['1px','1px','1px','1px']" :target="item" :node="node" :editor="editor" />
                            <color-control v-if="item.imageBorderType !== 'none'" label="Image Border Color" v-model="item.imageBorderColor" />
                            <div class="pb-product-level-style__group-label">Card Border</div>
                            <select-control label="Card Border Style" v-model="item.borderType" :options="productBorderTypeOptions" />
                            <sides-control v-if="item.borderType !== 'none'" label="Card Border Width" base="borderWidth" :control-id="'product-level-'+index+'-border-width'" :fallback-values="['1px','1px','1px','1px']" :target="item" :node="node" :editor="editor" />
                            <sides-control label="Card Radius" base="borderRadius" :control-id="'product-level-'+index+'-border-radius'" kind="corners" :fallback-values="['8px','8px','8px','8px']" :target="item" :node="node" :editor="editor" />
                            <template v-if="productStyleState==='normal'">
                                <color-control label="Normal Border" v-model="item.normalBorderColor" />
                                <color-control label="Normal Background" v-model="item.normalBackground" />
                                <component :is="editor.textShadowControl" aria-label="Normal Shadow" :model-value="item.normalShadow" :control-id="'product-level-'+index+'-normal-shadow'" @update:modelValue="item.normalShadow = $event" />
                            </template>
                            <template v-else-if="productStyleState==='hover'">
                                <color-control label="Hover Border" v-model="item.hoverBorderColor" />
                                <color-control label="Hover Background" v-model="item.hoverBackground" />
                                <sides-control label="Hover Border Width" base="hoverBorderWidth" :control-id="'product-level-'+index+'-hover-border-width'" :fallback-values="['1px','1px','1px','1px']" :target="item" :node="node" :editor="editor" />
                                <component :is="editor.textShadowControl" aria-label="Hover Shadow" :model-value="item.hoverShadow" :control-id="'product-level-'+index+'-hover-shadow'" @update:modelValue="item.hoverShadow = $event" />
                            </template>
                            <template v-else>
                                <color-control label="Selected Border" v-model="item.selectedBorderColor" />
                                <color-control label="Selected Background" v-model="item.selectedBackground" />
                                <sides-control label="Selected Border Width" base="selectedBorderWidth" :control-id="'product-level-'+index+'-selected-border-width'" :fallback-values="['1px','1px','1px','1px']" :target="item" :node="node" :editor="editor" />
                                <component :is="editor.textShadowControl" aria-label="Selected Shadow" :model-value="item.selectedShadow" :control-id="'product-level-'+index+'-selected-shadow'" @update:modelValue="item.selectedShadow = $event" />
                                <color-control label="Selected Label" v-model="item.selectedLabelColor" />
                            </template>
                            <div class="pb-product-level-style__group-label">Selected Indicator</div>
                            <toggle-control label="Show Selected Check" v-model="item.selectedCheckVisible" />
                            <responsive-select label="Selected Check Position" base="selectedCheckPosition" :control-id="'product-level-'+index+'-check-position'" :target="item" :node="node" :editor="editor" :options="productCheckPositionOptions" />
                            <size-control label="Selected Check Size" base="selectedCheckSize" :control-id="'product-level-'+index+'-check-size'" fallback="20px" :target="item" :node="node" :editor="editor" :min="8" :max="80" :allowed-units="['px','em','rem']" />
                            <size-control label="Selected Check Icon Size" base="selectedCheckIconSize" :control-id="'product-level-'+index+'-check-icon-size'" fallback="10px" :target="item" :node="node" :editor="editor" :min="4" :max="40" :allowed-units="['px','em','rem']" />
                            <size-control label="Selected Check Offset" base="selectedCheckOffset" :control-id="'product-level-'+index+'-check-offset'" fallback="14px" :target="item" :node="node" :editor="editor" :max="100" :allowed-units="['px','em','rem']" />
                            <sides-control label="Selected Check Radius" base="selectedCheckRadius" :control-id="'product-level-'+index+'-check-radius'" kind="corners" :fallback-values="['50%','50%','50%','50%']" :target="item" :node="node" :editor="editor" />
                            <color-control label="Selected Check Color" v-model="item.selectedCheckColor" />
                            <color-control label="Selected Check Background" v-model="item.selectedCheckBackground" />
                        </div>
                    </details>
                </section-box>
                <section-box title="Form Layout" :open="true"
                    ><size-control
                        label="Columns Gap"
                        base="columnGap"
                        fallback="10px"
                        ...
                        :editor="editor"
                /></section-box>
                <section-box title="Form Labels"
                    ><color-control
                        label="Text Color"
                        v-model="s.labelColor" /><component
                ...
                /></section-box>
                <section-box title="HTML Field"
                    ><color-control label="Text Color" v-model="s.htmlColor"
                /></section-box>
                <section-box title="Form Inputs"
                    ><color-control
                        label="Text Color"
                        v-model="s.fieldTextColor" /><color-control
                        label="Background Color"
                        v-model="s.fieldBackground" /><color-control
                        label="Border Color"
                        v-model="s.fieldBorderColor" /><size-control
                        label="Border Width"
                        base="fieldBorderWidth"
                        fallback="1px"
                        :node="node"
                        :editor="editor" /><size-control
                        label="Border Radius"
                        ...
                        v-model="s.fieldFocusBorderColor" /><color-control
                        label="Focus Background Color"
                        v-model="s.fieldFocusBackground"
                /></section-box>
                <section-box title="Form Button"
                    ><div class="pb-state-tabs pb-state-tabs--pro">
                        <button
                            type="button"
                            :class="{ active: buttonStates.form === 'normal' }"
                            @click="buttonStates.form = 'normal'"
                        >
                            Normal</button
                        ><button
                            type="button"
                            :class="{ active: buttonStates.form === 'hover' }"
                            @click="buttonStates.form = 'hover'"
                        >
                            Hover
                        </button>
                    </div>
                    <component
                        ...
                        :font-families="editor.fontFamilies"
                        @responsive-device="
                            editor.setResponsiveDevice
                        " /><template v-if="buttonStates.form === 'normal'"
                        ><color-control
                            label="Text Color"
                            v-model="s.buttonTextColor" /><color-control
                            label="Background Color"
                            v-model="s.buttonBackground" /></template
                    ><template v-else
                        ><color-control
                            label="Text Color"
                            v-model="s.buttonTextColorHover" /><color-control
                            label="Background Color"
                            v-model="s.buttonBackgroundHover" /></template
                    ><select-control
                        label="Alignment"
                        v-model="s.buttonAlign"
                        :options="alignmentOptions" /><size-control
                        label="Border Radius"
                        base="buttonRadius"
                        fallback="4px"
                        :node="node"
                        :editor="editor"
                /></section-box>
                <section-box title="Feedback Colors"
                    ><color-control
                        label="Success Color"
                        v-model="s.successColor" /><color-control
                        label="Error Color"
                        v-model="s.errorColor"
                /></section-box>
                <section-box title="Step Indicator"
                    ><color-control
                        label="Active Color"
                        v-model="s.stepActiveColor" /><color-control
                        label="Inactive Color"
                        v-model="s.stepInactiveColor"
                /></section-box>
        ...
        <form-dataset-manager
            
            :open="formDatasetModalOpen"
            :field="formDatasetField"
            :fields="s.fields"
            :datasets="formDatasets"
            :endpoints="formDatasetEndpoints()"
            :notice="formDatasetNotice"
            :server-error="formDatasetError"
            :loading="formDatasetLoading"
            :product-mode="productDatasetEditing"
            :editor="editor"
            @close="closeFormDatasetModal"
            @save="saveFormDataset"
            @deleted="handleDeletedDataset"
            @clear-notice="clearFormDatasetNotice"
        />
    </div>
</template>
...
const ResponsiveSelect = {
    ...
    props: ["label", "base", "controlId", "node", "target", "editor", "options"],
    computed: {
        controlNode() {
            return this.target ? { settings: this.target } : this.node;
        },
        value() {
            const settings = this.controlNode?.settings || {};
            return settings[this.editor.activeResponsiveKey(this.base)] || settings[this.base] || this.options?.[0]?.value || "";
        },
    },
    template: `<div class="pb-form-group"><div class="pb-label-row pb-label-row-device"><label class="pb-form-label mb-0">{{label}}</label><responsive-menu :editor="editor" :id="controlId || 'pro-'+base"/></div><select class="pb-select" :value="value" @change="editor.setResponsiveSetting(controlNode.settings,base,$event.target.value)"><option v-for="option in options" :key="base+'-'+option.value" :value="option.value">{{option.label}}</option></select></div>`,
};
const SizeControl = {
    components: { ResponsiveMenu },
    props: {
        ...
        allowedUnits: { type: Array, default: () => [] },
    },
    computed: {
        controlNode() {
            return this.target ? { settings: this.target } : this.node;
        ...
        options() {
            const units = this.allowedUnits.length ? this.allowedUnits : this.editor.sizeControlUnits;
            return {
                fallback: this.fallback,
                min: this.min,
                max: this.max,
                allowedUnits: units,
                fallbackUnit: units[0] || "px",
            };
        },
        maxValue() {
            const unitMax = this.editor.sizeControlMax(this.controlNode, this.base, this.fallback, this.options);
            return this.max === null ? unitMax : Math.min(this.max, unitMax);
        },
    },
    template: `<div class="pb-form-group pb-pro-responsive-unit"><div class="pb-label-row pb-label-row-device"><label class="pb-form-label mb-0">{{label}}</label><responsive-menu :editor="editor" :id="controlId || 'pro-'+base"/></div><div class="pb-range-value-row"><input class="pb-range" type="range" :min="min" :max="maxValue" :step="editor.sizeControlStep(controlNode,base,fallback,options)" :value="editor.sizeControlDisplayValue(controlNode,base,fallback,options)" @input="editor.onSizeControlInput(controlNode,base,$event,options)"><div class="pb-value-with-unit"><input class="pb-input pb-input-compact" type="number" :min="min" :max="maxValue" :step="editor.sizeControlStep(controlNode,base,fallback,options)" :value="editor.sizeControlDisplayValue(controlNode,base,fallback,options)" @input="editor.onSizeControlInput(controlNode,base,$event,options)"><select class="pb-mini-unit" :value="editor.sizeControlUnit(controlNode,base,fallback,options)" @change="editor.setSizeControlUnit(controlNode,base,$event.target.value,options)"><option v-for="unit in options.allowedUnits" :key="base+'-'+unit" :value="unit">{{unit}}</option></select></div></div></div>`,
};
const SidesControl = {
    components: { ResponsiveMenu },
    props: {
        ...
        base: String,
        controlId: String,
        kind: { type: String, default: "edges" },
        fallbackValues: { type: Array, default: () => ["0px", "0px", "0px", "0px"] },
        target: Object,
        node: Object,
        editor: Object,
    },
    ...
    computed: {
        ...
        sides() {
            const labels = this.kind === "corners" ? ["Top Left", "Top Right", "Bottom Right", "Bottom Left"] : ["Top", "Right", "Bottom", "Left"];
            return ["Top", "Right", "Bottom", "Left"].map((key, index) => ({ key, label: labels[index] }));
        },
    },
    methods: {
        fallback(index) {
            return this.fallbackValues[index] || this.fallbackValues[0] || "0px";
        },
        unit() {
            return this.editor.sizeControlUnit(this.controlNode, this.base + this.sides[0].key, this.fallback(0), {
        ...
        setSide(index, event) {
            ...
            const targets = this.linked ? this.sides : [this.sides[index]];
            targets.forEach((side) => this.editor.setResponsiveSetting(this.controlNode.settings, this.base + side.key, `${safe}${this.unit()}`));
        },
        setUnit(event) {
            const next = this.units.includes(event.target.value) ? event.target.value : "px";
            this.sides.forEach((side, index) => this.editor.setResponsiveSetting(this.controlNode.settings, this.base + side.key, `${this.value(index)}${next}`));
        },
    },
    template: `<div class="pb-form-group pb-icon-box-sides-control"><div class="pb-label-row pb-label-row-device"><label class="pb-form-label mb-0">{{label}}</label><div class="pb-label-tools"><responsive-menu :editor="editor" :id="controlId || 'pro-'+base"/><select class="pb-mini-unit" :value="unit()" :aria-label="label+' unit'" @change="setUnit($event)"><option v-for="option in units" :key="base+'-'+option" :value="option">{{option}}</option></select></div></div><div class="pb-four-sides pb-four-sides-with-link"><label v-for="(side,index) in sides" :key="side.key" class="pb-side-input"><input class="pb-input" type="number" min="0" :value="value(index)" :aria-label="label+' '+side.label" @input="setSide(index,$event)"><span>{{side.label}}</span></label><div class="pb-side-link-cell"><button type="button" class="pb-link-btn" :class="{active:linked}" @click="linked=!linked" :title="linked?'Unlink values':'Link values together'"><i class="fas" :class="linked?'fa-link':'fa-unlink'"></i></button></div></div></div>`,
};
...
const FormDatasetManager = {
    ...
    props: {
        ...
        productMode: { type: Boolean, default: false },
        editor: { type: Object, default: () => ({}) },
    },
    ...
    methods: {
        ...
        productMeta(value = {}) {
            ...
            return {
                thumbnailSource: source.thumbnailSource === "url" ? "url" : "ckfinder",
                thumbnailUrl: String(source.thumbnailUrl || ""),
                thumbnailAlt: String(source.thumbnailAlt || ""),
        ...
        normalizeProductNodes() {
            if (!this.productMode || !Array.isArray(this.draftDataset?.nodes)) return;
            this.draftDataset.nodes.forEach((node) => { node.meta = this.productMeta(node.meta); });
        },
        resetDraft() {
            ...
            if (this.productMode) this.draftField.datasetMode = "dataset";
            this.normalizeProductNodes();
        ...
        addNode(parentId = null) {
            ...
            const node = {
                ...
                active: true,
                ...(this.productMode ? { meta: this.productMeta() } : {}),
            };
...
FormDatasetManager.template = FormDatasetManager.template
    ...
    .replace(
        '<span class="pb-form-dataset-modal__eyebrow">FORM FIELD DATASET</span>',
        `<span class="pb-form-dataset-modal__eyebrow">{{ productMode ? 'PRODUCT DATASET' : 'FORM FIELD DATASET' }}</span>`,
    )
    .replace(
        '<div class="pb-form-group"><label class="pb-form-label">Data source</label><select class="pb-select" v-model="draftField.datasetMode">',
        '<div v-if="!productMode" class="pb-form-group"><label class="pb-form-label">Data source</label><select class="pb-select" v-model="draftField.datasetMode">',
    )
    .replace(
        '<button type="button" :class="{active:mode===\'conditions\'}" @click="mode=\'conditions\'"><i class="fas fa-sitemap"></i> Conditional Logic</button>',
        '<button v-if="!productMode" type="button" :class="{active:mode===\'conditions\'}" @click="mode=\'conditions\'"><i class="fas fa-sitemap"></i> Conditional Logic</button>',
    )
    .replace(
        '<div class="pb-form-dataset-node-actions">',
        `<template v-if="productMode"><div class="pb-product-dataset-media"><select-control label="Thumbnail Source" v-model="selectedNode.meta.thumbnailSource" :options="[{value:'ckfinder',label:'CKFinder / Media Library'},{value:'url',label:'External URL'}]" /><div class="pb-form-group"><label class="pb-form-label">Thumbnail URL</label><div class="pb-input-with-action"><input class="pb-input" v-model="selectedNode.meta.thumbnailUrl" /><button v-if="selectedNode.meta.thumbnailSource==='ckfinder'" type="button" title="Choose thumbnail from CKFinder" @click="editor.chooseMedia(selectedNode.meta,'thumbnailUrl','Choose thumbnail')"><i class="fas fa-folder-open"></i></button></div></div><text-control label="Thumbnail Alt" v-model="selectedNode.meta.thumbnailAlt" /><select-control label="Main Image Source" v-model="selectedNode.meta.imageSource" :options="[{value:'ckfinder',label:'CKFinder / Media Library'},{value:'url',label:'External URL'}]" /><div class="pb-form-group"><label class="pb-form-label">Main Image URL</label><div class="pb-input-with-action"><input class="pb-input" v-model="selectedNode.meta.imageUrl" /><button v-if="selectedNode.meta.imageSource==='ckfinder'" type="button" title="Choose main image from CKFinder" @click="editor.chooseMedia(selectedNode.meta,'imageUrl','Choose main image')"><i class="fas fa-folder-open"></i></button></div></div><text-control label="Main Image Alt" v-model="selectedNode.meta.imageAlt" /><div class="pb-form-group"><label class="pb-form-label">Description</label><textarea class="pb-textarea" rows="3" v-model="selectedNode.meta.description"></textarea></div><text-control label="Detail URL" v-model="selectedNode.meta.detailUrl" /><text-control label="Detail Label" v-model="selectedNode.meta.detailLabel" /></div></template><div class="pb-form-dataset-node-actions">`,
    );
...
export default {
    ...
    data() {
        return {
            ...
            productDatasetEditing: false,
            productStyleState: "normal",
            productStyleLevelIndex: 0,
        };
    },
    created() {
    ...
    computed: {
        ...
        activeProductLevelSettings() {
            return (Array.isArray(this.s.productLevels) ? this.s.productLevels : []).slice(0, Math.max(1, Math.min(3, Number(this.s.productLevelCount) || 3)));
        },
        activeProductLevelStyles() {
            return (Array.isArray(this.s.productLevelStyles) ? this.s.productLevelStyles : []).slice(0, this.activeProductLevelSettings.length);
        },
        productStyleLevelIndexSafe() {
            const max = Math.max(0, this.activeProductLevelSettings.length - 1);
            return Math.min(max, Math.max(0, Number(this.productStyleLevelIndex) || 0));
        },
        productStyleLevelOptions() {
            return this.activeProductLevelSettings.map((level, index) => option(String(index), level.label || level.key || "Product"));
        },
        productDataset() {
            return this.formDatasets.find((entry) => String(entry.id) === String(this.s.productData?.datasetId || "")) || null;
        },
        productDatasetLabel() {
            return this.productDataset ? `${this.productDataset.name} · ${this.productDataset.nodes?.length || 0} nodes` : "No Product Dataset selected";
        },
        productLevelCountOptions() {
            return [1, 2, 3].map((value) => ({ value, label: `${value} level${value === 1 ? "" : "s"}` }));
        },
        productPresentationOptions() {
            return [option("cards", "Cards"), option("select", "Select")];
        },
        productDesktopPositionOptions() {
            return [option("left", "Left"), option("right", "Right")];
        },
        productTabletPositionOptions() {
            return [option("", "Inherit Desktop"), ...this.productDesktopPositionOptions];
        },
        ...
        productImageShapeOptions() {
            return [option("circle", "Circle"), option("rounded", "Rounded"), option("custom", "Custom")];
        },
        productImageLabelPlacementOptions() {
            return [option("above", "Above image"), option("below", "Below image"), option("inside", "Inside image")];
        },
        productBorderTypeOptions() {
            return ["none", "solid", "double", "dotted", "dashed"].map((value) => option(value, value[0].toUpperCase() + value.slice(1)));
        },
        productCardAlignOptions() {
            return ["left", "center", "right"].map((value) => option(value, value[0].toUpperCase() + value.slice(1)));
        },
        productCardHeightModeOptions() {
            return [option("auto", "Auto"), option("fixed", "Fixed")];
        },
        productCheckPositionOptions() {
            return [
    ...
    methods: {
        setProductStyleLevel(value) {
            this.productStyleLevelIndex = Math.min(Math.max(0, Number(value) || 0), Math.max(0, this.activeProductLevelSettings.length - 1));
        },
        setProductStyleState(state) {
            const next = ["normal", "hover", "selected"].includes(state) ? state : "normal";
            this.productStyleState = next;
            window.dispatchEvent(new CustomEvent("pagebuilder:v24-product-card-state-preview", {
                detail: { nodeId: this.node.id, state: next },
            }));
        },
        ensureFormRowGrid() {
            if (this.type !== "product_lead_form") return;
        ...
        async loadFormDatasets(force = false) {
            ...
            try {
                ...
                window.dispatchEvent(new CustomEvent("pagebuilder:v24-form-datasets-loaded"));
        ...
        async openFormDatasetModal(field) {
            ...
            this.productDatasetEditing = false;
            this.formDatasetField = field;
            this.formDatasetNotice = "";
            this.formDatasetError = "";
            this.formDatasetModalOpen = true;
        },
        async openProductDatasetModal() {
            await this.loadFormDatasets();
            this.productDatasetEditing = true;
            this.formDatasetField = {
                id: "product_catalog",
                label: "Product Catalog",
                datasetMode: "dataset",
                datasetId: String(this.s.productData?.datasetId || ""),
                datasetParentFieldId: "",
                conditionalLogic: { enabled: false, relation: "all", rules: [] },
            };
            this.formDatasetNotice = "";
        ...
        async saveFormDataset(payload) {
            ...
            if (this.productDatasetEditing) payload.field.datasetMode = "dataset";
            ...
            try {
                ...
                if (this.productDatasetEditing) {
                    this.s.productData ||= { datasetMode: "dataset", datasetId: "" };
                    this.s.productData.datasetMode = "dataset";
                    this.s.productData.datasetId = String(saved.id || dataset.id || "");
                }
        ...
        async handleDeletedDataset({ datasetId } = {}) {
            ...
            if (String(this.s.productData?.datasetId || "") === id) {
                this.s.productData.datasetId = "";
            }
            this.formDatasetNotice = "Dataset deleted successfully.";
        },
        productNodeDepth(node) {
            const nodes = Array.isArray(this.productDataset?.nodes) ? this.productDataset.nodes : [];
        ...
        productDefaultOptions(levelIndex) {
            const nodes = Array.isArray(this.productDataset?.nodes) ? this.productDataset.nodes : [];
            return [
                option("", "Automatic first active item"),
                ...nodes
                    .filter((node) => node && node.active !== false && this.productNodeDepth(node) === levelIndex)
                    .map((node) => option(String(node.id), String(node.label || node.name || node.value || node.id))),
            ];
        },
...
.pb-product-dataset-summary { display:grid;gap:10px; }
.pb-product-dataset-summary>span { display:flex;align-items:center;gap:8px;color:#344054; }
.pb-product-dataset-summary>span i { color:#6979f8; }
.pb-product-level-settings { display:grid;gap:8px;margin-top:12px;padding:12px;border:1px solid #e4e7ec;border-radius:8px;background:#f8fafc; }
.pb-product-level-settings>strong { color:#344054;font-size:11px; }
.pb-product-level-style { margin-top:10px;border:1px solid #e4e7ec;border-radius:8px;background:#fff; }
.pb-product-level-style>summary { padding:10px 12px;color:#344054;font-size:11px;font-weight:700;cursor:pointer; }
.pb-product-level-style__body { display:grid;gap:8px;padding:0 12px 12px; }
.pb-product-level-style__group-label { margin-top:6px;padding-top:8px;border-top:1px solid #f2f4f7;color:#667085;font-size:10px;font-weight:800;letter-spacing:.05em;text-transform:uppercase; }
.pb-product-dataset-media { display:grid;grid-column:1/-1;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;padding:12px;border:1px solid #e4e7ec;border-radius:8px;background:#f8fafc; }
.pb-product-dataset-media>.pb-form-group:has(.pb-textarea),.pb-product-dataset-media>.pb-form-group:nth-last-child(-n+3) { grid-column:1/-1; }
.pb-product-dataset-media .pb-input-with-action { display:grid;grid-template-columns:minmax(0,1fr) 36px; }
.pb-product-dataset-media .pb-input-with-action button { border:1px solid #d0d5dd;border-left:0;border-radius:0 6px 6px 0;background:#fff;color:#667085; }
</style>

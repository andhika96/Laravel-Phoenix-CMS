	...
	$__pbLayoutAdvanced = app(\App\Support\PageBuilderElementorV24\WidgetAdvancedStyleResolver::class)->resolve($s, (string) ($node['id'] ?? ''));
		...
		foreach ($containerColumns as $idx => $col) {
			...
			$normalizedColumns[] = [
				...
				'styleOverrides' => is_array($c['styleOverrides'] ?? null) ? $c['styleOverrides'] : [],
		...
		$gridColumnDesktopStyles = [];
		$gridColumnMediaRules = ['tablet' => [], 'mobile' => []];
		if ($display === 'grid') {
			$gridColumnCount = max(1, min(12, (int) ($s['gridColumns'] ?? 3)));
			$gridColumnStyles = is_array($node['columnStyles'] ?? null) ? $node['columnStyles'] : [];
			foreach ($normalizedColumns as $cellIndex => $col) {
				$trackIndex = ((int) $cellIndex) % $gridColumnCount;
				$trackStyle = $gridColumnStyles[$trackIndex] ?? [];
				$cellOverrides = is_array($col['styleOverrides'] ?? null) ? $col['styleOverrides'] : [];
				$gridColumnDesktopStyles[$cellIndex] = $grid_column_style_css($grid_column_style_resolve($trackStyle, $cellOverrides));
				...
				$selector = '#' . $nodeDomId . ' > .el-cont-columns > .el-grid-col[data-pb-grid-cell="' . $cellId . '"]';
				$gridColumnMediaRules['tablet'][] = $selector . '{' . $grid_column_style_css($grid_column_style_resolve($trackStyle, $cellOverrides, 'Tablet')) . '}';
				$gridColumnMediaRules['mobile'][] = $selector . '{' . $grid_column_style_css($grid_column_style_resolve($trackStyle, $cellOverrides, 'Mobile')) . '}';
			}
		}
		$styleBlocks = [];
		...
		if ($gridColumnMediaRules['tablet']) $styleBlocks[] = '@media (max-width: 1024px){' . implode('', $gridColumnMediaRules['tablet']) . '}';
		if ($gridColumnMediaRules['mobile']) $styleBlocks[] = '@media (max-width: 767px){' . implode('', $gridColumnMediaRules['mobile']) . '}';
	...
	>
		...
		<div class="el-cont-columns" style="{{ $contColumnsStyle }}">
			@if($hasCanonicalChildren)
				...
				@endforeach
			@else
			@foreach($normalizedColumns as $colIndex => $col)
				@php
					$colStyles = [];
					$colChildren = is_array($col['children'] ?? null) ? $col['children'] : [];
					$childCount = count($colChildren);
					$childWrapperStyles = [];
					$gridCellId = preg_replace('/[^A-Za-z0-9_-]/', '', (string) ($col['id'] ?? '')) ?: ('cell-' . $colIndex);
					if ($display === 'flex') {
						...
						$colStyles[] = $gridColumnDesktopStyles[$colIndex] ?? '';
					}
					$colStyle = implode(';', array_filter($colStyles));
				@endphp
				<div class="el-grid-col" @if($display === 'grid') data-pb-grid-track="{{ $colIndex % max(1, $gridColumnCount ?? 1) }}" data-pb-grid-cell="{{ $gridCellId }}" @endif style="{{ $colStyle }}">

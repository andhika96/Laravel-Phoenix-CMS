<template>
	<div class="pb-widget-settings pb-widget-settings--basic pb-widget-settings--text-editor">
		...
		<div v-show="editor.settingsTab==='style'" class="pb-tab-content pb-basic-text-style-settings">
			<details class="pb-collapsible" open>
				<summary>Text Editor</summary>
				<div class="pb-collapsible-body">
...
const ResponsiveDimensionControl = {
	...
	template: `<div class="pb-form-group pb-basic-dimension-control"><div class="pb-label-row pb-label-row-device"><label class="pb-form-label mb-0">{{ label }}</label><div class="pb-label-tools"><responsive-menu :editor="editor" :id="controlId" /><select class="pb-mini-unit" :value="parsed.unit" :aria-label="label + ' unit'" @change="setUnit($event.target.value)"><option v-for="option in units" :key="option" :value="option">{{ option }}</option></select></div></div><div class="pb-range-value-row"><input class="pb-range" type="range" min="0" :max="max" :step="parsed.unit === 'px' ? 1 : .1" :value="parsed.value" @input="setValue($event.target.value)"><input class="pb-input pb-input-compact" type="number" min="0" :max="max" :step="parsed.unit === 'px' ? 1 : .1" :value="parsed.value" @input="setValue($event.target.value)"></div></div>`,
};

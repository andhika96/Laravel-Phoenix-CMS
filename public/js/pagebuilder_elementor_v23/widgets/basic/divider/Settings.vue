		...
		<div v-if="editor.settingsTab === 'style'" class="pb-tab-content pb-basic-divider-style-settings">
			<details class="pb-collapsible" open>
				...
				<div class="pb-collapsible-body">
					...
					<div class="pb-form-group"><div class="pb-label-row pb-label-row-device"><label class="pb-form-label mb-0">Thickness</label><div class="pb-control-device-wrap"><button class="pb-control-device-btn" @click.stop="editor.openControlResponsiveMenu('divider-thickness')" :title="'Responsive: ' + editor.responsiveDeviceLabel()"><i :class="editor.responsiveDeviceIcon()"></i></button><div v-if="editor.isControlResponsiveMenuOpen('divider-thickness')" class="pb-control-device-menu"><button v-for="device in editor.responsiveDevices" :key="'divider-thickness-' + device.value" class="pb-control-device-item" :class="{active: editor.responsiveDevice === device.value}" @click.stop="editor.applyResponsiveDevice('divider-thickness', device.value)"><i :class="device.icon"></i><span>{{ editor.deviceOptionLabel(device) }}</span></button></div></div></div><div class="pb-range-value-row"><input type="range" class="pb-range" min="0" :max="thicknessMax()" :step="thicknessStep()" :value="thicknessValue()" @input="setThicknessValue($event)"><div class="pb-value-with-unit"><input class="pb-input pb-input-compact" type="number" min="0" :max="thicknessMax()" :step="thicknessStep()" :value="thicknessValue()" @input="setThicknessValue($event)"><select class="pb-mini-unit" :value="thicknessUnit()" aria-label="Thickness unit" @change="setThicknessUnit($event)"><option v-for="unit in thicknessUnits" :key="'divider-thickness-unit-' + unit" :value="unit">{{ unit }}</option></select></div></div></div>
					<div class="pb-form-group"><label class="pb-form-label">Color</label><div class="pb-color-row"><input class="pb-input coloris pb-coloris-input" v-model="node.settings.color" placeholder="#d0d7e6"></div></div>
				</div>
			</details>
		</div>
...
<script>
export default {
	name: 'BasicDividerSettings',
	props: { node: { type: Object, required: true }, editor: { type: Object, required: true } },
	data() { return { thicknessUnits: ['px', 'em', 'rem'] }; },
	methods: {
		thicknessOptions() { return { fallback: '2px', fallbackUnit: 'px', allowedUnits: this.thicknessUnits }; },
		thicknessUnit() { return this.editor.sizeControlUnit(this.node, 'thickness', '2px', this.thicknessOptions()); },
		thicknessValue() { return this.editor.sizeControlDisplayValue(this.node, 'thickness', '2px', this.thicknessOptions()); },
		thicknessMax() { return ['em', 'rem'].includes(this.thicknessUnit()) ? 30 : 20; },
		thicknessStep() { return ['em', 'rem'].includes(this.thicknessUnit()) ? 0.1 : 1; },
		setThicknessValue(event) { this.editor.onSizeControlInput(this.node, 'thickness', event, this.thicknessOptions()); },
		setThicknessUnit(event) { this.editor.setSizeControlUnit(this.node, 'thickness', event.target.value, this.thicknessOptions()); },
	},
};
</script>

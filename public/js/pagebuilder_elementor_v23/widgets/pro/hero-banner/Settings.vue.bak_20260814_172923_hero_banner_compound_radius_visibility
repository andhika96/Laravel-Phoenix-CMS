        ...
        <div v-show="editor.settingsTab==='content'" class="pb-tab-content">
                    ...
                    <div class="pb-hero-buttons">
                        <div v-for="(button,index) in settings.buttons" :key="button.id" class="pb-hero-button-item" :class="{'is-open':expandedButtonId===button.id}">
                            <div class="pb-hero-button-item__header" role="button" tabindex="0" :aria-expanded="expandedButtonId===button.id?'true':'false'" @click="toggleButton(button.id)" @keydown.enter.prevent="toggleButton(button.id)" @keydown.space.prevent="toggleButton(button.id)">
                                <i class="fas fa-grip-vertical"></i><strong>{{button.text||'Button '+(index+1)}}</strong>
                            ...
                            <div v-if="expandedButtonId===button.id" class="pb-hero-button-item__body">
                                ...
                                <template v-if="button.actionType==='video_popup'">
                                    ...
                                    <div class="pb-form-group"><label class="pb-form-label">Video URL</label><div class="pb-media-field"><input class="pb-input" v-model="button.videoUrl"><button v-if="button.videoSource==='self_hosted'" type="button" @click="editor.chooseMedia(button,'videoUrl','Paste video URL')"><i class="fas fa-folder-open"></i></button></div></div>
                                </template>
                                <template v-if="button.actionType==='image_popup'">
                                    ...
                                    <div class="pb-form-group"><label class="pb-form-label">Image URL</label><div class="pb-media-field"><input class="pb-input" v-model="button.imageUrl"><button v-if="button.imageSource!=='url'" type="button" @click="editor.chooseMedia(button,'imageUrl')"><i class="fas fa-folder-open"></i></button></div></div>
            ...
            <details class="pb-collapsible" open>
                ...
                <div class="pb-collapsible-body">
                    ...
                    <div class="pb-form-note">Editing: <strong>{{contentLabel(positionTarget)}}</strong></div>
                    <div class="pb-form-group"><label class="pb-form-label">Anchor Point</label><div class="pb-hero-anchor"><button v-for="anchor in anchors" :key="anchor" type="button" :class="{active:responsiveValue(positionTarget+'Anchor','center-left')===anchor}" :title="anchor" @click="setResponsive(positionTarget+'Anchor',anchor)"><span></span></button></div></div>
                    <div class="pb-hero-grid"><NumberField label="Horizontal (X)" :value="responsiveValue(positionTarget+'X',0)" unit="%" @change="setResponsive(positionTarget+'X',$event)"/><NumberField label="Vertical (Y)" :value="responsiveValue(positionTarget+'Y',0)" unit="%" @change="setResponsive(positionTarget+'Y',$event)"/></div>
                    <NumberField label="Content Width" :value="responsiveValue(positionTarget+'Width',40)" unit="%" :min="10" @change="setResponsive(positionTarget+'Width',$event)"/>
                    <div class="pb-form-group"><label class="pb-form-label">Alignment</label><div class="pb-btn-group"><button v-for="align in ['left','center','right']" :key="align" type="button" class="pb-seg-btn" :class="{active:responsiveValue(positionTarget+'Align','left')===align}" @click="setResponsive(positionTarget+'Align',align)">{{align}}</button></div></div>
                </div>
            </details>
            ...
            <details class="pb-collapsible" open>
                ...
                <div class="pb-collapsible-body">
                    <DeviceTabs :editor="editor" /><InheritanceBadge :label="inheritanceLabel('buttonDirection')" />
                    <div class="pb-form-group"><label class="pb-form-label">Direction</label><div class="pb-btn-group"><button v-for="value in ['row','column']" :key="value" type="button" class="pb-seg-btn" :class="{active:responsiveValue('buttonDirection','row')===value}" @click="setResponsive('buttonDirection',value)">{{value}}</button></div></div>
                    <div class="pb-form-group"><label class="pb-form-label">Alignment</label><div class="pb-btn-group"><button v-for="value in ['left','center','right']" :key="value" type="button" class="pb-seg-btn" :class="{active:responsiveValue('buttonAlign','left')===value}" @click="setResponsive('buttonAlign',value)">{{value}}</button></div></div>
                    <div class="pb-form-group"><label class="pb-form-label">Gap</label><input class="pb-input" :value="responsiveValue('buttonGap','10px')" @input="setResponsive('buttonGap',$event.target.value)"></div>
                    <ToggleField :id="'hero-wrap-buttons-'+node.id+'-'+currentDevice" label="Wrap Buttons" :model-value="Boolean(responsiveValue('buttonWrap',true))" @update:model-value="setResponsive('buttonWrap',$event)" />
                    <button v-if="currentDevice!=='desktop'" type="button" class="pb-reset-btn" @click="resetOverride(['buttonDirection','buttonAlign','buttonGap','buttonWrap'])"><i class="fas fa-undo"></i> Reset Override</button>
                </div>
            </details>
            ...
            <details class="pb-collapsible" open>
                ...
                <div class="pb-collapsible-body">
                    ...
                    <div class="pb-form-group"><label class="pb-form-label">Image URL</label><div class="pb-media-field"><input class="pb-input" :value="responsiveValue('imageUrl','')" @input="setResponsive('imageUrl',$event.target.value)"><button v-if="responsiveValue('imageSource','ckfinder')!=='url'" type="button" @click="chooseHeroMedia"><i class="fas fa-folder-open"></i></button></div><div v-if="responsiveValue('imageSource','ckfinder')==='url'" class="pb-form-note">Remote hosts may block hotlinking.</div></div>
                    ...
                    <div class="pb-hero-grid"><div class="pb-form-group"><label class="pb-form-label">Object Fit</label><select class="pb-select" :value="responsiveValue('objectFit','cover')" @change="setResponsive('objectFit',$event.target.value)"><option value="cover">Cover</option><option value="contain">Contain</option><option value="fill">Fill</option></select></div><div class="pb-form-group"><label class="pb-form-label">Object Position</label><select class="pb-select" :value="responsiveValue('objectPosition','center center')" @change="setResponsive('objectPosition',$event.target.value)"><option v-for="value in objectPositions" :key="value" :value="value">{{value}}</option></select></div></div>
                    <button v-if="currentDevice!=='desktop'" type="button" class="pb-reset-btn" @click="resetOverride(['imageSource','imageUrl','imageAlt','objectFit','objectPosition'])"><i class="fas fa-undo"></i> Reset Override</button>
                </div>
            </details>
        </div>
...
const NumberField={props:{label:String,value:[String,Number],unit:String,min:{type:Number,default:0}},emits:['change'],template:`<div class="pb-form-group"><label class="pb-form-label">{{label}}</label><div class="pb-hero-number"><input class="pb-input" type="number" :min="min" max="100" :value="value" @input="$emit('change',Number($event.target.value))"><span>{{unit}}</span></div></div>`};
const ColorField={props:['label','modelValue'],emits:['update:modelValue'],template:`<div class="pb-form-group"><label class="pb-form-label">{{label}}</label><div class="pb-hero-color"><input type="color" :value="/^#[0-9a-f]{6}$/i.test(modelValue||'')?modelValue:'#000000'" @input="$emit('update:modelValue',$event.target.value)"><input class="pb-input" :value="modelValue" @input="$emit('update:modelValue',$event.target.value)"></div></div>`};
...
export default{
    name:'HeroBannerSettings',components:{DeviceTabs,InheritanceBadge,NumberField,ColorField,ToggleField},props:{node:{type:Object,required:true},editor:{type:Object,required:true}},
    data(){return{expandedButtonId:'',selectedTarget:'title',anchors:['top-left','top-center','top-right','center-left','center','center-right','bottom-left','bottom-center','bottom-right'],objectPositions:['left top','left center','left bottom','center top','center center','center bottom','right top','right center','right bottom']};},
    computed:{settings(){return this.node.settings;},currentDevice(){return ['tablet','mobile'].includes(this.editor.responsiveDevice)?this.editor.responsiveDevice:'desktop';},positionTarget(){return this.settings.positioningMode==='grouped'?'group':this.selectedTarget;}},
...
<style scoped>
.pb-hero-mode,.pb-hero-targets{display:grid;grid-template-columns:repeat(2,minmax(0,1fr))}.pb-hero-targets{grid-template-columns:repeat(3,minmax(0,1fr));margin-bottom:12px}.pb-hero-mode button,.pb-hero-targets button{text-transform:capitalize}.pb-hero-mode .pb-seg-btn{gap:5px}.pb-hero-order,.pb-hero-buttons{display:grid;gap:7px;margin-top:12px}.pb-hero-order__row,.pb-hero-button-item__header{display:grid;grid-template-columns:18px 1fr 28px 28px;align-items:center;gap:4px;min-height:38px;padding:5px 6px;border:1px solid #e4e7ec;border-radius:6px;background:#fff}.pb-hero-order__row strong,.pb-hero-button-item__header strong{min-width:0;overflow:hidden;color:#475467;font-size:11px;text-overflow:ellipsis;white-space:nowrap}.pb-hero-order__row button,.pb-hero-button-item__header button{width:27px;height:27px;border:0;border-radius:4px;color:#667085;background:#f2f4f7}.pb-hero-order__row button:disabled,.pb-hero-button-item__header button:disabled{cursor:not-allowed;opacity:.35}.pb-hero-button-item.is-open{border:1px solid #b8c0ff;border-radius:7px}.pb-hero-button-item__header{cursor:pointer;border:0}.pb-hero-button-item__body{padding:10px;border-top:1px solid #e4e7ec}.pb-hero-add{width:100%;margin-top:9px;padding:9px;border:1px dashed #aeb7c6;border-radius:6px;color:#5b6cff;background:#fafbff;font-size:11px}.pb-hero-add:disabled{cursor:not-allowed;opacity:.45}.pb-hero-devices{display:grid;grid-template-columns:repeat(3,1fr);gap:5px;margin-bottom:10px}:deep(.pb-hero-devices button){min-height:32px;border:1px solid #e0e4ec;border-radius:5px;color:#667085;background:#fff;font-size:9px}:deep(.pb-hero-devices button.active){border-color:#aeb7ff;color:#4f60da;background:#f1f2ff}:deep(.pb-hero-devices span){margin-left:4px}.pb-hero-inheritance{width:max-content;margin:0 0 10px auto;padding:4px 7px;border-radius:999px;color:#26795e;background:#e8f7f0;font-size:8px}.pb-hero-anchor{width:108px;display:grid;grid-template-columns:repeat(3,36px);margin:auto;overflow:hidden;border:1px solid #d9dfeb;border-radius:7px}.pb-hero-anchor button{height:32px;border:0;border-right:1px solid #e5e9f1;border-bottom:1px solid #e5e9f1;background:#fbfcfe}.pb-hero-anchor button:nth-child(3n){border-right:0}.pb-hero-anchor button:nth-last-child(-n+3){border-bottom:0}.pb-hero-anchor span{width:5px;height:5px;display:block;margin:auto;border-radius:50%;background:#a8b1c1}.pb-hero-anchor button.active{background:#eef0ff}.pb-hero-anchor button.active span{width:8px;height:8px;background:#5b6cff}.pb-hero-grid{display:grid;grid-template-columns:1fr 1fr;gap:9px}:deep(.pb-hero-number){display:grid;grid-template-columns:1fr 34px}:deep(.pb-hero-number input){border-radius:6px 0 0 6px}:deep(.pb-hero-number span){display:grid;place-items:center;border:1px solid #d0d5dd;border-left:0;border-radius:0 6px 6px 0;color:#667085;font-size:10px;background:#f9fafb}.pb-media-field,:deep(.pb-hero-color){display:flex}.pb-media-field input,:deep(.pb-hero-color .pb-input){border-radius:6px 0 0 6px}.pb-media-field button,:deep(.pb-hero-color input[type=color]){width:38px;border:1px solid #d0d5dd;border-left:0;border-radius:0 6px 6px 0;background:#f9fafb}:deep(.pb-hero-color input[type=color]){height:36px;padding:4px;order:2}.pb-reset-btn{width:100%;padding:8px;border:1px solid #d0d5dd;border-radius:6px;color:#667085;background:#fff;font-size:10px}
</style>

                                ...
                                <details class="pb-hero-slider-subsection" open>
                                    ...
                                    <div class="pb-hero-slider-subsection__body">
                                        ...
                                        <template v-if="slide.mediaType==='image'">
                                            ...
                                            <responsive-select label="Object Fit" :control-id="'hero-slider-object-fit-'+slide.id" :editor="editor" :model-value="slideResponsiveValue(slide,'objectFit','cover')" :options="objectFitOptions" @update:model-value="setSlideResponsive(slide,'objectFit',$event)" />
                                            <responsive-select label="Object Position" :control-id="'hero-slider-object-position-'+slide.id" :editor="editor" :model-value="slideResponsiveValue(slide,'objectPosition','center center')" :options="objectPositionOptions" @update:model-value="setSlideResponsive(slide,'objectPosition',$event)" />
                                        </template>
                                        <template v-else>
                                ...
                                <details class="pb-hero-slider-subsection">
                                    ...
                                    <div class="pb-hero-slider-subsection__body">
                                        ...
                                        <template v-if="slide.styleOverride">
                                            ...
                                            <size-control label="Title Size" base="slideTitleFontSize" :control-id="'hero-slider-slide-title-size-'+slide.id" fallback="52px" :target="slide" :node="node" :editor="editor" :min="8" :max="160" />
        ...
        <div v-show="editor.settingsTab==='style'" class="pb-tab-content">
            ...
            <details class="pb-collapsible" open>
                ...
                <div class="pb-collapsible-body">
                    ...
                    <size-control label="Title Size" base="titleFontSize" control-id="hero-slider-title-size" fallback="52px" :node="node" :editor="editor" :min="8" :max="160" />
...
const SizeControl = { components: { ResponsiveMenu }, props: { label: String, base: String, controlId: String, fallback: String, node: Object, target: Object, editor: Object, min: { type: Number, default: 0 }, max: { type: Number, default: null }, allowedUnits: { type: Array, default: () => [] } }, computed: { controlNode() { return this.target ? { settings: this.target } : this.node; }, options() { const units = this.allowedUnits.length ? this.allowedUnits : this.editor.sizeControlUnits; return { fallback: this.fallback, min: this.min, max: this.max, allowedUnits: units, fallbackUnit: units[0] || 'px' }; }, maxValue() { const unitMax = this.editor.sizeControlMax(this.controlNode, this.base, this.fallback, this.options); return this.max === null ? unitMax : Math.min(this.max, unitMax); } }, template: `<div class="pb-form-group pb-hero-slider-size-control"><div class="pb-label-row pb-label-row-device"><label class="pb-form-label mb-0">{{label}}</label><responsive-menu :editor="editor" :id="controlId"/></div><div class="pb-range-value-row"><input class="pb-range" type="range" :min="min" :max="maxValue" :step="editor.sizeControlStep(controlNode,base,fallback,options)" :value="editor.sizeControlDisplayValue(controlNode,base,fallback,options)" @input="editor.onSizeControlInput(controlNode,base,$event,options)"><div class="pb-value-with-unit"><input class="pb-input pb-input-compact" type="number" :min="min" :max="maxValue" :value="editor.sizeControlDisplayValue(controlNode,base,fallback,options)" @input="editor.onSizeControlInput(controlNode,base,$event,options)"><select class="pb-mini-unit" :value="editor.sizeControlUnit(controlNode,base,fallback,options)" @change="editor.setSizeControlUnit(controlNode,base,$event.target.value,options)"><option v-for="unit in options.allowedUnits" :key="unit" :value="unit">{{unit}}</option></select></div></div></div>` };
const OffsetControl = {

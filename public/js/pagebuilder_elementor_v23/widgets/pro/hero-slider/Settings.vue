        ...
        <div v-show="editor.settingsTab==='content'" class="pb-tab-content">
                    ...
                    <div class="pb-hero-slider-slides">
                        <div v-for="(slide,index) in slides" :key="slide.id" class="pb-hero-slider-slide" :class="{'is-open':expandedSlideId===slide.id}">
                            <div class="pb-hero-slider-slide__header" role="button" tabindex="0" :aria-expanded="expandedSlideId===slide.id?'true':'false'" @click="toggleSlide(slide.id)" @keydown.enter.prevent="toggleSlide(slide.id)" @keydown.space.prevent="toggleSlide(slide.id)">
                                <i class="fas fa-grip-vertical"></i><strong>{{ slide.title || (slide.mediaType==='video' ? 'Video' : 'Image') }} {{ index+1 }}</strong>
            ...
            <details class="pb-collapsible" open>
                ...
                <div class="pb-collapsible-body">
                    ...
                    <div class="pb-form-group"><label class="pb-form-label">Direction</label><div class="pb-btn-group"><button v-for="direction in ['horizontal','vertical']" :key="direction" type="button" class="pb-seg-btn" :class="{active:responsiveValue('direction','horizontal')===direction}" @click="setResponsive('direction',direction)"><i :class="direction==='vertical'?'fas fa-arrows-alt-v':'fas fa-arrows-alt-h'"></i> {{direction==='vertical'?'Vertical':'Horizontal'}}</button></div></div>
                    ...
                    <div class="pb-subsection-title pb-subsection-title--spaced">Pagination Position</div>
                    <div class="pb-hero-slider-grid pb-hero-slider-pagination-settings">
                        <div class="pb-form-group"><label class="pb-form-label">Horizontal Slider Position</label><select class="pb-select" :value="responsiveValue('paginationPositionHorizontal','bottom-center')" @change="setResponsive('paginationPositionHorizontal',$event.target.value)"><option v-for="position in paginationPositions" :key="'horizontal-'+position.value" :value="position.value">{{position.label}}</option></select></div>
                        <div class="pb-form-group"><label class="pb-form-label">Vertical Slider Position</label><select class="pb-select" :value="responsiveValue('paginationPositionVertical','center-right')" @change="setResponsive('paginationPositionVertical',$event.target.value)"><option v-for="position in paginationPositions" :key="'vertical-'+position.value" :value="position.value">{{position.label}}</option></select></div>
                    </div>
                    <div class="pb-form-note">Dots stay centered below horizontal slides by default and form a centered side rail for vertical slides. Choose a position independently for each direction.</div>
                    <div v-if="currentDevice!=='desktop'" class="pb-form-note">Direction overrides inherit Desktop when left empty.</div>
                </div>
            </details>
        ...
        <div v-show="editor.settingsTab==='style'" class="pb-tab-content">
            ...
            <details class="pb-collapsible" open>
                ...
                <div class="pb-collapsible-body">
                    ...
                    <div class="pb-hero-slider-grid"><size-control label="Button Radius" base="buttonRadius" control-id="hero-slider-button-radius" fallback="999px" :node="node" :editor="editor" :min="0" :max="999" :allowed-units="['px','%','em','rem']" /><size-control label="Horizontal Padding" base="buttonPaddingX" control-id="hero-slider-button-padding-x" fallback="18px" :node="node" :editor="editor" :min="0" :max="100" :allowed-units="['px','%','em','rem']" /><size-control label="Vertical Padding" base="buttonPaddingY" control-id="hero-slider-button-padding-y" fallback="10px" :node="node" :editor="editor" :min="0" :max="100" :allowed-units="['px','%','em','rem']" /></div>
                </div>
            </details>
        </div>
...
export default {
    ...
    components: { DeviceTabs, ToggleField, ColorField, SizeControl, ScalarControl, MediaField },
    props: { node: { type: Object, required: true }, editor: { type: Object, required: true } },
    data() { return { expandedSlideId: '', ratios: ['16/9', '4/3', '1/1', '3/2', '21/9', '9/16', '4/5'], paginationPositions: [{ value: 'top-left', label: 'Top Left' }, { value: 'top-center', label: 'Top Center' }, { value: 'top-right', label: 'Top Right' }, { value: 'center-left', label: 'Center Left' }, { value: 'center', label: 'Center' }, { value: 'center-right', label: 'Center Right' }, { value: 'bottom-left', label: 'Bottom Left' }, { value: 'bottom-center', label: 'Bottom Center' }, { value: 'bottom-right', label: 'Bottom Right' }] }; },
    computed: {
...
<style scoped>
.pb-hero-slider-settings .pb-hero-slider-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.pb-hero-slider-settings .pb-hero-slider-slides{display:grid;gap:8px}.pb-hero-slider-settings .pb-hero-slider-slide{border:1px solid var(--line);border-radius:8px;background:#fff;overflow:hidden}.pb-hero-slider-settings .pb-hero-slider-slide__header{display:flex;align-items:center;gap:8px;min-height:42px;padding:8px 10px;cursor:pointer}.pb-hero-slider-settings .pb-hero-slider-slide__header>strong{flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.pb-hero-slider-settings .pb-hero-slider-slide__header button{width:26px;height:26px;padding:0;border:0;background:transparent;color:var(--muted);cursor:pointer}.pb-hero-slider-settings .pb-hero-slider-slide__header button:disabled{opacity:.35;cursor:not-allowed}.pb-hero-slider-settings .pb-hero-slider-slide__body{padding:10px;border-top:1px solid var(--line);background:var(--soft)}.pb-hero-slider-settings .pb-hero-add{width:100%;margin-top:10px;padding:9px;border:1px dashed var(--line);border-radius:7px;background:transparent;color:var(--primary,#6979f8);cursor:pointer}.pb-hero-slider-settings .pb-hero-add:disabled{opacity:.45;cursor:not-allowed}.pb-hero-slider-settings .pb-hero-devices{display:flex;gap:4px;margin-bottom:12px;padding:3px;border:1px solid var(--line);border-radius:9px;background:var(--soft)}.pb-hero-slider-settings :deep(.pb-hero-devices button){flex:1;border:0!important;border-radius:6px!important;background:transparent}.pb-hero-slider-settings :deep(.pb-hero-devices button.active){background:#fff;box-shadow:var(--shadow-sm)}.pb-hero-slider-settings .pb-media-field.has-action{display:flex}.pb-hero-slider-settings .pb-media-field.has-action input{border-radius:6px 0 0 6px!important}.pb-hero-slider-settings .pb-media-field.has-action button{width:46px;border:1px solid var(--line);border-left:0;border-radius:0 6px 6px 0;background:#fff;color:var(--primary,#6979f8);cursor:pointer}.pb-hero-slider-settings .pb-hero-slider-button-row{display:grid;grid-template-columns:1fr 1.5fr auto;gap:6px;margin-top:6px}.pb-hero-slider-settings .pb-hero-slider-button-row button{width:32px;border:1px solid var(--line);border-radius:6px;background:#fff;color:var(--muted);cursor:pointer}.pb-hero-slider-settings .pb-hero-slider-add-button{margin-top:8px;border:0;background:transparent;color:var(--primary,#6979f8);cursor:pointer}.pb-hero-slider-settings .pb-hero-slider-add-button:disabled{opacity:.45;cursor:not-allowed}.pb-hero-slider-settings .pb-form-note{font-size:12px;line-height:1.4;color:var(--muted)}
</style>

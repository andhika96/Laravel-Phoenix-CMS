(async function () {
	...
	const sharedControlPaths = Object.freeze({
		...
		gridColumnStyle: '/js/pagebuilder_elementor_v23/widgets/shared/GridColumnStyleControls.vue',
	});
	...
	function gridColumnCountForNode(node) {
		...
		const raw = ['container', 'container_fluid'].includes(node && node.type) && (settings.displayType || 'flex') === 'grid'
	...
	const NODE_LABEL_ICONS = Object.freeze({
		...
		feature_showcase: 'fas fa-layer-group',
		tabs: 'far fa-folder',
	...
	const BuilderNode = {
		...
		hasNewGeneralAdvancedControls() {
			return ['counter', 'progress_bar', 'testimonial', 'social_icons', 'alert', 'rating', 'text_path'].includes(this.node.type)
				|| ['form', 'slides', 'animated_headline', 'hotspot', 'price_list', 'price_table', 'call_to_action', 'countdown', 'carousel', 'reviews', 'testimonial_carousel', 'media_carousel', 'flip_box', 'code_highlight', 'blockquote', 'share_buttons', 'progress_tracker', 'video_playlist', 'hero_banner', 'hero_slider', 'product_color_selector', 'feature_showcase'].includes(this.node.type);
		},

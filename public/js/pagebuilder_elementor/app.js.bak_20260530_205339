(function () {
	...
	const sfcOptions = {
		moduleCache: { vue: Vue },
		getFile(url) { return fetch(url).then(r => { if (!r.ok) throw new Error(url); return r.text(); }); },
		addStyle(css) {
		...
		computed: {
			...
			gridStyle() {
				...
				const style = {
					...
					columnGap: cssSize(s.columnGap || s.gap, '20px'),
					rowGap: cssSize(s.rowGap || s.gap, '20px'),
					gridAutoFlow: s.autoFlow || 'row',
				};
				const rows = gridRowsTemplate(s.gridRows);
			...
			contColumnsStyle() {
				...
				if (dt === 'grid') {
					const cols = s.gridTemplateColumns || ('repeat(' + clamp(Number(s.gridColumns || 3), 1, 12) + ', minmax(0, 1fr))');
					const style = {
						display: 'grid',
						gridTemplateColumns: cols,
						columnGap: cssSize(s.gridColumnGap, '20px'),
						rowGap: cssSize(s.gridRowGap, '20px'),
						gridAutoFlow: s.autoFlow || 'row',
						justifyItems: s.gridJustifyItems || 'stretch',
						alignItems: s.gridAlignItems || 'start',
						width: '100%',
					};
					const rows = containerGridRowsTemplate(s.gridRows); if (rows) style.gridTemplateRows = rows;
					return style;
				}
				if (dt === 'flex') {
					return { display:'flex', flexDirection:s.direction||'row', flexWrap:s.flexWrap||'nowrap',
						justifyContent:s.justifyContent||'flex-start', alignItems:s.alignItems||'flex-start', alignContent:s.alignContent||'stretch',
						gap:cssSize(s.gap,'0'), rowGap:cssSize(s.flexRowGap||s.gap,'0'), columnGap:cssSize(s.flexColumnGap||s.gap,'0'), width:'100%' };
				}
			...
			colGroup() {
				return {
					...
					put: (to, from, el) => {
						...
						const nodeType = el && el.dataset && el.dataset.nodeType;
						// Tolak jika yang di-drag adalah container
		...
		methods: {
			...
			contColStyle(col) {
				...
				if (dt === 'flex') {
					...
					const wrapMode = s.flexWrap || 'nowrap';
			...
			contDropzoneStyle(col) {
				...
				const alignItems = String(s.gridAlignItems || 'start').toLowerCase();
				return { ...base, justifyContent: mapMain[alignItems] || 'flex-start' };
			},
			contGridNodeStyle(col) {
				const s = this.node.settings || {};
				if ((s.displayType || 'flex') !== 'grid') return {};

				const justifyItems = String(s.gridJustifyItems || 'stretch').toLowerCase();
				const style = {
					maxWidth: '100%',
					marginTop: '0',
					marginBottom: '0',
...
<div
	class="pb-node"
	:class="['pb-node-' + node.type, { active: selectedId === node.id, 'pb-grid-outline-enabled': !!node.settings?.gridOutline }]"
	:data-hover-label="label"
	:data-node-type="node.type"
	:data-node-id="node.id"
>
	<div class="pb-node-toolbar" @click.stop>
		<div class="pb-node-label"><i class="fa-solid fa-cube"></i> {{ label }}</div>
		<span class="pb-node-label-sep"></span>
		<div class="pb-node-actions">
			<button v-if="isCont" class="pb-node-btn" @click.stop="onOpenModal(node.type, 'root', { containerNode: node, list: [] })" title="Structure"><i class="fa-solid fa-table-columns"></i></button>
			<button class="pb-node-btn" @click.stop="onDuplicate(node.id)" title="Duplicate"><i class="fa-regular fa-copy"></i></button>
			<button class="pb-node-btn remove" @click.stop="onRemove(node.id)" title="Delete"><i class="fa-solid fa-trash"></i></button>
		</div>
	</div>

	<div class="pb-node-content" @click.stop="onSelect(node)">
				...
				<div class="el-cont-columns" :style="contColumnsStyle">
					...
					>
						<draggable
							...
							@add="(e) => onAddCol(e, col)"
							@start="onDragStart"
							@end="onDragEnd"
						>
							<template #header>
								<div v-if="!col.children || col.children.length === 0" class="pb-grid-col-label pb-grid-col-label-inside">Column {{ ci + 1 }}</div>
							</template>
							<template #item="{ element }">
								<BuilderNode :node="element" :style="contGridNodeStyle(col)" v-bind="passdown()" />
							</template>
							<template #footer>
		...
		<template v-else-if="isGrid">
			<component :is="loadWidget(node.type)" :item="node">
				<div class="el-grid-columns" :style="gridStyle">
					<div v-for="(col, ci) in node.columns" :key="col.id" class="el-grid-col pb-grid-col">
						<draggable
							v-model="col.children"
							item-key="id"
							:group="colGroup"
	...
	createApp({
		...
		setup() {
			...
			const responsiveDevices = [
				...
				{ value: 'tablet', label: 'Tablet', icon: 'fa-solid fa-tablet-screen-button' },
				{ value: 'mobile', label: 'Mobile', icon: 'fa-solid fa-mobile-screen-button' },
			];
			...
			// ── Column sync ───────────────────────────────────────────────────
			function syncCols(node, forceCount) {
				if (!node) return;
				const isContNode = isCont(node.type);
				const isGridNode = isGrid(node.type);
				if (!isContNode && !isGridNode) return;
				const colSetting = isContNode ? (node.settings.gridColumns || 3) : (node.settings.columns || 1);
				const baseCols = forceCount != null ? clamp(Number(forceCount), 1, 12) : clamp(Number(colSetting), 1, 12);
				let t = baseCols;
				if (isContNode && (node.settings.displayType || 'flex') === 'grid') {
					const rows = containerGridRowsCount(node.settings.gridRows);
			...
			function activeResponsiveKey(base) {
				return responsiveKey(base, responsiveDevice.value);
			}
			...
			function containerWidthSource(node) {
				...
				return boxedMode ? (s.maxWidth || '') : (s.containerWidth || '');
			}
			function containerWidthValue(node) {
				const parsed = parseNumberUnit(containerWidthSource(node), '%', ['px', '%', 'vw']);
			...
			function setContainerWidthValue(node, next) {
				...
				const token = toSizeToken(next, unit, s.contentWidth === 'boxed' ? 'auto' : '100%');
				if (s.contentWidth === 'boxed') s.maxWidth = token;
				else s.containerWidth = token;
			}
			function setContainerWidthUnit(node, unit) {
				if (!node || !node.settings) return;
				const allowed = ['px', '%', 'vw'];
				const safe = allowed.includes(unit) ? unit : '%';
				const value = containerWidthValue(node);
				setContainerWidthValue(node, value);
				if (node.settings.contentWidth === 'boxed') {
					node.settings.maxWidth = toSizeToken(value, safe, 'auto');
				} else {
					node.settings.containerWidth = toSizeToken(value, safe, '100%');
				}
			}
			function onContainerContentWidthChange(node) {
				...
				if (s.contentWidth === 'boxed') {
					if (!s.maxWidth || String(s.maxWidth).trim() === '') {
						s.maxWidth = s.containerWidth || '100%';
					}
					return;
				}
				if (!s.containerWidth || String(s.containerWidth).trim() === '' || s.containerWidth === 'auto') {
					s.containerWidth = (s.maxWidth && s.maxWidth !== 'auto') ? s.maxWidth : '100%';
				}
			}
			function minHeightValue(node) {
				if (!node || !node.settings) return '';
				const parsed = parseNumberUnit(node.settings.minHeight, 'px', ['px', 'vh']);
				return parsed.value;
			}
			...
			function setMinHeightValue(node, next) {
				...
				node.settings.minHeight = toSizeToken(next, unit, 'auto');
			}
			function setMinHeightUnit(node, unit) {
				if (!node || !node.settings) return;
				const safe = ['px', 'vh'].includes(unit) ? unit : 'px';
				const value = minHeightValue(node);
				node.settings.minHeight = toSizeToken(value, safe, 'auto');
			}
			function containerGridRowsValue(node) {
				if (!node || !node.settings) return 1;
				return containerGridRowsCount(node.settings.gridRows);
			}
			function setContainerGridRowsValue(node, next) {
				if (!node || !node.settings) return;
				const value = clamp(Number(next) || 1, 1, 12);
				node.settings.gridRows = String(value);
			}
			function syncContainerGap(settings, source) {
				if (!settings || !settings.containerGapLinked) return;
				if (source === 'column') settings.flexRowGap = settings.flexColumnGap;
				if (source === 'row') settings.flexColumnGap = settings.flexRowGap;
				if (source === 'gridColumn') settings.gridRowGap = settings.gridColumnGap;
				if (source === 'gridRow') settings.gridColumnGap = settings.gridRowGap;
			}
			...
			function onContainerDisplayTypeChange(node) {
				...
				if (dt === 'flex') {
					const nextRow = firstGapValue(s.gridRowGap, s.rowGap, s.gridColumnGap, s.columnGap, s.gap, '0');
					const nextCol = firstGapValue(s.gridColumnGap, s.columnGap, s.gridRowGap, s.rowGap, s.gap, nextRow, '0');

					// Selalu sinkron saat switch mode agar user melihat value yang konsisten.
					s.flexRowGap = nextRow;
					s.flexColumnGap = nextCol;
					s.rowGap = nextRow;
					s.columnGap = nextCol;
				...
				if (dt === 'grid') {
					const nextRow = firstGapValue(s.flexRowGap, s.rowGap, s.gap, s.flexColumnGap, '10px');
					const nextCol = firstGapValue(s.flexColumnGap, s.columnGap, s.gap, s.flexRowGap, nextRow, '10px');

					// Selalu sinkron saat switch mode agar user melihat value yang konsisten.
					s.gridRowGap = nextRow;
					s.gridColumnGap = nextCol;
					s.rowGap = nextRow;
					s.columnGap = nextCol;
				}
			}
			function syncGridColumnsForDevice(node) {
				if (responsiveDevice.value === 'desktop') syncCols(node);
			}
			watch(selectedNode, n => { if (n && isGrid(n.type)) syncCols(n); }, { deep: true });
			// Sync jumlah cell saat Grid Container desktop settings berubah.
			watch(
				() => [
				...
				() => {
					...
					if (node && isCont(node.type)) syncCols(node);
				}
			);
			watch(selectedNode, n => { if (n?.type==='video') n.settings.youtubeEmbed = toEmbed(n.settings.youtubeUrl||''); }, { deep: true });
			watch(selectedId, () => { settingsTab.value = 'layout'; responsiveDevice.value = 'desktop'; });
			...
			function removeNode(id) { delFrom(rootNodes.value, id); if (selectedId.value===id) selectedId.value=''; }
			function dupNode(id)    { dupIn(rootNodes.value, id); }
			function selectNode(n)  { selectedId.value = n.id; }
			function clearSel()     { selectedId.value = ''; }
			...
			function onRootAdd(evt) {
				...
				if (isCont(item.type)) {
					openModal(item.type, 'root', { containerNode: item, list: rootNodes.value });
					return;
				}
				...
				if (isWgt(item.type)) {
					nextTick(() => {
						...
						const c = makeNode('container');
						c.settings.displayType = 'block'; c.settings.gridColumns = 1;
			...
			// ── Structure modal ───────────────────────────────────────────────
			const modal = ref({ visible:false, step:1, layoutType:'', zone:'', list:null });
			function openModal(layoutType, zone, list) {
				modal.value = { visible:true, step: (layoutType==='container'||layoutType==='container_fluid')?1:2, layoutType, zone, list };
			}
					...
					function configureContainer(c, isGridLayout) {
						if (isGridLayout || layoutType === 'container_grid') {
							...
							if (p.flexWidths && p.flexWidths.length) {
								c.settings.gridTemplateColumns = p.flexWidths.join(' ');
			...
			function applyGridPreset(cols) {
				...
				const toolbox = {
				layout: [
					{ type:'container',       label:'Container',       icon:'fa-regular fa-square-full' },
					{ type:'container_fluid', label:'Container Fluid', icon:'fa-solid fa-grip-lines-vertical' },
					{ type:'grid',            label:'Grid',            icon:'fa-solid fa-table-cells-large' },
				],
				basic: [
					{ type:'heading',     label:'Heading',     icon:'fa-solid fa-heading' },
					{ type:'text_editor', label:'Text Editor', icon:'fa-solid fa-pen-to-square' },
					{ type:'image',       label:'Image',       icon:'fa-regular fa-image' },
					{ type:'video',       label:'Video',       icon:'fa-solid fa-video' },
					{ type:'button',      label:'Button',      icon:'fa-solid fa-link' },
			...
			return {
				...
				selectedId, settingsTab, selectedNode, selectedType,
				responsiveDevice, responsiveDevices, activeResponsiveKey, syncResponsiveSides, syncGridGap, syncGridColumnsForDevice,
				onContainerDisplayTypeChange, onContainerContentWidthChange,
				containerWidthValue, containerWidthUnit, setContainerWidthValue, setContainerWidthUnit,
				...
				displayNodeLabel,
				selectNode, clearSel, removeNode, dupNode, syncCols, chooseBgImage, clearBgImage, chooseMedia, clearMedia,
				pageName, pageStatus, customCss, savePage, saveState, saveMsg,
				toastVisible, responseStatusToast, isArrayMessageAfterSubmit, responseMessageAfterSubmit, closeToast,
...
<div class="pb-app">
	<div class="pb-topbar">
		...
		<div class="pb-top-actions">
			<button class="pb-btn icon" :disabled="!canUndo" @click="undo" title="Undo"><i class="fa-solid fa-rotate-left"></i></button>
			<button class="pb-btn icon" :disabled="!canRedo" @click="redo" title="Redo"><i class="fa-solid fa-rotate-right"></i></button>
	...
	<div class="pb-main">
		<div class="pb-panel left">
			<div v-if="!selectedNode">
				<div class="pb-section">
					...
					<div class="pb-form-group mt-3"><label class="pb-form-label">Custom CSS</label><textarea class="pb-textarea" v-model="customCss"></textarea></div>
				</div>
				...
				<div class="pb-section">
					<div class="pb-props-header">
						<button class="pb-btn icon-sm" @click="clearSel" title="Back"><i class="fa-solid fa-chevron-left"></i></button>
						<div class="pb-props-type-badge">
							<i :class="selectedType==='container'?'fa-regular fa-square-full':selectedType==='container_fluid'?'fa-solid fa-grip-lines-vertical':selectedType==='grid'||selectedType==='row_grid'?'fa-solid fa-table-cells-large':'fa-solid fa-cube'"></i>
					...
					<template v-if="selectedType==='container'||selectedType==='container_fluid'">
						<div class="pb-tab-nav">
							<button class="pb-tab-btn pb-tab-btn-icon" :class="{active:settingsTab==='layout'}" @click="settingsTab='layout'"><i class="fa-regular fa-square"></i><span>Layout</span></button>
							<button class="pb-tab-btn pb-tab-btn-icon" :class="{active:settingsTab==='style'}" @click="settingsTab='style'"><i class="fa-solid fa-circle-half-stroke"></i><span>Style</span></button>
							<button class="pb-tab-btn pb-tab-btn-icon" :class="{active:settingsTab==='advanced'}" @click="settingsTab='advanced'"><i class="fa-solid fa-gear"></i><span>Advanced</span></button>
						</div>
						<!-- TAB LAYOUT -->
						<div v-show="settingsTab==='layout'" class="pb-tab-content">
							<details class="pb-collapsible" open>
								...
								<div class="pb-form-group">
									...
									<div class="pb-range-value-row">
										<input type="range" class="pb-range" min="0" max="2000" step="1" :value="containerWidthValue(selectedNode)" @input="setContainerWidthValue(selectedNode, $event.target.value)">
										<div class="pb-value-with-unit">
											<input class="pb-input pb-input-compact" type="number" min="0" max="2000" step="1" :value="containerWidthValue(selectedNode)" @input="setContainerWidthValue(selectedNode, $event.target.value)">
								...
								<div class="pb-form-group">
									<label class="pb-form-label">Min Height</label>
									<div class="pb-range-value-row">
							...
							<details class="pb-collapsible" v-if="selectedNode.settings.displayType==='flex'||!selectedNode.settings.displayType" open>
								...
								<div class="pb-form-group">
									...
									<div class="pb-btn-group">
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.direction==='row'}"            @click="selectedNode.settings.direction='row'"            title="Row"><i class="fa-solid fa-arrow-right"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.direction==='column'}"         @click="selectedNode.settings.direction='column'"         title="Column"><i class="fa-solid fa-arrow-down"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.direction==='row-reverse'}"    @click="selectedNode.settings.direction='row-reverse'"    title="Row Reverse"><i class="fa-solid fa-arrow-left"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.direction==='column-reverse'}" @click="selectedNode.settings.direction='column-reverse'" title="Col Reverse"><i class="fa-solid fa-arrow-up"></i></button>
									</div>
								</div>
								<div class="pb-form-group">
									<label class="pb-form-label">Justify Content</label>
									<div class="pb-btn-group">
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.justifyContent==='flex-start'}"    @click="selectedNode.settings.justifyContent='flex-start'"    title="Start"><i class="fa-solid fa-align-left"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.justifyContent==='center'}"        @click="selectedNode.settings.justifyContent='center'"        title="Center"><i class="fa-solid fa-align-center"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.justifyContent==='flex-end'}"      @click="selectedNode.settings.justifyContent='flex-end'"      title="End"><i class="fa-solid fa-align-right"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.justifyContent==='space-between'}" @click="selectedNode.settings.justifyContent='space-between'" title="Space Between"><i class="fa-solid fa-arrows-left-right-to-line"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.justifyContent==='space-around'}"  @click="selectedNode.settings.justifyContent='space-around'"  title="Space Around"><i class="fa-solid fa-grip-lines-vertical"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.justifyContent==='space-evenly'}"  @click="selectedNode.settings.justifyContent='space-evenly'"  title="Space Evenly"><i class="fa-solid fa-grip"></i></button>
									</div>
								</div>
								<div class="pb-form-group">
									<label class="pb-form-label">Align Items</label>
									<div class="pb-btn-group">
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.alignItems==='flex-start'}" @click="selectedNode.settings.alignItems='flex-start'" title="Start"><i class="fa-solid fa-arrow-up"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.alignItems==='center'}"    @click="selectedNode.settings.alignItems='center'"    title="Center"><i class="fa-solid fa-arrows-up-down"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.alignItems==='flex-end'}"  @click="selectedNode.settings.alignItems='flex-end'"  title="End"><i class="fa-solid fa-arrow-down"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.alignItems==='stretch'}"   @click="selectedNode.settings.alignItems='stretch'"   title="Stretch"><i class="fa-solid fa-arrows-up-down-left-right"></i></button>
									</div>
								</div>
								<div class="pb-form-group">
									...
									<div class="pb-gap-row pb-gap-row-with-link">
										<div class="pb-gap-field"><input class="pb-input" v-model="selectedNode.settings.flexColumnGap" @input="syncContainerGap(selectedNode.settings, 'column')" placeholder="20px"><span>Column</span></div>
										<div class="pb-gap-field"><input class="pb-input" v-model="selectedNode.settings.flexRowGap" @input="syncContainerGap(selectedNode.settings, 'row')" placeholder="20px"><span>Row</span></div>
										<button class="pb-link-btn" @click="selectedNode.settings.containerGapLinked=!selectedNode.settings.containerGapLinked" :title="selectedNode.settings.containerGapLinked?'Unlink':'Link'"><i :class="selectedNode.settings.containerGapLinked?'fa-solid fa-link':'fa-solid fa-link-slash'"></i></button>
									</div>
								</div>
								<div class="pb-form-group">
									<label class="pb-form-label">Wrap</label>
									<div class="pb-btn-group">
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.flexWrap==='nowrap'}" @click="selectedNode.settings.flexWrap='nowrap'" title="No Wrap"><i class="fa-solid fa-right-long"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.flexWrap==='wrap'}" @click="selectedNode.settings.flexWrap='wrap'" title="Wrap"><i class="fa-solid fa-turn-down"></i></button>
									</div>
								...
								<div class="pb-form-group" v-if="selectedNode.settings.flexWrap==='wrap' || selectedNode.settings.flexWrap==='wrap-reverse'">
									<label class="pb-form-label">Align Content</label>
									<div class="pb-btn-group">
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.alignContent==='flex-start'}"    @click="selectedNode.settings.alignContent='flex-start'"    title="Start"><i class="fa-solid fa-align-left"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.alignContent==='center'}"        @click="selectedNode.settings.alignContent='center'"        title="Center"><i class="fa-solid fa-align-center"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.alignContent==='flex-end'}"      @click="selectedNode.settings.alignContent='flex-end'"      title="End"><i class="fa-solid fa-align-right"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.alignContent==='space-between'}" @click="selectedNode.settings.alignContent='space-between'" title="Space Between"><i class="fa-solid fa-arrows-left-right-to-line"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.alignContent==='space-around'}"  @click="selectedNode.settings.alignContent='space-around'"  title="Space Around"><i class="fa-solid fa-grip-lines-vertical"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.alignContent==='stretch'}"       @click="selectedNode.settings.alignContent='stretch'"       title="Stretch"><i class="fa-solid fa-left-right"></i></button>
									</div>
								</div>
								</div>
							</details>
							<details class="pb-collapsible" v-if="selectedNode.settings.displayType==='grid'" open>
								...
								<div class="pb-form-group">
									...
									<div class="pb-range-value-row">
										<input type="range" class="pb-range" min="1" max="12" step="1" :value="selectedNode.settings.gridColumns || 3" @input="selectedNode.settings.gridColumns=Number($event.target.value)">
										<input class="pb-input pb-input-compact" type="number" min="1" max="12" step="1" :value="selectedNode.settings.gridColumns || 3" @input="selectedNode.settings.gridColumns=Number($event.target.value)">
									</div>
								</div>
								<div class="pb-form-group">
									<div class="pb-label-row">
								...
								<div class="pb-form-group">
									...
									<div class="pb-gap-row pb-gap-row-with-link">
										<div class="pb-gap-field"><input class="pb-input" v-model="selectedNode.settings.gridColumnGap" @input="syncContainerGap(selectedNode.settings, 'gridColumn')" placeholder="20px"><span>Column</span></div>
										<div class="pb-gap-field"><input class="pb-input" v-model="selectedNode.settings.gridRowGap" @input="syncContainerGap(selectedNode.settings, 'gridRow')" placeholder="20px"><span>Row</span></div>
										<button class="pb-link-btn" @click="selectedNode.settings.containerGapLinked=!selectedNode.settings.containerGapLinked" :title="selectedNode.settings.containerGapLinked?'Unlink':'Link'"><i :class="selectedNode.settings.containerGapLinked?'fa-solid fa-link':'fa-solid fa-link-slash'"></i></button>
									</div>
								</div>
								<div class="pb-form-group">
								...
								<div class="pb-form-group">
									<label class="pb-form-label">Justify Items</label>
									<div class="pb-btn-group">
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.gridJustifyItems==='start'}" @click="selectedNode.settings.gridJustifyItems='start'" title="Start"><i class="fa-solid fa-align-left"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.gridJustifyItems==='center'}" @click="selectedNode.settings.gridJustifyItems='center'" title="Center"><i class="fa-solid fa-align-center"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.gridJustifyItems==='end'}" @click="selectedNode.settings.gridJustifyItems='end'" title="End"><i class="fa-solid fa-align-right"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.gridJustifyItems==='stretch'}" @click="selectedNode.settings.gridJustifyItems='stretch'" title="Stretch"><i class="fa-solid fa-left-right"></i></button>
									</div>
								</div>
								<div class="pb-form-group">
									<label class="pb-form-label">Align Items</label>
									<div class="pb-btn-group">
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.gridAlignItems==='start'}" @click="selectedNode.settings.gridAlignItems='start'" title="Start"><i class="fa-solid fa-arrow-up"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.gridAlignItems==='center'}" @click="selectedNode.settings.gridAlignItems='center'" title="Center"><i class="fa-solid fa-arrows-up-down"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.gridAlignItems==='end'}" @click="selectedNode.settings.gridAlignItems='end'" title="End"><i class="fa-solid fa-arrow-down"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.gridAlignItems==='stretch'}" @click="selectedNode.settings.gridAlignItems='stretch'" title="Stretch"><i class="fa-solid fa-arrows-up-down-left-right"></i></button>
									</div>
								</div>
								</div>
							</details>
						...
						<!-- TAB STYLE -->
						<div v-show="settingsTab==='style'" class="pb-tab-content">
							<details class="pb-collapsible" open>
										...
										<div class="pb-bg-media-field" :class="{ 'has-image': !!selectedNode.settings[bgStateKey(selectedNode,'bgImage')] }">
											<div class="pb-bg-media-preview" :style="selectedNode.settings[bgStateKey(selectedNode,'bgImage')] ? { backgroundImage: 'url(' + selectedNode.settings[bgStateKey(selectedNode,'bgImage')] + ')' } : {}">
												<button type="button" class="pb-bg-media-center-btn" :title="selectedNode.settings[bgStateKey(selectedNode,'bgImage')] ? 'Change Image' : 'Choose Image'" @click="chooseBgImage(selectedNode, bgStateKey(selectedNode,'bgImage'))">
													<i :class="selectedNode.settings[bgStateKey(selectedNode,'bgImage')] ? 'fa-solid fa-pen' : 'fa-solid fa-plus'"></i>
												</button>
											</div>
								...
								<div class="pb-form-group pb-toggle-label-row">
									...
									<div class="pb-toggle-switch-wrap">
										<div class="pb-toggle-wrap"><input type="checkbox" class="pb-toggle" :id="'containerScrollEffectsStyle-' + selectedNode.id" v-model="selectedNode.settings.scrollingEffects"><label :for="'containerScrollEffectsStyle-' + selectedNode.id"></label></div>
								...
								<div class="pb-form-group pb-toggle-label-row">
									...
									<div class="pb-toggle-switch-wrap">
										<div class="pb-toggle-wrap"><input type="checkbox" class="pb-toggle" :id="'containerMouseEffectsStyle-' + selectedNode.id" v-model="selectedNode.settings.mouseEffects"><label :for="'containerMouseEffectsStyle-' + selectedNode.id"></label></div>
								...
								<div class="pb-collapsible-body">
									<div class="pb-form-group">
										...
										<div class="pb-btn-group pb-icon-btn-group">
											<button class="pb-seg-btn pb-icon-btn" :class="{active:selectedNode.settings[bgStateKey(selectedNode,'bgOverlayType')]==='color'}" @click.prevent="setBgOverlayTypeForState(selectedNode, 'color')" title="Classic"><i class="fa-solid fa-brush"></i></button>
											<button class="pb-seg-btn pb-icon-btn" :class="{active:selectedNode.settings[bgStateKey(selectedNode,'bgOverlayType')]==='gradient'}" @click.prevent="setBgOverlayTypeForState(selectedNode, 'gradient')" title="Gradient"><i class="fa-solid fa-fill-drip"></i></button>
											<button class="pb-seg-btn pb-icon-btn" :class="{active:selectedNode.settings[bgStateKey(selectedNode,'bgOverlayType')]==='image'}" @click.prevent="setBgOverlayTypeForState(selectedNode, 'image')" title="Image"><i class="fa-regular fa-image"></i></button>
											<button class="pb-seg-btn pb-icon-btn" :class="{active:selectedNode.settings[bgStateKey(selectedNode,'bgOverlayType')]==='none'}" @click.prevent="setBgOverlayTypeForState(selectedNode, 'none')" title="None"><i class="fa-solid fa-ban"></i></button>
										</div>
									</div>
											...
											<div class="pb-bg-media-field" :class="{ 'has-image': !!selectedNode.settings[bgStateKey(selectedNode,'bgOverlayImage')] }">
												<div class="pb-bg-media-preview" :style="selectedNode.settings[bgStateKey(selectedNode,'bgOverlayImage')] ? { backgroundImage: 'url(' + selectedNode.settings[bgStateKey(selectedNode,'bgOverlayImage')] + ')' } : {}">
													<button type="button" class="pb-bg-media-center-btn" :title="selectedNode.settings[bgStateKey(selectedNode,'bgOverlayImage')] ? 'Change Image' : 'Choose Image'" @click="chooseBgImage(selectedNode, bgStateKey(selectedNode,'bgOverlayImage'))">
														<i :class="selectedNode.settings[bgStateKey(selectedNode,'bgOverlayImage')] ? 'fa-solid fa-pen' : 'fa-solid fa-plus'"></i>
													</button>
												</div>
												<div class="pb-bg-media-actions">
													...
													<button type="button" class="pb-bg-media-remove" :disabled="!selectedNode.settings[bgStateKey(selectedNode,'bgOverlayImage')]" title="Remove Image" @click="clearBgImage(selectedNode, bgStateKey(selectedNode,'bgOverlayImage'))"><i class="fa-solid fa-trash-can"></i></button>
												</div>
											</div>
										</div>
						...
						<!-- TAB ADVANCED -->
						<div v-show="settingsTab==='advanced'" class="pb-tab-content">
							<details class="pb-collapsible" open>
								<summary>Layout</summary>
								<div class="pb-collapsible-body">
								<div class="pb-form-group">
									<div class="pb-label-row">
										<label class="pb-form-label">Margin</label>
										<div class="pb-control-unit-wrap">px</div>
									</div>
									<div class="pb-label-row">
										<div></div>
										<button class="pb-link-btn" @click="selectedNode.settings.marginLinked=!selectedNode.settings.marginLinked" :title="selectedNode.settings.marginLinked?'Unlink':'Link'"><i :class="selectedNode.settings.marginLinked?'fa-solid fa-link':'fa-solid fa-link-slash'"></i></button>
									</div>
									<div class="pb-four-sides mt-1">
										<div class="pb-side-input"><input class="pb-input" v-model="selectedNode.settings[activeResponsiveKey('marginTop')]" @input="syncResponsiveSides(selectedNode.settings, 'margin', 'Top', selectedNode.settings.marginLinked)" placeholder=""><span>Top</span></div>
										<div class="pb-side-input"><input class="pb-input" v-model="selectedNode.settings[activeResponsiveKey('marginRight')]" @input="syncResponsiveSides(selectedNode.settings, 'margin', 'Right', selectedNode.settings.marginLinked)" placeholder=""><span>Right</span></div>
										<div class="pb-side-input"><input class="pb-input" v-model="selectedNode.settings[activeResponsiveKey('marginBottom')]" @input="syncResponsiveSides(selectedNode.settings, 'margin', 'Bottom', selectedNode.settings.marginLinked)" placeholder=""><span>Bottom</span></div>
										<div class="pb-side-input"><input class="pb-input" v-model="selectedNode.settings[activeResponsiveKey('marginLeft')]" @input="syncResponsiveSides(selectedNode.settings, 'margin', 'Left', selectedNode.settings.marginLinked)" placeholder=""><span>Left</span></div>
									</div>
								</div>
								<div class="pb-form-group">
									<div class="pb-label-row">
										<label class="pb-form-label">Padding</label>
										<div class="pb-control-unit-wrap">px</div>
									</div>
									<div class="pb-label-row">
										<div></div>
										<button class="pb-link-btn" @click="selectedNode.settings.paddingLinked=!selectedNode.settings.paddingLinked" :title="selectedNode.settings.paddingLinked?'Unlink':'Link'"><i :class="selectedNode.settings.paddingLinked?'fa-solid fa-link':'fa-solid fa-link-slash'"></i></button>
									</div>
									<div class="pb-four-sides mt-1">
										<div class="pb-side-input"><input class="pb-input" v-model="selectedNode.settings[activeResponsiveKey('paddingTop')]" @input="syncResponsiveSides(selectedNode.settings, 'padding', 'Top', selectedNode.settings.paddingLinked)" placeholder=""><span>Top</span></div>
										<div class="pb-side-input"><input class="pb-input" v-model="selectedNode.settings[activeResponsiveKey('paddingRight')]" @input="syncResponsiveSides(selectedNode.settings, 'padding', 'Right', selectedNode.settings.paddingLinked)" placeholder=""><span>Right</span></div>
										<div class="pb-side-input"><input class="pb-input" v-model="selectedNode.settings[activeResponsiveKey('paddingBottom')]" @input="syncResponsiveSides(selectedNode.settings, 'padding', 'Bottom', selectedNode.settings.paddingLinked)" placeholder=""><span>Bottom</span></div>
										<div class="pb-side-input"><input class="pb-input" v-model="selectedNode.settings[activeResponsiveKey('paddingLeft')]" @input="syncResponsiveSides(selectedNode.settings, 'padding', 'Left', selectedNode.settings.paddingLinked)" placeholder=""><span>Left</span></div>
									</div>
								</div>
								<div class="pb-form-group">
									<label class="pb-form-label">Align Self</label>
									<div class="pb-btn-group">
										<button class="pb-seg-btn" :class="{active:!selectedNode.settings.alignSelf||selectedNode.settings.alignSelf==='auto'}" @click="selectedNode.settings.alignSelf='auto'" title="Auto"><i class="fa-solid fa-minus"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.alignSelf==='flex-start'}" @click="selectedNode.settings.alignSelf='flex-start'" title="Start"><i class="fa-solid fa-arrow-up"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.alignSelf==='center'}" @click="selectedNode.settings.alignSelf='center'" title="Center"><i class="fa-solid fa-arrows-up-down"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.alignSelf==='flex-end'}" @click="selectedNode.settings.alignSelf='flex-end'" title="End"><i class="fa-solid fa-arrow-down"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.alignSelf==='stretch'}" @click="selectedNode.settings.alignSelf='stretch'" title="Stretch"><i class="fa-solid fa-arrows-up-down-left-right"></i></button>
									</div>
									<div class="pb-form-note">This control will affect contained elements only.</div>
								</div>
								<div class="pb-form-group">
									<label class="pb-form-label">Order</label>
									<div class="pb-btn-group">
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.order==='-1'}" @click="selectedNode.settings.order='-1'" title="Start"><i class="fa-solid fa-arrow-left"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.order===''||selectedNode.settings.order==null}" @click="selectedNode.settings.order=''" title="Default"><i class="fa-solid fa-ellipsis-vertical"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.order==='1'}" @click="selectedNode.settings.order='1'" title="End"><i class="fa-solid fa-arrow-right"></i></button>
									</div>
									<div class="pb-form-note">This control will affect contained elements only.</div>
								</div>
								<div class="pb-form-group">
									<label class="pb-form-label">Size</label>
									<div class="pb-btn-group">
										<button class="pb-seg-btn" :class="{active:!selectedNode.settings.sizeMode||selectedNode.settings.sizeMode==='default'}" @click="selectedNode.settings.sizeMode='default'" title="Default"><i class="fa-solid fa-ban"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.sizeMode==='grow'}" @click="selectedNode.settings.sizeMode='grow'" title="Grow"><i class="fa-solid fa-arrows-left-right"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.sizeMode==='shrink'}" @click="selectedNode.settings.sizeMode='shrink'" title="Shrink"><i class="fa-solid fa-compress"></i></button>
										<button class="pb-seg-btn" :class="{active:selectedNode.settings.sizeMode==='custom'}" @click="selectedNode.settings.sizeMode='custom'" title="Custom"><i class="fa-solid fa-ellipsis"></i></button>
									</div>
								</div>
								<div class="pb-form-group">
								...
								<div class="pb-form-group">
									...
									<div class="pb-input-with-action">
										...
										<button class="pb-field-action-btn" type="button" title="Dynamic Tag"><i class="fa-solid fa-database"></i></button>
									</div>
								</div>
								<div class="pb-form-group">
								...
								<div class="pb-form-group">
									<div class="pb-label-row">
										...
										<button class="pb-field-action-btn" type="button" title="Display Conditions"><i class="fa-solid fa-sitemap"></i></button>
									</div>
							...
							<details class="pb-collapsible">
								...
								<div class="pb-collapsible-body">
									<div class="pb-form-group">
										<div class="pb-label-row"><label class="pb-form-label mb-0">Animate With AI</label><div class="pb-toggle-wrap"><input type="checkbox" class="pb-toggle" :id="'containerAnimateWithAi-' + selectedNode.id" v-model="selectedNode.settings.animateWithAI"><label :for="'containerAnimateWithAi-' + selectedNode.id"></label></div></div>
									</div>
									<div class="pb-form-group">
					...
					<template v-if="selectedType==='grid'||selectedType==='row_grid'">
						<div class="pb-tab-nav">
							<button class="pb-tab-btn" :class="{active:settingsTab==='layout'}"   @click="settingsTab='layout'">Layout</button>
							<button class="pb-tab-btn" :class="{active:settingsTab==='style'}"    @click="settingsTab='style'">Style</button>
							<button class="pb-tab-btn" :class="{active:settingsTab==='advanced'}" @click="settingsTab='advanced'">Advanced</button>
						</div>
						<!-- TAB LAYOUT (GRID) -->
						<div v-show="settingsTab==='layout'" class="pb-tab-content">
							<div class="pb-prop-section">
								<div class="pb-label-row">
									...
									<div class="pb-responsive-switch">
										<button v-for="device in responsiveDevices" :key="device.value" class="pb-device-btn" :class="{active:responsiveDevice===device.value}" @click="responsiveDevice=device.value"><i :class="device.icon"></i><span>{{ device.label }}</span></button>
									</div>
								</div>
								<div class="pb-form-group">
								...
								<div class="pb-form-group">
									<div class="pb-label-row"><label class="pb-form-label mb-0">Column / Row Gap</label><button class="pb-link-btn" @click="selectedNode.settings.gapLinked=!selectedNode.settings.gapLinked" :title="selectedNode.settings.gapLinked?'Unlink':'Link'"><i :class="selectedNode.settings.gapLinked?'fa-solid fa-link':'fa-solid fa-link-slash'"></i></button></div>
						...
						<div v-show="settingsTab==='style'" class="pb-tab-content">
							<div class="pb-prop-section">
								...
								<div class="pb-form-group">
									<div class="pb-label-row"><label class="pb-form-label">Margin</label><button class="pb-link-btn" @click="selectedNode.settings.marginLinked=!selectedNode.settings.marginLinked" :title="selectedNode.settings.marginLinked?'Unlink':'Link'"><i :class="selectedNode.settings.marginLinked?'fa-solid fa-link':'fa-solid fa-link-slash'"></i></button></div>
									<div class="pb-four-sides mt-1">
										...
										<div class="pb-bg-media-field" :class="{ 'has-image': !!selectedNode.settings.bgImage }">
											<div class="pb-bg-media-preview" :style="selectedNode.settings.bgImage ? { backgroundImage: 'url(' + selectedNode.settings.bgImage + ')' } : {}">
												<button type="button" class="pb-bg-media-center-btn" :title="selectedNode.settings.bgImage ? 'Change Image' : 'Choose Image'" @click="chooseBgImage(selectedNode)">
													<i :class="selectedNode.settings.bgImage ? 'fa-solid fa-pen' : 'fa-solid fa-plus'"></i>
												</button>
											</div>
											<div class="pb-bg-media-actions">
						...
						<div v-show="settingsTab==='advanced'" class="pb-tab-content">
							<div class="pb-prop-section">
								...
								<div class="pb-form-group">
									<div class="pb-label-row"><label class="pb-form-label mb-0">Animate With AI</label><div class="pb-toggle-wrap"><input type="checkbox" class="pb-toggle" :id="'gridAnimateWithAi-' + selectedNode.id" v-model="selectedNode.settings.animateWithAI"><label :for="'gridAnimateWithAi-' + selectedNode.id"></label></div></div>
								</div>
								<div class="pb-form-group">
							...
							<div class="pb-prop-section">
								<div class="pb-label-row"><div class="pb-prop-section-title mb-0">Custom Attributes</div><button class="pb-seg-btn pb-mini-btn" @click="selectedNode.settings.attributes=(selectedNode.settings.attributes||[]).concat({name:'',value:''})"><i class="fa-solid fa-plus"></i></button></div>
								<div v-for="(attr,i) in (selectedNode.settings.attributes||[])" :key="i" class="pb-attr-row">
									...
									<button class="pb-btn icon-sm" @click="selectedNode.settings.attributes.splice(i,1)"><i class="fa-solid fa-trash"></i></button>
								</div>
							</div>
						</div>
					</template><!-- /grid tabs -->
					...
					<template v-if="selectedType==='text_editor'">
						<div class="pb-form-group"><label class="pb-form-label">Content</label><CkEditorField v-model="selectedNode.settings.html" /></div>
					</template>
					<template v-if="selectedType==='image'">
						<div class="pb-form-group">
							<label class="pb-form-label">Image</label>
							<div class="pb-bg-media-field" :class="{ 'has-image': !!selectedNode.settings.src }">
								<div class="pb-bg-media-preview" :style="selectedNode.settings.src ? { backgroundImage: 'url(' + selectedNode.settings.src + ')' } : {}">
									<button type="button" class="pb-bg-media-center-btn" :title="selectedNode.settings.src ? 'Change Image' : 'Choose Image'" @click="chooseMedia(selectedNode.settings, 'src')">
										<i :class="selectedNode.settings.src ? 'fa-solid fa-pen' : 'fa-solid fa-plus'"></i>
									</button>
								</div>
								<div class="pb-bg-media-actions">
		...
		<div class="pb-canvas-wrap" @click="clearSel">
			<div class="pb-stage-window" @click.stop>
				<div class="pb-stage-chrome">
					<div class="pb-stage-chrome-left">
						<span class="pb-stage-dot is-red"></span>
						<span class="pb-stage-dot is-yellow"></span>
						<span class="pb-stage-dot is-green"></span>
					</div>
					<div class="pb-stage-address">
						<i class="fa-solid fa-lock"></i>
						<span>Non code website builder</span>
					</div>
					<div class="pb-stage-chrome-right">
						<button class="pb-stage-icon-btn" type="button" title="Share"><i class="fa-solid fa-arrow-up-from-bracket"></i></button>
						<button class="pb-stage-icon-btn" type="button" title="Add"><i class="fa-solid fa-plus"></i></button>
						<button class="pb-stage-icon-btn" type="button" title="Copy"><i class="fa-regular fa-copy"></i></button>
					</div>
				</div>

				<div class="pb-stage-toolbar">
					<div class="pb-stage-device-group">
						<button class="pb-stage-device-btn" :class="{ active: responsiveDevice==='mobile' }" @click="responsiveDevice='mobile'" title="Mobile"><i class="fa-solid fa-mobile-screen-button"></i></button>
						<button class="pb-stage-device-btn" :class="{ active: responsiveDevice==='tablet' }" @click="responsiveDevice='tablet'" title="Tablet"><i class="fa-solid fa-tablet-screen-button"></i></button>
						<button class="pb-stage-device-btn" :class="{ active: responsiveDevice==='desktop' }" @click="responsiveDevice='desktop'" title="Desktop"><i class="fa-solid fa-desktop"></i></button>
					</div>
					<div class="pb-stage-width-pill">
						<i class="fa-solid fa-arrows-left-right"></i>
						<span>{{ responsiveDevice==='desktop' ? '1096px' : (responsiveDevice==='tablet' ? '820px' : '430px') }}</span>
	...
	<teleport to="body">
		<div v-if="modal.visible" class="pb-modal-backdrop" @click.self="closeModal">
			<div class="pb-modal">
				<div class="pb-modal-header"><button class="pb-modal-close" @click="closeModal"><i class="fa-solid fa-xmark"></i></button></div>

<template>
	<div :class="['el-widget-divider', customClass]" :style="wrapStyle">
		<hr :style="lineStyle">
	</div>
</template>
...
export default {
	...
	computed: {
		...
		lineStyle() {
			...
			const thickness = this.item.settings?.thickness ?? 2;
			return {
				margin: 0,
				border: 'none',
				borderTop: thickness + 'px ' + styleType + ' ' + (this.item.settings?.color || '#d0d7e6'),

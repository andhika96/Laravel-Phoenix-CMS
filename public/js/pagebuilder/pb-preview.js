	...
	const PreviewViewer = {
		...
		<div class="preview-container" style="position: relative; width: 100%; height: calc(100vh - 60px); overflow: auto; background-color: var(--builder-bg);">
			...
			<div class="preview-frame" :style="scaleStyle">
				...
				<template v-for="cont in data" :key="cont.id">
					<div :style="getPreviewSectionWrapperStyle(cont)">
						<div :class="[getContainerClass(cont), filterResponsiveClasses(cont.customClass)]" :style="getPreviewContainerStyle(cont)">
							<div v-for="row in cont.children" :key="row.id" class="row" :class="[getRowClasses(row), filterResponsiveClasses(row.customClass)]">
								<div v-for="col in row.children" :key="col.id" :class="[getColClasses(col), filterResponsiveClasses(col.customClass)]">
									<div v-for="item in col.children" :key="item.id">
										<div v-if="item.type === 'row'" class="row" :class="[getRowClasses(item), filterResponsiveClasses(item.customClass)]">
											<div v-for="nCol in item.children" :key="nCol.id" :class="[getColClasses(nCol), filterResponsiveClasses(nCol.customClass)]">
		...
		setup(props) {
			...
			// Helper: Section wrapper style (full width background)
			const getPreviewSectionWrapperStyle = (cont) => {
				...
				return {
					...
					backgroundImage: ss.bgImage ? 'url(' + ss.bgImage + ')' : '',
					backgroundPosition: ss.bgPos || 'center',
					backgroundRepeat: ss.bgRepeat || 'no-repeat',
					backgroundSize: ss.bgSize || 'cover',
					minHeight: ss.minHeight && ss.minHeight !== 'auto' ? ss.minHeight : ''
				};
			};

			// Helper: Container inner style
			const getPreviewContainerStyle = (cont) => {
				const target = cont.bgTarget || 'section';
				const s = cont.styles || {};
				if (target === 'section') {
			...
			return { getContainerClass, getRowClasses, getColClasses, filterResponsiveClasses, scaleStyle, getWidgetTemplate, getPreviewSectionWrapperStyle, getPreviewContainerStyle };
		}
	};

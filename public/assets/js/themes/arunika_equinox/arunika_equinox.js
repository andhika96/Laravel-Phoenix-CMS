...
let colorMainList = ['#0F766E', '#1FA675', '#9D00FF', '#1DA1F2', '#FF5733', '#FFC107', '#E91E63', '#6C5CE7', '#C7CCD8'];
const defaultEquinoxColor = '#0F766E';
...
function syncColorPickerSelection(color)
{
	...
	pickerContainer.querySelectorAll('.ph-color-switch').forEach((button) =>
	{
		const isActive = button.dataset.themeColor?.toUpperCase() === normalizedColor;
...
function renderColorPicker()
{
	...
	colorMainList.forEach((color) =>
	{
		...
		col.className = 'col-3';
		button.type = 'button';
		button.className = 'ph-color-switch';
		button.style.backgroundColor = color;
		button.dataset.themeColor = color;
		button.title = color;
		button.setAttribute('aria-label', `Use theme color ${color}`);
		button.setAttribute('aria-pressed', 'false');
		button.addEventListener('click', () => changeMainColor(color));

		col.appendChild(button);
		pickerContainer.appendChild(col);
	});

	syncColorPickerSelection(localStorage.getItem('theme-color') || defaultEquinoxColor);
}
...
function changeMainColor(color)
{
	...
	syncColorPickerSelection(color);
}
...
document.addEventListener('DOMContentLoaded', () =>
{
	...
	syncColorPickerSelection(savedColor);

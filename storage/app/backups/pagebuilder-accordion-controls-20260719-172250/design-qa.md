...
## 2026-07-19 - Page Builder Accordion interaction and controls

- Result: passed.
- Scope: Accordion widget controls and interaction parity against the supplied Elementor screenshots, while retaining the existing light page-builder design system.
- Accordion open and close states animate for the configured duration; Chrome captured an in-progress close frame at 96.6px with a 2s test duration, followed by the final 0px state.
- Scalar dimensions use a range slider, numeric input, and separate unit selector. Changing the first control from px to em changed its step to 0.1 and the live CSS token to 0em.

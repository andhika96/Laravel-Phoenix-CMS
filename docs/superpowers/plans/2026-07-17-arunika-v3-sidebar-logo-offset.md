...
**Files:**
- Modify: `tests/arunika-v3-content-frame-static.test.mjs`
...
Require `margin: 15px 15px 15px 0` on `.ph-layout-right` and `margin-top: 15px` on the V3 logo container. Run `node --test tests/arunika-v3-content-frame-static.test.mjs`; expect failure because the logo offset is absent.

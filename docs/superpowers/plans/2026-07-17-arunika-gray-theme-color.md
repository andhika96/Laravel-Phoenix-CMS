...
**Goal:** Add reference-matched cool gray `#C7CCD8` as the eighth Theme Color option in Arunika Mosaic, Aurora, and Canvas.
...
- Append `#C7CCD8` as the final item in all three palettes.
...
- Produces: eight color swatches per theme with `#C7CCD8` as the final swatch.
...
Create a Node test that parses `colorMainList` from each file and asserts eight values, one occurrence of `#C7CCD8`, and placement as the final item.

- [ ] **Step 3: Run the focused test and verify RED**

Run: `node --test tests/arunika-gray-theme-color-static.test.mjs`

Expected: three failures because each list does not yet contain `#C7CCD8`.

- [ ] **Step 4: Implement the minimal palette change**

Append `'#C7CCD8'` to each existing `colorMainList` array without modifying any other JavaScript behavior.

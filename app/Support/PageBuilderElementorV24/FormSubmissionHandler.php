...
class FormSubmissionHandler
{
    ...
    private function rules(array $definitions): array
    {
        ...
        foreach ($definitions as $id => $field) {
            ...
            $options = $this->optionValues($field, $field['resolvedOptions'] ?? null);
            ...
            $constrainOptions = in_array($type, ['select', 'radio'], true)
                && ($options !== [] || ($field['datasetMode'] ?? 'static') === 'dataset');
            if (! $multiple && $constrainOptions) {
    ...
    private function optionValues(array $field, ?array $resolvedOptions = null): array
    {
    ...
    private function resolveDatasetOptions(Page_Builder $page, array $definitions, array $values): array
    {
        $ownerId = (int) ($page->user_id ?? 1);

        foreach ($definitions as $id => &$field) {
            if (($field['datasetMode'] ?? 'static') !== 'dataset') {
                continue;
            }

            $datasetId = (int) ($field['datasetId'] ?? 0);
            $dataset = $datasetId > 0
                ? FormDataset::query()->where('user_id', $ownerId)->find($datasetId)
                : null;
            $nodes = is_array($dataset?->nodes) ? $dataset->nodes : [];
            $parentFieldId = trim((string) ($field['datasetParentFieldId'] ?? ''));
            $parentValues = $parentFieldId === ''
                ? []
                : (array) ($values[$parentFieldId] ?? []);
            $parentValues = array_values(array_filter(array_map('strval', $parentValues), fn ($value) => $value !== ''));

            if ($parentFieldId === '') {
                $parentIds = [null];
            } else {
                $parentIds = array_values(array_map(
                    fn (array $node): string => (string) $node['id'],
                    array_filter($nodes, fn (array $node): bool => in_array((string) ($node['id'] ?? ''), $parentValues, true)
                        || in_array((string) ($node['value'] ?? ''), $parentValues, true)),
                ));
            }

            $field['resolvedOptions'] = array_values(array_map(
                fn (array $node): array => [
                    'label' => (string) ($node['label'] ?? $node['name'] ?? $node['value'] ?? ''),
                    'value' => (string) ($node['value'] ?? $node['code'] ?? $node['id'] ?? ''),
                ],
                array_filter($nodes, fn (array $node): bool => ($node['active'] ?? true)
                    && in_array($node['parentId'] ?? null, $parentIds, true)),
            ));
        }
        unset($field);
    ...
    private function sendEmail(array $settings, string $suffix, array $definitions, array $fields, array $files): void
    {
        ...
        $mail = new PageBuilderElementorV24FormMail($subject, $body, $isHtml, $files);

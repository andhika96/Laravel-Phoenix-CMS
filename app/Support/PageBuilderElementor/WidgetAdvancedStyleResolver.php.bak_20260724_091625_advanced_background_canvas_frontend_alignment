        ...
        if ($type === 'classic') {
            ...
            if ($image !== '') {
                return [
                    ...
                    'background-position:'.$this->backgroundPosition($settings['advancedBackgroundPosition'.$suffix] ?? 'center center'),
                    'background-attachment:'.(($settings['advancedBackgroundAttachment'.$suffix] ?? 'scroll') === 'fixed' ? 'fixed' : 'scroll'),
                    'background-repeat:'.$this->enum($settings['advancedBackgroundRepeat'.$suffix] ?? 'no-repeat', ['no-repeat', 'repeat', 'repeat-x', 'repeat-y'], 'no-repeat'),
                    'background-size:'.$this->enum($settings['advancedBackgroundSize'.$suffix] ?? 'cover', ['auto', 'cover', 'contain', '100% 100%'], 'cover'),
                ];
            }

            return ['background-color:'.$this->color($settings['advancedBackgroundColor'.$suffix] ?? '', 'transparent')];
        }

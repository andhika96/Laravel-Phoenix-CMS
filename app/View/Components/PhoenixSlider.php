...
class PhoenixSlider extends Component
{
	 ...
	 */
	public function __construct($uri, $slug)
	{
		$this->uri = $uri;
		// $this->type = $type;
		$this->slug = $slug;
	...
	public function render(): View|Closure|string
	{
		...
		if ($getCoverImageDetail)
		{
			if (Browser::isDesktop() || Browser::isTablet())
			{
				...
				{
					return view('components.slider.desktop.phoenix-slider-horizontal-desktop', ['data' => $getCoverImageDetail, 'slug' => $this->slug]);
				}
				elseif ($getCoverImageDetail['cover_type'] == 'slideshow' &&
						$getCoverImageDetail['cover_slideshow_desktop_direction'] == 'vertical')
				{
					return view('components.slider.desktop.phoenix-slider-vertical-desktop', ['data' => $getCoverImageDetail, 'slug' => $this->slug]);
				}
				else
				{
					return view('components.slider.mobile.phoenix-slider-horizontal-mobile', ['data' => $getCoverImageDetail, 'slug' => $this->slug]);
				}
			}
			elseif (Browser::isMobile())
			{
				...
				{
					return view('components.slider.mobile.phoenix-slider-vertical-mobile', ['data' => $getCoverImageDetail, 'slug' => $this->slug]);
				}
				else
				{
					return view('components.slider.mobile.phoenix-slider-horizontal-mobile', ['data' => $getCoverImageDetail, 'slug' => $this->slug]);

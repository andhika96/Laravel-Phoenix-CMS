...
class ArunaPermission
{
	...
	public function handle(Request $request, Closure $next, string $permission): Response
	{
		...
		elseif (site_config()->management_menu == 'v2')
		{
			...
			$custom_permissions = Custom_Permissions::where('role_id', $userAccount->getCurrentUserRoleById())->where('menu_link', $getCurrentURI)->get();

			if (is_array($custom_permissions) && count($custom_permissions) > 0)
			{

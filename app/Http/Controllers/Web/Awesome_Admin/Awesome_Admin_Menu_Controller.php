...
class Awesome_Admin_Menu_Controller extends Controller
{
	...
	{
		return view('awesome_admin.menu.awesome_admin_menu');
	}
	...
	public function create_submenu(MenuRequest $request)
	{
		...
		try
		{
			if ($request->validated())
			{
				...
				$role = Sub_Menu_JSON::create(['parent_code' => $request->input('parent_code'), 'parent_name' => $getParentName]);
	...
	{
		return view('awesome_admin.awesome_admin_menu_submenu');
	}

	public function submenu_detail(string $idOrSlug)
	{
		$getDetailMenu = Sub_Menu_JSON::find($idOrSlug);

		if ( ! $getDetailMenu)
		{			
			$getDetailMenu = Sub_Menu_JSON::where('parent_code', $idOrSlug)->first();
		}

		if ($getDetailMenu)
		{
			return view('awesome_admin.awesome_admin_menu_submenu_detail', ['data' => $getDetailMenu, 'idOrSlug' => $idOrSlug]);
		}
	...
	public function parentmenu()
	{
		...
		return view('awesome_admin.awesome_admin_parent_menu', ['categorymenu' => $getListCategoryMenu]);
	}

	public function categorymenu()
	{
		return view('awesome_admin.awesome_admin_category_menu');
	}
	...
	public function update_parentmenu(Request $request)
	{
		...
		try 
		{
			$getDetailMenuParentJSON 	= Parent_Menu_JSON::find(1);
	...
	public function update_submenu_detail(string $idOrSlug)
	{
		...
		try 
		{
			$getDetailMenuParentJSON 	= Sub_Menu_JSON::where('parent_code', $idOrSlug)->first();
			$getJSONDecodeMenuVars 		= json_decode(request()->input('menu_vars'), true);
	...
	public function update_categorymenu(Request $request)
	{
		...
		try 
		{
			$getDetailMenuParentJSON 	= Category_Menu_JSON::find(1);
			$getJSONDecodeMenuVars 		= json_decode($request->input('menu_vars'), true);
	...
	public function delete_submenu(Request $request)
	{
		...
		try
		{
			$detail_data = Sub_Menu_JSON::where('id', $request->route('idOrSlug'))->first();
	...
	{
		$menu_categorylist = Category_Menu_JSON::where('menu_page', 'awesome_admin')->get();
	...
	{
		$menu_parentlist = Parent_Menu_JSON::where('menu_page', 'awesome_admin')->get();
	...
	{
		$menulist = Sub_Menu_JSON::where('parent_code', $request->route('idOrSlug'))->get();
	...
	{
		$parentMenu = Parent_Menu_JSON::where('menu_page', 'awesome_admin')->first();
	...
	{
		$subMenu = Sub_Menu_JSON::where('parent_code', $request->input('parent_code'))->first();

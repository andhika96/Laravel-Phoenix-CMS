...
class Awesome_Admin_Header_Navigation_Controller extends Controller
{
	...
	/**
	 * Get frontend menu data for the integrated header preview.
	 */
	public function previewData()
	{
		$items = collect(FrontendMenuBuilder::items())->map(function (array $item)
		{
			...
			$submenuCount = count($item['submenus']);
			...
			return [
				...
				'has_dropdown' => in_array($dropdownType, ['bootstrap', 'mega'], true) && $submenuCount > 0,
				'show_arrow' => (bool) ($dropdownBox['show_arrow'] ?? true),
				'toggle_icon_type' => $dropdownBox['toggle_icon_type'] ?? '',
				'toggle_icon_url' => $dropdownBox['toggle_icon_url'] ?? '',
				'toggle_icon_html' => $dropdownBox['toggle_icon_html'] ?? ''
			];
		})->values();

...
namespace App\Http\Controllers\Api\v1\Facebook\Adset;
...
class Adset_Controller extends Controller
{
	...
	public function list(Request $request): JsonResponse
	{
		try
		{
			...
			$cursor = $account->getAdSets($this->adsetFields, $this->params);
	...
	public function create(Request $request): JsonResponse
	{
		try
		{
			...
			[
				...
				'user_os' 				=> $request->input('user_os'),
				'publisher_platforms' 	=> $request->input('publisher_platforms'),
				'device_platforms' 		=> $request->input('device_platforms')
			];

			$createAdset = new AdAccount($this->adAccount);
			$newAdset = $createAdset->createAdSet($fields, $params);

			$resultAdset = $newAdset->exportAllData();
			$adsetId = $newAdset->{AdSetFields::ID};

			$response = $this->jsonResponse->respondOK(['data' => $adsetId]);
		}
		catch (RequestException $e) 
		{
			$getErrorMessage = 

...
class FileManagerV2Storage
{
    ...
    {
        $profile = config("filemanager_v2.connections.{$storage}");
    ...
    public function profiles(): array
    {
        ...
        return collect(config('filemanager_v2.connections', []))
            ...
            ->map(function (array $profile, string $id): array {
                return [
                    ...
                    'root' => $id === 'local' ? config('filemanager_v2.local.files_root') : '',
                ];
            })
            ->values()
            ->all();
    ...
    public function disk(string $storage): FilesystemAdapter
    {
        ...
        if (($profile['driver'] ?? null) === 'local') {
            ...
            return Storage::build([
                'driver' => 'local',
                'root' => config('filemanager_v2.local.files_root'),
                'throw' => false,
            ]);
        }
    ...
    public function bootstrap(): array
    {
        ...
        $default = config('filemanager_v2.default_storage', 'local');
        ...
        return [
            ...
            'profiles' => $profiles,
            'upload' => [
                'chunkSize' => config('filemanager_v2.uploads.chunk_size'),
                'chunkThreshold' => config('filemanager_v2.uploads.chunk_threshold'),
                'maxParallel' => config('filemanager_v2.uploads.max_parallel'),
                'maxFileSize' => config('filemanager_v2.uploads.max_file_size'),
            ],
        ];
    }
    ...
    {
        try {
            $this->disk($storage)->listContents('', false)->current();
    ...
    private function assertUploadMeta(string $name, int $size, int $parts): void
    {
        ...
        abort_if($size < 1 || $size > config('filemanager_v2.uploads.max_file_size'), 422, 'Ukuran file melampaui batas upload.');

...
if ( ! function_exists('menu_v2'))
{
	function menu_v2()
	{
			...
			foreach (get_menus_with_category_v2()['categorized'] as $key0 => $value0)
			{
				if (isset($value0['parent_menu']))
				{
					...
					<div class="ph-nav-category">
						<span class="ph-nav-category-initial">'.mb_strtoupper(mb_substr($value0['category_name'], 0, 1)).'</span>
						...
						foreach ($value0['parent_menu'] as $key1 => $value1)
						{
							...
							if ($value1['is_for_parent_menu'] == 'parent')
							{
								if (isset($value1['parent_submenu']) && is_array($value1['parent_submenu']['list']) && count($value1['parent_submenu']['list']) > 0)
								{
									...
									<div class="ph-floating-submenu">
										<div class="ph-floating-header">'.$value1['parent_name'].'</div>
								...
								{
									$output .= '
									<a href="'.url($value1['parent_link']).'" class="list-group-item list-group-item-action">
							...
							elseif ($value1['is_for_parent_menu'] == 'single')
							{
								...
								<a href="'.url($value1['parent_link']).'" class="list-group-item list-group-item-action">
									...
									<div class="ph-custom-tooltip"><span>'.$value1['parent_name'].'</span></div>
								</a>';
							}
						}
					}
			...
			count(get_menus_with_category_v2()['uncategorized']) > 0)
		{
			$output .= '
			<div class="ph-nav-category">
				<span class="ph-nav-category-initial">'.mb_strtoupper(mb_substr(t('Uncategorized'), 0, 1)).'</span>

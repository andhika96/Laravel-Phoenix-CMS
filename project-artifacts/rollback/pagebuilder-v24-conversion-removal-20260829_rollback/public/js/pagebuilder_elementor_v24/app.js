(async function () {
	...
	const widgetRegistry = window.PageBuilderElementorV24Widgets || null;
	function moduleDefinition(type) {
		return widgetRegistry?.get(String(type || '').trim()) || null;
	}
	function moduleCapabilities(type) {
		const capabilities = moduleDefinition(type)?.capabilities;
		return Array.isArray(capabilities) ? capabilities : [];
	}
	function hasModuleCapability(type, capability) {
		return moduleCapabilities(type).includes(capability);
	}
	function moduleTypeForCapability(capability) {
		return widgetRegistry?.all().find((definition) => moduleCapabilities(definition.type).includes(capability))?.type || '';
	}
	...
	function staticImportMetadata(nodes) {
		if (!Array.isArray(nodes)) return null;
		const root = nodes.find((node) => node && node.settings && node.settings.staticImport);
		const metadata = root && root.settings.staticImport;
		return metadata && typeof metadata === 'object' && !Array.isArray(metadata) ? metadata : null;
	}
	function safeStaticImportStylesheetUrl(value) {
		try {
	...
	function syncStaticImportDependencies(nodes) {
		const metadata = staticImportMetadata(nodes);
		if (!metadata) {
			document.querySelectorAll('[data-pb-static-import]').forEach((element) => element.remove());
			return;
		}
		if (metadata.mode === 'compiled') {
			document.querySelectorAll('[data-pb-static-import]').forEach((element) => element.remove());
			return;
		}
		const frameworks = Array.isArray(metadata.frameworks) ? metadata.frameworks : [];
		const stylesheets = (Array.isArray(metadata.stylesheets) ? metadata.stylesheets : [])
			.map(safeStaticImportStylesheetUrl)
			.filter(Boolean);
		stylesheets.forEach((href) => {
			if (Array.from(document.querySelectorAll('link[data-pb-static-import="stylesheet"]')).some((link) => link.href === href)) return;
			const link = document.createElement('link');
			link.rel = 'stylesheet';
			link.href = href;
			link.dataset.pbStaticImport = 'stylesheet';
			document.head.appendChild(link);
		});
		if (frameworks.includes('tailwind') && !document.querySelector('script[data-pb-static-import="tailwind"]')) {
			window.tailwind = window.tailwind || {};
			const importedTailwindConfig = metadata.tailwindConfig && typeof metadata.tailwindConfig === 'object'
				? metadata.tailwindConfig
				: {};
			const currentTheme = window.tailwind.config?.theme || {};
			window.tailwind.config = {
				...(window.tailwind.config || {}),
				...importedTailwindConfig,
				theme: {
					...currentTheme,
					...(importedTailwindConfig.theme || {}),
					extend: {
						...(currentTheme.extend || {}),
						...(importedTailwindConfig.theme?.extend || {}),
					},
				},
			...
			script.dataset.pbStaticImport = 'tailwind';
			document.head.appendChild(script);
		}
	}
	const LEFT_SIDEBAR_COLLAPSED_STORAGE_KEY = 'phoenix.pagebuilder.v24.leftCollapsed';
	...
	const sharedControlPaths = Object.freeze({
		advanced: '/pagebuilder-elementor/v2.4/shared-assets/advanced.vue',
		typography: '/pagebuilder-elementor/v2.4/shared-assets/typography.vue',
		link: '/pagebuilder-elementor/v2.4/shared-assets/link.vue',
		dynamicTag: '/pagebuilder-elementor/v2.4/shared-assets/dynamic-tag.vue',
		cssFilter: '/pagebuilder-elementor/v2.4/shared-assets/css-filter.vue',
		textStroke: '/pagebuilder-elementor/v2.4/shared-assets/text-stroke.vue',
		textShadow: '/pagebuilder-elementor/v2.4/shared-assets/text-shadow.vue',
		gridColumnStyle: '/pagebuilder-elementor/v2.4/shared-assets/grid-column-style.vue',
	});
	...
	const GridColumnStyleControls = defineAsyncComponent(() => loadSfcModule(sharedControlPaths.gridColumnStyle));
	...
	// V24_CONTEXT_TARGET_HELPERS_START
	...
	function resolveContextMenuTarget(event, target) {
		...
		if (!node || !hasModuleCapability(node.type, 'form-fields')) return target;
	...
	// V24_CONTEXT_CLIPBOARD_HELPERS_END
	function isCont(t)    { return hasModuleCapability(t, 'structure-container'); }
	function isGrid(t)    { return hasModuleCapability(t, 'structure-grid'); }
	function isTabs(t)    { return hasModuleCapability(t, 'nested-tabs'); }
	function isAccordion(t) { return hasModuleCapability(t, 'nested-accordion'); }
	...
	// V24_CHILD_CONTAINER_HELPERS_START
	...
	function migrateLegacyContainerColumns(node, state, createContainer) {
		if (!node || !isCont(node.type) || (node.settings?.displayType || 'flex') === 'grid' || !Array.isArray(node.columns)) return false;
		...
		const migratedChildren = legacyColumns.map((rawColumn, index) => {
			...
			child.type = defaultContainerType() || child.type;
	...
	function appendChildContainer(parent, createContainer) {
		...
		const existing = parent.children.filter((child) => child && isCont(child.type));
	...
	function gridColumnStyleDefaults() {
		const defaults = {
			borderType: 'none',
			borderColor: 'transparent',
			borderWidthTop: '0px', borderWidthRight: '0px', borderWidthBottom: '0px', borderWidthLeft: '0px',
			borderRadiusTL: '0px', borderRadiusTR: '0px', borderRadiusBR: '0px', borderRadiusBL: '0px',
			borderWidthLinked: true,
			borderRadiusLinked: true,
			bgType: 'none', bgColor: '#ffffff', bgOpacity: 1,
			bgGradientType: 'linear', bgGradientAngle: 90, bgGradientStart: '#ffffff', bgGradientEnd: '#000000', bgGradientPosition: 50,
			bgImage: '', bgSize: 'cover', bgPosition: 'center center', bgRepeat: 'no-repeat', bgAttachment: 'scroll',
		};
		[
			...
			'bgType', 'bgColor', 'bgOpacity', 'bgGradientType', 'bgGradientAngle', 'bgGradientStart', 'bgGradientEnd', 'bgGradientPosition',
			'bgImage', 'bgSize', 'bgPosition', 'bgRepeat', 'bgAttachment',
		].forEach((key) => {
			defaults[key + 'Tablet'] = '';
			defaults[key + 'Mobile'] = '';
		});
	...
	function normalizeGridColumnStyle(source, sparse = false) {
		const input = source && typeof source === 'object' && !Array.isArray(source) ? source : {};
		const defaults = gridColumnStyleDefaults();
		const output = sparse ? {} : { ...defaults };
		const borderTypes = ['none', 'solid', 'double', 'dotted', 'dashed', 'groove'];
		const backgroundTypes = ['none', 'color', 'gradient', 'image'];

		Object.keys(defaults).forEach((key) => {
			if (!Object.prototype.hasOwnProperty.call(input, key)) return;
			const raw = input[key];
			if (sparse && (raw === '' || raw === null || raw === undefined)) return;
			if (!sparse && (raw === '' || raw === null || raw === undefined)) {
				output[key] = defaults[key];
				return;
			}
			if (key.startsWith('borderType')) {
				output[key] = borderTypes.includes(String(raw || '').trim().toLowerCase()) ? String(raw).trim().toLowerCase() : (sparse ? 'none' : defaults[key]);
				return;
			}
			if (key.startsWith('bgType')) {
				output[key] = backgroundTypes.includes(String(raw || '').trim().toLowerCase()) ? String(raw).trim().toLowerCase() : (sparse ? 'none' : defaults[key]);
				return;
			}
			if (key.endsWith('Opacity') || key === 'bgOpacity' || key === 'bgOpacityTablet' || key === 'bgOpacityMobile') {
				const opacity = Number(raw);
				output[key] = Number.isFinite(opacity) ? Math.min(1, Math.max(0, opacity)) : (sparse ? 1 : defaults[key]);
				return;
			}
			if (key.startsWith('bgGradientAngle') || key.startsWith('bgGradientPosition')) {
				const numeric = Number(raw);
				output[key] = Number.isFinite(numeric) ? numeric : (sparse ? raw : defaults[key]);
				return;
			}
	...
	function gridColumnResponsiveValue(source, key, device = 'desktop', fallback = undefined) {
		const settings = source && typeof source === 'object' ? source : {};
		const suffix = device === 'tablet' ? 'Tablet' : (device === 'mobile' ? 'Mobile' : '');
		const responsive = settings[key + suffix];
		if (responsive !== '' && responsive !== null && responsive !== undefined) return responsive;
		const base = settings[key];
		return base !== '' && base !== null && base !== undefined ? base : fallback;
	}

	function reconcileGridColumnStyles(node, trackCount) {
		if (!node || typeof node !== 'object') return [];
		const count = Math.min(12, Math.max(1, Math.floor(Number(trackCount) || 1)));
		const current = Array.isArray(node.columnStyles) ? node.columnStyles : [];
		node.columnStyles = Array.from({ length: count }, (_, index) => normalizeGridColumnStyle(current[index], false));
		return node.columnStyles;
	}

	function resolveGridColumnStyle(trackStyle, cellOverrides, device = 'desktop') {
		const track = normalizeGridColumnStyle(trackStyle, false);
		const cell = normalizeGridColumnStyle(cellOverrides, true);
		const defaults = gridColumnStyleDefaults();
		const baseKeys = Object.keys(defaults).filter((key) => !key.endsWith('Tablet') && !key.endsWith('Mobile'));
		const resolved = {};

		baseKeys.forEach((key) => {
			const cellValue = gridColumnResponsiveValue(cell, key, device, undefined);
			resolved[key] = cellValue === undefined ? gridColumnResponsiveValue(track, key, device, defaults[key]) : cellValue;
		});

		return resolved;
	}

	function resetGridColumnStyleOverrides(cell) {
		if (!cell || typeof cell !== 'object') return;
		cell.styleOverrides = {};
	}

	function gridColumnCountForNode(node) {
		const settings = node && node.settings && typeof node.settings === 'object' ? node.settings : {};
		const raw = isCont(node && node.type) && (settings.displayType || 'flex') === 'grid'
			? settings.gridColumns
			: settings.columns;
		return Math.min(12, Math.max(1, Math.floor(Number(raw) || 1)));
	}

	function gridColumnStyleSafeMediaUrl(value) {
		const url = String(value == null ? '' : value).trim();
		if (!url || /[\u0000-\u001f\u007f"'()]/.test(url) || /^(?:javascript|vbscript|data):/i.test(url) || url.startsWith('//')) return '';
		return /^(?:https?:\/\/|\/)/i.test(url) ? url : '';
	}

	function gridColumnStyleSafeColor(value, fallback = 'transparent') {
		const color = String(value == null ? '' : value).trim();
		if (!color || /[;{}]/.test(color)) return fallback;
		return /^(?:#[0-9a-f]{3,8}|(?:rgb|rgba|hsl|hsla)\([^(){};]+\)|[a-z]+)$/i.test(color) ? color : fallback;
	}

	function gridColumnStyleToCss(style) {
		const source = style && typeof style === 'object' ? style : gridColumnStyleDefaults();
		const borderType = ['none', 'solid', 'double', 'dotted', 'dashed', 'groove'].includes(String(source.borderType || '').toLowerCase())
			? String(source.borderType).toLowerCase()
			: 'none';
		const css = {
			boxSizing: 'border-box',
			borderStyle: borderType,
			borderWidth: borderType === 'none' ? '0' : [source.borderWidthTop, source.borderWidthRight, source.borderWidthBottom, source.borderWidthLeft].map((value) => cssSize(value, '0')).join(' '),
			borderRadius: [source.borderRadiusTL, source.borderRadiusTR, source.borderRadiusBR, source.borderRadiusBL].map((value) => cssSize(value, '0')).join(' '),
		};
		if (borderType !== 'none') css.borderColor = gridColumnStyleSafeColor(source.borderColor, 'transparent');

		if (source.bgType === 'color') {
			css.backgroundColor = colorWithOpacity(gridColumnStyleSafeColor(source.bgColor, '#ffffff'), source.bgOpacity == null ? 1 : source.bgOpacity);
			css['--pb-grid-column-background'] = css.backgroundColor;
		} else if (source.bgType === 'gradient') {
			const start = gridColumnStyleSafeColor(source.bgGradientStart, '#ffffff');
			const end = gridColumnStyleSafeColor(source.bgGradientEnd, '#000000');
			const position = Number(source.bgGradientPosition == null ? 50 : source.bgGradientPosition);
			css.backgroundImage = source.bgGradientType === 'radial'
				? 'radial-gradient(circle, ' + start + ' 0%, ' + end + ' ' + position + '%)'
				: 'linear-gradient(' + (Number(source.bgGradientAngle) || 90) + 'deg, ' + start + ' 0%, ' + end + ' ' + position + '%)';
			css['--pb-grid-column-background'] = css.backgroundImage;
		} else if (source.bgType === 'image') {
			const imageUrl = gridColumnStyleSafeMediaUrl(source.bgImage);
			if (imageUrl) {
				css.backgroundImage = 'url("' + imageUrl + '")';
				css.backgroundSize = source.bgSize === 'stretch' ? '100% 100%' : (source.bgSize || 'cover');
				css.backgroundPosition = source.bgPosition || 'center center';
				css.backgroundRepeat = source.bgRepeat || 'no-repeat';
				css.backgroundAttachment = source.bgAttachment || 'scroll';
				css['--pb-grid-column-background'] = css.backgroundImage;
				css['--pb-grid-column-background-size'] = css.backgroundSize;
				css['--pb-grid-column-background-position'] = css.backgroundPosition;
				css['--pb-grid-column-background-repeat'] = css.backgroundRepeat;
				css['--pb-grid-column-background-attachment'] = css.backgroundAttachment;
			}
		}
	...
	function gridColumnTrackRemovalImpact(node, trackIndex) {
		const settings = node && node.settings && typeof node.settings === 'object' ? node.settings : {};
		const isContainerGrid = isCont(node && node.type)
	...
	function removeGridColumnTrack(node, trackIndex) {
		...
		node.columns = (Array.isArray(node.columns) ? node.columns : [])
			...
			.map((column) => ({
				...
				styleOverrides: normalizeGridColumnStyle(column && column.styleOverrides, true),
			}));
		if (Array.isArray(node.columnStyles)) node.columnStyles.splice(impact.trackIndex, 1);
	...
	function reconcileGridColumns(node, columns, rows, createColumnId) {
		...
		let slots = current.map((rawColumn) => {
			...
			return {
				...
				styleOverrides: normalizeGridColumnStyle(column.styleOverrides, true),
			};
		});
		const targetCount = gridSlotCount(columns, rows);
		while (slots.length < targetCount) slots.push({ id: nextId(''), children: [], styleOverrides: {} });
	...
	function nestedOwnerBlocksAncestorCapture(fromDropzone, targetDropzone, originalEvent) {
		if (!fromDropzone || !targetDropzone || !originalEvent) return false;
		const owner = typeof fromDropzone.closest === 'function'
			? fromDropzone.closest('.pb-node[data-node-type="tabs"], .pb-node[data-node-type="accordion"]')
			: null;
		if (!owner || typeof targetDropzone.contains !== 'function' || !targetDropzone.contains(owner)) return false;
		const point = originalEvent.touches?.[0] || originalEvent.changedTouches?.[0] || originalEvent;
		const x = Number(point.clientX);
		const y = Number(point.clientY);
		if (!Number.isFinite(x) || !Number.isFinite(y) || typeof owner.getBoundingClientRect !== 'function') return false;
	...
	function canonicalLayoutForSave(nodes) {
		...
		const visit = (list) => (Array.isArray(list) ? list : []).map((node) => {
			if (!node || typeof node !== 'object') return node;
				const isContainerGrid = isCont(node.type)
				&& (node.settings?.displayType || 'flex') === 'grid';
				if (isCont(node.type) && !isContainerGrid) delete node.columns;
	...
	function normalizeLegacyContainerSnapshot(nodes, createContainer) {
		...
		const normalize = (list) => (Array.isArray(list) ? list : [])
			...
			.map((rawNode) => {
				...
				const isContainerGrid = isCont(node.type)
					&& (node.settings?.displayType || 'flex') === 'grid';
				if (isCont(node.type) && !isContainerGrid) {
					migrateLegacyContainerColumns(node, migrationState, createContainer);
				}
				if (isContainerGrid || isGrid(node.type)) {
	...
	// V24_CHILD_CONTAINER_HELPERS_END
	// V24_CONTEXTUAL_PROPERTY_HELPERS_START
	function propertyTabsForNodeType(type) {
		const definition = moduleDefinition(type);
		if (definition?.advanced?.profile === 'layout') {
			return [
		...
		if (hasModuleCapability(type, 'content-only-settings')) {
			return [
	...
	// V24_CONTEXTUAL_PROPERTY_HELPERS_END
	...
	function isFullBleedValue(value) {
		const normalized = String(value ?? '').trim().toLowerCase();
		return value === true || value === 1 || normalized === '1' || normalized === 'true' || normalized === 'full';
	}
	function widgetAdvancedDefaults() { return widgetRegistry.advancedDefaults(); }
	function normalizeWidgetAdvancedSettings(settings) { return widgetRegistry.normalizeAdvanced(settings); }
	function widgetAdvancedPreviewStyle(settings, device) {
		...
		const fullBleed = isFullBleedValue(get('fullBleed', false));
		if (fullBleed) {
			style.width = '100%';
			style.maxWidth = '100%';
	...
	function baseNodeLabel(type, fallback = 'Widget') { return moduleDefinition(type)?.label || fallback; }
	...
	const BuilderNode = {
		...
		computed: {
			...
			hasSharedAdvancedControls() { return !!moduleDefinition(this.node.type)?.advanced; },
		hasNewGeneralAdvancedControls() { return this.hasSharedAdvancedControls; },
		...
		labelIcon() {
			...
			nodeAdvancedClasses() {
				...
				if (isFullBleedValue(fullBleed)) classes.push('pb-full-bleed');
				return classes;
			},
			nodeShellStyle() {
				const s = this.node.settings || {};
				const device = this.responsiveDevice || 'desktop';
                if (this.hasNewGeneralAdvancedControls) return this.compiledInlineWidgetShellStyle(widgetAdvancedPreviewStyle(s, device));
                if (this.hasSharedAdvancedControls) return this.compiledInlineWidgetShellStyle(widgetAdvancedPreviewStyle(s, device));
			...
			widgetPreviewShellStyle() {
				...
				const fullBleed = this.nodeResponsiveValue('fullBleed', false);
				const enabled = isFullBleedValue(fullBleed);
			...
			contGroup() {
				...
				return {
					...
					put(to, from, element) {
						...
						.some((child) => isCont(child?.type));
					const isLayoutNode = isCont(draggedType) || isGrid(draggedType)
		...
		methods: {
			gridColumnTrackIndex(cellIndex) {
				return Math.max(0, Number(cellIndex) || 0) % gridColumnCountForNode(this.node);
			},
			gridColumnCellStyle(column, cellIndex) {
				const trackIndex = this.gridColumnTrackIndex(cellIndex);
				const trackStyle = Array.isArray(this.node.columnStyles) ? this.node.columnStyles[trackIndex] : null;
				return gridColumnStyleToCss(resolveGridColumnStyle(trackStyle, column && column.styleOverrides, this.responsiveDevice || 'desktop'));
			},
			gridColumnHasCustomBorder(column, cellIndex) {
				const trackIndex = this.gridColumnTrackIndex(cellIndex);
				const trackStyle = Array.isArray(this.node.columnStyles) ? this.node.columnStyles[trackIndex] : null;
				return resolveGridColumnStyle(trackStyle, column && column.styleOverrides, this.responsiveDevice || 'desktop').borderType !== 'none';
			},
			gridColumnHasCustomBackground(column, cellIndex) {
				const trackIndex = this.gridColumnTrackIndex(cellIndex);
				const trackStyle = Array.isArray(this.node.columnStyles) ? this.node.columnStyles[trackIndex] : null;
				return resolveGridColumnStyle(trackStyle, column && column.styleOverrides, this.responsiveDevice || 'desktop').bgType !== 'none';
			},
			...
			contGridNodeStyle(col, childNode = null) {
				...
				if (isWidgetChild) {
					...
					const stretchContent = justifyItems === 'stretch' || !hasModuleCapability(childNode.type, 'shrinkable-inline');
					style.width = '100%';
					style.marginLeft = '0';
					style.marginRight = '0';
					style.alignSelf = 'stretch';
			...
			compiledInlineWidgetShellStyle(style) {
				const output = style && typeof style === 'object' ? { ...style } : {};
				const marker = String(this.node?.settings?.importNodeKey || '').trim();
				if (!/^import-node-[A-Za-z0-9_-]+$/.test(marker)) return output;
				const parent = this.parentNode;
				if (!parent || !isCont(parent.type)) return output;
				const parentSettings = parent.settings || {};
				const direction = String(
					parentSettings[responsiveKey('direction', this.responsiveDevice || 'desktop')]
						|| parentSettings.direction
						|| 'column'
				).toLowerCase();
				if (!['row', 'row-reverse'].includes(direction)) return output;
				output.flex = '0 1 auto';
			...
			hasSingleCompactChild(children) {
				return Array.isArray(children)
					&& children.length === 1
					&& hasModuleCapability(children[0]?.type, 'compact-single-column');
			},
...
>
		...
		<template v-if="isCont">
			<component :is="loadWidget(node.type)" :item="node" :responsive-device="responsiveDevice" :fill-editor-shell="!!parentNode">
				<div v-if="(node.settings?.displayType || 'flex') === 'grid'" class="el-cont-columns" :style="contColumnsStyle">
					<div v-for="(col, ci) in node.columns" :key="col.id" class="el-grid-col pb-grid-col" :style="gridColumnCellStyle(col, ci)" :data-col-index="ci" :data-pb-grid-track="gridColumnTrackIndex(ci)" :data-pb-grid-cell="col.id" :data-pb-grid-custom-border="gridColumnHasCustomBorder(col, ci) ? 'true' : null" :data-pb-grid-custom-background="gridColumnHasCustomBackground(col, ci) ? 'true' : null" @contextmenu.stop.prevent="onContextMenu($event, { kind: 'gridColumn', node: node, column: col, columnIndex: ci })">
						<draggable
							...
							:class="['pb-dropzone', 'pb-dropzone-col', {
								...
								'has-single-heading': hasSingleCompactChild(col.children),
				...
				>
					...
					<template #footer>
						<div v-if="!node.children || node.children.length === 0" class="pb-dropzone-empty pb-container-empty-hint">
							<button type="button" class="pb-inline-add" data-pb-interactive="true" @click.stop.prevent="onShowToolbox({ type: 'children', nodeId: node.id })">
								<i class="fas fa-plus"></i><span>Add</span>
							</button>
						...
						>
							...
							<template #footer>
								<div v-if="activeTabsChildren().length === 0" class="pb-dropzone-empty pb-tabs-empty-hint">
									<button type="button" class="pb-inline-add" data-pb-interactive="true" @click.stop.prevent="onOpenModal(defaultContainerType(), 'tabs', activeTabsChildren())">
						...
						<template #panel="{ item }">
							<draggable
								...
								:class="['pb-dropzone', 'pb-dropzone-accordion', {
									...
									'is-pending-insert-target': pendingInsertTarget && pendingInsertTarget.type === 'nested-accordion' && pendingInsertTarget.nodeId === node.id && pendingInsertTarget.itemId === item.id
								}]"
								ghost-class="pb-ghost"
								dragover-class="is-drop-hover"
							...
							>
								...
								<template #footer>
									...
									<button type="button" class="pb-inline-add" data-pb-interactive="true" @click.stop.prevent="onShowToolbox({ type: 'nested-accordion', nodeId: node.id, itemId: item.id })">
											<i class="fas fa-plus"></i>
											<span>Add</span>
										</button>
										<div class="pb-dropzone-empty-text">Drop here</div>
	...
	// ── App ────────────────────────────────────────────────────────────────────
	// Module-specific defaults and normalizers are owned by their module definitions.

	const PBC = window.PAGE_BUILDER_ELEMENTOR_V24_CONTEXT || {};
	...
	createApp({
		...
		setup() {
			...
			const customJsModes = Object.freeze({
				disabled: 'Disabled',
				exact_sandbox: 'Sandbox only',
				published: 'Published frontend',
			});
			const hasInitialCustomJs = String(pd?.custom_js || '').trim() !== '';
			const initialCustomJsMode = hasInitialCustomJs && ['disabled', 'exact_sandbox', 'published'].includes(pd?.custom_js_mode)
				? pd.custom_js_mode
				: 'disabled';
			...
			const staticImportSourceFile = ref(null);
			...
			const staticImportBaseUrl = ref('');
			const staticImportMode = ref('exact');
			const staticImportCompileController = ref(null);
			const staticImportCompileStage = ref('idle');
			const staticImportCompileProgress = ref(0);
			const staticImportCompileMessage = ref('');
			const staticImportCompileError = ref('');
			const staticImportCompileFallback = ref(null);
			const staticImportCompileResult = ref(null);
			const staticImportCompileDialog = ref(null);
			const showStaticImportCompileModal = ref(false);
			const staticImportGeneratedCssStats = ref(null);
			const showStaticImportReportModal = ref(false);
			const staticImportCopyConfirmation = ref(false);
			const staticImportCopyAcknowledged = ref(false);
			const staticImportCopyBusy = ref(false);
			const guidedStaticImportApi = window.PhoenixGuidedStaticImport || {};
			const guidedStaticImportState = ref(typeof guidedStaticImportApi.createState === 'function'
				? guidedStaticImportApi.createState()
				: { phase: 'idle', sourceFile: null, sourceHash: '', analysis: null, selectedRegionId: '', expandedBlockIds: [], regionMappings: {}, errors: [] });
			const showGuidedStaticImportModal = ref(false);
			const guidedStaticImportDialog = ref(null);
			const staticImportTrigger = ref(null);
			...
			const customCssSummary = computed(() => {
				...
				if (String(customCss.value || '').includes('PHOENIX_STATIC_IMPORT_COMPILED_START')) {
					const stats = staticImportGeneratedCssStats.value || {};
					const generatedChars = Number(stats.generatedChars || 0) || customCssCharCount.value;
					return 'Generated CSS · ' + generatedChars + ' chars';
				}
				return customCssCharCount.value + ' chars / ' + customCssLineCount.value + ' lines';
			});
			const customCssHasGeneratedBlock = computed(() => String(customCss.value || '').includes('PHOENIX_STATIC_IMPORT_COMPILED_START'));
			...
			function clearCustomCss() {
				customCss.value = '';
				staticImportGeneratedCssStats.value = null;
				customCssActiveLine.value = 0;
				nextTick(() => {
					if (customCssEditorTextarea.value && typeof customCssEditorTextarea.value.focus === 'function') {
						customCssEditorTextarea.value.focus();
			...
			const selectedGridColumnStyleTarget = ref(null);
			...
			const rootNodes   = ref([]);
			const canvasCustomCss = computed(() => {
				const metadata = staticImportMetadata(rootNodes.value);
				if (metadata?.mode !== 'compiled') return customCss.value;
				const adapter = window.PhoenixStaticImportCompiler?.adaptCompiledCssForCanvas;
				if (typeof adapter !== 'function') return customCss.value;
				const width = responsiveDevice.value === 'tablet'
					? 768
					: (responsiveDevice.value === 'mobile' ? 390 : Number(desktopPreviewWidth.value));
				return adapter(customCss.value, width);
			});
			...
			const fontFamilies = Array.isArray(window.PB_ELEMENTOR_V24_FONT_FAMILIES)
				? window.PB_ELEMENTOR_V24_FONT_FAMILIES
				: [];
			...
			function scheduleHistorySnapshotFromEvent(event) {
				if (traveling.value || suppressHistory.value) return;
				const appRoot = document.getElementById('pbElementorV24App');
				const target = event && event.target;
			...
			const initialLegacySnapshot = normalizeLegacyContainerSnapshot(parsedLayout, () => makeNode(defaultContainerType()));
			...
			function norm(nodes, legacyMigrationState = createLegacyContainerMigrationState(nodes)) {
				return (nodes || []).map(n => {
					...
					if (isCont(c.type)) {
						if (hasModuleCapability(c.type, 'fluid-content')) c.settings.contentWidth = 'fluid';
			...
			if (isAccordion(c.type)) {
					...
					if (isCont(c.type) && (c.settings?.displayType || 'flex') !== 'grid') migrateLegacyContainerColumns(c, legacyMigrationState, () => makeNode(defaultContainerType()));
			...
			syncStaticImportDependencies(rootNodes.value);
			...
			async function pasteExternalContextClipboard(meta, node) {
				...
				if (parsed.reason === 'unsupported') {
					showSaveToast('info', 'Format clipboard tidak kompatibel. Saat ini hanya payload Phoenix Page Builder v2.4 yang didukung.');
					return;
				}
				...
				if (!parsed.nodes.length) {
					showSaveToast('info', 'Clipboard tidak berisi element Phoenix v2.4 yang valid.');
					return;
				}
				const imported = parsed.nodes.map(normalizeExternalContextNode);
				if (imported.some((item) => !item)) {
					showSaveToast('info', 'Clipboard berisi widget atau layout yang belum didukung v2.4.');
					return;
				}
				const destination = contextPasteDestination(meta, node);
				if (destination?.blocked) {
			...
			function addContextContainer(node) {
				...
				showToolboxPanel({ type: 'children', nodeId: node.id });
			}
			function openContextStructure(node) {
				if (isGrid(node?.type)) {
				openModal(defaultGridType(), 'root', { containerNode: node, list: [] });
					return;
				}
				if (isCont(node?.type)) {
			...
			function runContextMenuAction(item) {
				...
				switch (item.id) {
					...
					case 'resetStyle':
						...
						break;
					case 'saveGlobal':
						showUnsupportedControlNotice('Save as global', 'Global widget belum memiliki persistence/backend di v2.4.');
						break;
					case 'saveTemplate':
						showUnsupportedControlNotice('Save as Template', 'Template belum memiliki persistence/backend di v2.4.');
						break;
					case 'structure':
			...
			function canMoveCanvasNode(event, originalEvent) {
				if (isFormLayoutDrag(event?.from) || isFormLayoutDrag(event?.to) || isFormLayoutDrag(event?.dragged)) return false;
				if (nestedOwnerBlocksAncestorCapture(event?.from, event?.to, originalEvent)) return false;
			...
			const selectedSupportsRichTextModal = computed(() => hasModuleCapability(selectedType.value, 'rich-text-modal'));
			...
			const textEditorModalSummary = computed(() => {
				const html = selectedSupportsRichTextModal.value ? String(selectedNode.value?.settings?.html || '') : '';
				const text = html.replace(/<[^>]*>/g, '').replace(/\s+/g, ' ').trim();
				if (!text.length) return 'Empty';
				return text.length + ' chars';
			});
		function setTextEditorHtml(value) {
		if (!selectedSupportsRichTextModal.value || !selectedNode.value) return;
			...
			function openTextEditorModal() {
				if (!selectedSupportsRichTextModal.value) return;
			...
			watch(selectedType, (type) => {
				if (!hasModuleCapability(type, 'rich-text-modal') && showTextEditorModal.value) closeTextEditorModal();
				if (!hasModuleCapability(type, 'icon-source')) iconLinkOptionsOpenFor.value = '';
			});
			function tabsItemsForNode(node = selectedNode.value) {
				if (!node || !isTabs(node.type)) return [];
			...
			function addContainerChild(node) {
				...
				const child = appendChildContainer(node, () => makeNode(defaultContainerType()));
			...
			function insertToolIntoPendingTarget(toolDef, target = pendingInsertTarget.value) {
				...
				if (target.type === 'tab') {
					...
					if (!tabsNode || !isTabs(tabsNode.type) || !Array.isArray(tabsNode.tabItems)) return false;
				...
				if (target.type === 'nested-accordion') {
					const accordionNode = findById(rootNodes.value, target.nodeId);
					if (!accordionNode || !isAccordion(accordionNode.type) || !Array.isArray(accordionNode.accordionItems)) return false;
			...
			function insertToolAtRoot(toolDef) {
				...
				if (!item) return false;
				if (isWgt(item.type)) {
					const container = makeNode(defaultContainerType());
			...
			function accordionItemsForNode(node = selectedNode.value) {
				if (!node || !isAccordion(node.type)) return [];
			...
			function iconListItemsForNode(node = selectedNode.value) {
				if (!node || !hasModuleCapability(node.type, 'repeater-icon-list')) return [];
			...
			function toggleIconLinkOptions(node = selectedNode.value) {
				if (!node || !hasModuleCapability(node.type, 'icon-link-options')) return;
		...
		function isIconLibraryWidgetNode(node) {
            return !!node && hasModuleCapability(node.type, 'primary-icon');
		}
		function normalizeIconLibraryWidgetSettings(node) {
			if (!isIconLibraryWidgetNode(node)) return;
			moduleDefinition(node.type)?.normalize?.(node);
			}
			...
			function resolveProIconTarget(targetKey, itemId, node = selectedNode.value) {
				if (!node || !hasModuleCapability(node.type, 'pro-icon-targets')) return null;
				const target = moduleDefinition(node.type)?.editor?.iconTargets?.[String(targetKey || '')];
				if (!target) return null;
				let entry = node.settings || (node.settings = {});
				if (target.collection) {
					const items = String(target.collection).split('.').reduce(
			...
			async function openSocialIconLibrary(itemId, node = selectedNode.value) {
				if (!node || !hasModuleCapability(node.type, 'social-icon-items')) return;
			...
			async function openAlertIconLibrary(node = selectedNode.value) {
				if (!node || !hasModuleCapability(node.type, 'dismiss-icon')) return;
				await ensureIconLibraryLoaded();
			...
			async function openIconListItemIconLibrary(itemId, node = selectedNode.value) {
				if (!node || !hasModuleCapability(node.type, 'icon-list-items')) return;
			...
			async function openTabsItemIconLibrary(itemId, settingKey, node = selectedNode.value) {
				if (!node || !isTabs(node.type) || settingKey !== 'iconClass') return;
			...
			async function openAccordionIconLibrary(role, node = selectedNode.value) {
				if (!node || !isAccordion(node.type) || !['expand', 'collapse'].includes(role)) return;
				await ensureIconLibraryLoaded();
			...
			async function openImageCarouselArrowIconLibrary(settingKey, node = selectedNode.value) {
				if (!node || !hasModuleCapability(node.type, 'carousel-arrow-icons') || !['previousArrowIcon', 'nextArrowIcon'].includes(settingKey)) return;
				await ensureIconLibraryLoaded();
				const parsed = parseIconWidgetClassParts(node.settings?.[settingKey]);
				iconLibraryTargetNodeId.value = String(node.id || '');
				iconLibraryTargetSettingKey.value = settingKey;
			...
			function insertSelectedIcon() {
				...
				if (node && hasModuleCapability(node.type, 'icon-list-items') && settingKey === 'iconListItem' && itemId) {
					normalizeIconListSettings(node.settings || (node.settings = {}));
				...
				if (node && isTabs(node.type) && settingKey === 'iconClass' && itemId) {
				...
				if (node && hasModuleCapability(node.type, 'social-icon-items') && settingKey === 'socialIconItem' && itemId) {
				...
				if (node && hasModuleCapability(node.type, 'dismiss-icon') && settingKey === 'alertDismissIcon') {
					if (!node.settings || typeof node.settings !== 'object') node.settings = {};
				...
				if (node && isAccordion(node.type) && ['expandIconClass', 'collapseIconClass'].includes(settingKey)) {
					if (!node.settings || typeof node.settings !== 'object') node.settings = {};
				...
				if (node && hasModuleCapability(node.type, 'carousel-arrow-icons') && ['previousArrowIcon', 'nextArrowIcon'].includes(settingKey)) {
					if (!node.settings || typeof node.settings !== 'object') node.settings = {};
					node.settings[settingKey] = iconLibrarySelected.value.className;
			...
			function chooseRatingSvg(node = selectedNode.value) {
				if (!node || !hasModuleCapability(node.type, 'rating-icon')) return;
			...
			function chooseButtonSvg(node = selectedNode.value) {
				if (!node || !hasModuleCapability(node.type, 'primary-icon')) return;
				const markup = window.prompt('Paste trusted SVG markup', String(node.settings?.iconSvg || ''));
				if (markup === null) return;
				const sanitized = sanitizeAccordionSvgMarkup(markup);
				if (!sanitized) {
			...
			function normalizeVideoNodeSettings(node) {
				if (!node?.settings || typeof node.settings !== 'object') return;
				const definition = moduleDefinition(node.type);
				if (typeof definition?.normalize !== 'function') return;
				const settings = node.settings;
				const normalized = definition.normalize({ ...node, settings: { ...settings } });
			...
			function setVideoSourceType(node, value) {
				...
				normalizeVideoNodeSettings(node);
			}
			...
			function ensureNodeGridColumns(node) {
				if (!node || !node.settings) return [];
				if (isCont(node.type) && (node.settings.displayType || 'flex') === 'grid') {
					const slots = reconcileGridColumns(node, node.settings.gridColumns || 3, node.settings.gridRows || 1, () => uid('c'));
					reconcileGridColumnStyles(node, node.settings.gridColumns || 3);
					return slots;
				}
				if (isGrid(node.type)) {
					const slots = reconcileGridColumns(node, node.settings.columns || 3, node.settings.gridRows || 1, () => uid('c'));
					reconcileGridColumnStyles(node, node.settings.columns || 3);
					return slots;
				}
				return [];
			}
			...
			watch(selectedId, (nextId) => {
				...
				if (!nextId || selectedGridColumnStyleTarget.value?.nodeId !== String(nextId)) clearGridColumnStyleTarget();
				nextTick(resetPropertiesPanelScroll);
				closeControlResponsiveMenu();
				closeWidthPreviewMenu();
				scheduleColorisInit();
			...
			function gridColumnTracks(node) {
				...
				return Array.from({ length: impact.columnCount }, (_, index) => {
					...
					return {
						...
						hasOverride: gridColumnTrackCells(node, index).some((cell) => cell.hasOverride),
					};
				});
			}
			function gridColumnTrackCells(node, trackIndex) {
				const columnCount = gridColumnCountForNode(node);
				const settings = node && node.settings && typeof node.settings === 'object' ? node.settings : {};
				const rawRows = String(settings.gridRows == null ? '' : settings.gridRows).trim().match(/^(\d+)/);
				const rowCount = Math.min(12, Math.max(1, Math.floor(Number(rawRows && rawRows[1]) || 1)));
				const safeTrackIndex = Math.min(columnCount - 1, Math.max(0, Math.floor(Number(trackIndex) || 0)));
				const columns = Array.isArray(node && node.columns) ? node.columns : [];

				return Array.from({ length: rowCount }, (_, rowIndex) => {
					const cellIndex = (rowIndex * columnCount) + safeTrackIndex;
					const cell = columns[cellIndex] && typeof columns[cellIndex] === 'object' ? columns[cellIndex] : { id: '', children: [] };
					const overrides = cell.styleOverrides && typeof cell.styleOverrides === 'object' ? cell.styleOverrides : {};
					return {
						rowIndex,
						cellIndex,
						label: 'Row ' + (rowIndex + 1),
						itemCount: Array.isArray(cell.children) ? cell.children.length : 0,
						hasOverride: Object.keys(overrides).length > 0,
						cell,
					};
				});
			}
			function clearGridColumnStyleTarget() {
				selectedGridColumnStyleTarget.value = null;
			}
			function selectGridColumnStyleTarget(node, trackIndex, cellIndex = -1) {
				if (!node || !node.id) return;
				const columnCount = gridColumnCountForNode(node);
				const safeTrackIndex = Math.min(columnCount - 1, Math.max(0, Math.floor(Number(trackIndex) || 0)));
				const safeCellIndex = Number.isFinite(Number(cellIndex)) && Number(cellIndex) >= 0 ? Math.floor(Number(cellIndex)) : -1;
				selectedGridColumnStyleTarget.value = { nodeId: String(node.id), trackIndex: safeTrackIndex, cellIndex: safeCellIndex };
				settingsTab.value = 'style';
				nextTick(() => {
					resetPropertiesPanelScroll();
					scheduleColorisInit();
				});
			}
			function gridColumnStyleTargetFor(node) {
				const target = selectedGridColumnStyleTarget.value;
				if (!node || !target || String(target.nodeId) !== String(node.id)) return null;
				return target;
			}
			function gridColumnStyleTargetLabel(node) {
				const target = gridColumnStyleTargetFor(node);
				if (!target) return '';
				const columnLabel = 'Column ' + (target.trackIndex + 1);
				return target.cellIndex >= 0 ? 'Cell Override — ' + columnLabel + ' / Row ' + (Math.floor(target.cellIndex / gridColumnCountForNode(node)) + 1) : 'Column Style — ' + columnLabel;
			}
			function gridColumnStyleTargetIsCell(node) {
				return !!(gridColumnStyleTargetFor(node) && gridColumnStyleTargetFor(node).cellIndex >= 0);
			}
			function gridColumnStyleTargetSettings(node) {
				const target = gridColumnStyleTargetFor(node);
				if (!target) return null;
				reconcileGridColumnStyles(node, gridColumnCountForNode(node));
				if (target.cellIndex >= 0) {
					const cell = Array.isArray(node.columns) ? node.columns[target.cellIndex] : null;
					if (!cell) return null;
					if (!cell.styleOverrides || typeof cell.styleOverrides !== 'object') cell.styleOverrides = {};
					return cell.styleOverrides;
				}
				return node.columnStyles[target.trackIndex] || null;
			}
			function gridColumnStyleValue(node, key) {
				const target = gridColumnStyleTargetFor(node);
				if (!target) return '';
				const track = Array.isArray(node.columnStyles) ? node.columnStyles[target.trackIndex] : null;
				const cell = target.cellIndex >= 0 && Array.isArray(node.columns) ? node.columns[target.cellIndex]?.styleOverrides : null;
				return resolveGridColumnStyle(track, cell, responsiveDevice.value)[key];
			}
			function setGridColumnStyleValue(node, key, value) {
				const targetSettings = gridColumnStyleTargetSettings(node);
				if (!targetSettings) return;
				const activeKey = key.endsWith('Linked') ? key : responsiveKey(key, normalizeResponsiveDevice(responsiveDevice.value));
				targetSettings[activeKey] = value;
				scheduleSnap();
			}
			function resetSelectedGridColumnStyle(node) {
				const target = gridColumnStyleTargetFor(node);
				if (!target || target.cellIndex < 0) return;
				const cell = Array.isArray(node.columns) ? node.columns[target.cellIndex] : null;
				resetGridColumnStyleOverrides(cell);
				scheduleSnap();
			}
			function chooseGridColumnBgImage(node) {
				const targetSettings = gridColumnStyleTargetSettings(node);
				if (!targetSettings) return;
				chooseMedia(targetSettings, responsiveKey('bgImage', normalizeResponsiveDevice(responsiveDevice.value)), 'Paste image URL');
			}
			function clearGridColumnBgImage(node) {
				const targetSettings = gridColumnStyleTargetSettings(node);
				if (!targetSettings) return;
				targetSettings[responsiveKey('bgImage', normalizeResponsiveDevice(responsiveDevice.value))] = '';
				scheduleSnap();
			}
			...
			function regenIds(node) {
				...
				if (isTabs(node.type) && node.settings) {
			...
			function onRootAdd(evt) {
				...
				if (isWgt(item.type)) {
					...
					const c = makeNode(defaultContainerType());
			...
			function applyContPreset(p) {
					...
					function configureContainer(c, isGridLayout) {
						...
						if (isGridLayout || layoutType === 'container_grid') {
							...
							c.children = presetChildContainers(() => makeNode(defaultContainerType()), p);
							delete c.columns;
						}
						seedResponsiveSettings(c.settings, true);
						return c;
			...
			async function submitFormDraft(node, form) {
				...
				if (!endpoint || !node || !hasModuleCapability(node.type, 'draft-form') || !form) {
			...
			const guidedStaticImportActiveRegion = computed(() => {
				const regions = guidedStaticImportState.value.analysis?.regions || [];
				return regions.find((region) => region.id === guidedStaticImportState.value.selectedRegionId) || regions[0] || null;
			});
			const guidedStaticImportPhaseLabel = computed(() => ({
				analyzing: 'Analyzing regions',
				mapping: 'Review recommended mapping',
				compiling: 'Compiling selected regions',
				complete: 'Completed',
				failed: 'Compilation failed',
			}[guidedStaticImportState.value.phase] || 'Preparing import'));
			const guidedStaticImportPreviewSrcdoc = computed(() => {
				const region = guidedStaticImportActiveRegion.value;
				return region && typeof guidedStaticImportApi.buildRegionPreviewSrcdoc === 'function'
					? guidedStaticImportApi.buildRegionPreviewSrcdoc(guidedStaticImportState.value.analysis?.previewPayload || {}, region.marker)
					: '';
			});
			const guidedStaticImportRegionValidation = computed(() => {
				const region = guidedStaticImportActiveRegion.value;
				return region && typeof guidedStaticImportApi.regionValidation === 'function'
					? guidedStaticImportApi.regionValidation(guidedStaticImportState.value, region.id)
					: { valid: false, errors: [] };
			});
			const guidedStaticImportCanCompile = computed(() => {
				const regions = guidedStaticImportState.value.analysis?.regions || [];
				if (regions.length === 0 || guidedStaticImportState.value.phase === 'analyzing' || guidedStaticImportState.value.phase === 'compiling' || guidedStaticImportState.value.errors?.length) return false;
				return typeof guidedStaticImportApi.regionValidation === 'function'
					&& regions.every((region) => guidedStaticImportApi.regionValidation(guidedStaticImportState.value, region.id).valid);
			});
			const guidedStaticImportCanReanalyze = computed(() => {
				const retryable = ['source-hash-mismatch', 'inactive-widget', 'source-stale', 'analyze-request-failed', 'compile-request-failed'];
				return (guidedStaticImportState.value.errors || []).some((error) => retryable.includes(error?.code));
			});
			const staticImportReportMapping = computed(() => staticImportReport.value?.mappingReport || {});
			const staticImportReportDetectedRegions = computed(() => Number(staticImportReport.value?.detectedRegions || staticImportReportMapping.value.regions || staticImportPayload.value?.regions?.length || staticImportPayload.value?.compilePayload?.regions?.length || 0));
			const staticImportReportNativeRegions = computed(() => Number(staticImportReport.value?.nativeRegions || staticImportReportMapping.value.nativeRegions || 0));
			const staticImportReportExactRegions = computed(() => Number(staticImportReport.value?.exactRegions || staticImportReportMapping.value.exactRegions || 0));
			const staticImportReportSkippedRegions = computed(() => Number(staticImportReport.value?.skippedRegions || staticImportReportMapping.value.skippedRegions || 0));
			const staticImportReportMappedBlocks = computed(() => Number(staticImportReport.value?.mappedBlocks || staticImportReportMapping.value.mappedBlocks || 0));
			const staticImportReportUnmappedBlocks = computed(() => Number(staticImportReport.value?.unmappedBlocks || staticImportReportMapping.value.unmappedBlocks || 0));
			const staticImportReportRelativeAssets = computed(() => Array.isArray(staticImportReport.value?.relativeAssets) ? staticImportReport.value.relativeAssets.length : 0);
			const staticImportReportSourceScripts = computed(() => Number(staticImportReport.value?.sourceScripts?.total || 0));
			function openGuidedStaticImportModal() {
				showGuidedStaticImportModal.value = true;
				nextTick(() => guidedStaticImportDialog.value?.focus?.());
			}
			function beginGuidedStaticImportAnalysis(file) {
				if (typeof guidedStaticImportApi.createState !== 'function') return;
				guidedStaticImportState.value = { ...guidedStaticImportApi.createState(), phase: 'analyzing', sourceFile: file };
				openGuidedStaticImportModal();
			}
			function guidedStaticImportSelectRegion(regionId) {
				if (!regionId) return;
				guidedStaticImportState.value = { ...guidedStaticImportState.value, selectedRegionId: regionId, errors: [] };
			}
			function guidedStaticImportRegionStrategy(region) {
				if (!region) return 'guided_native';
				return guidedStaticImportState.value.regionMappings?.[region.id]?.strategy || region.recommendedStrategy || 'guided_native';
			}
			function guidedStaticImportSetStrategy(regionId, strategy) {
				if (typeof guidedStaticImportApi.setRegionStrategy !== 'function') return;
				const next = guidedStaticImportApi.setRegionStrategy(guidedStaticImportState.value, regionId, strategy);
				guidedStaticImportState.value = { ...next, errors: [] };
			}
			function guidedStaticImportSetBlockWidget(regionId, blockId, widgetType) {
				if (typeof guidedStaticImportApi.setBlockWidget !== 'function') return;
				const next = guidedStaticImportApi.setBlockWidget(guidedStaticImportState.value, regionId, blockId, widgetType);
				guidedStaticImportState.value = { ...next, errors: [] };
			}
			function guidedStaticImportBlockWidget(regionId, block) {
				return guidedStaticImportState.value.regionMappings?.[regionId]?.blocks?.[block.id] || block.recommendedWidget || '';
			}
			function guidedStaticImportBlockHasChildren(block) {
				return Array.isArray(block?.children) && block.children.length > 0;
			}
			function guidedStaticImportBlockExpanded(blockId) {
				return guidedStaticImportState.value.expandedBlockIds.includes(blockId);
			}
			function toggleGuidedStaticImportBlock(blockId) {
				const expanded = new Set(guidedStaticImportState.value.expandedBlockIds);
				if (expanded.has(blockId)) expanded.delete(blockId);
				else expanded.add(blockId);
				guidedStaticImportState.value = { ...guidedStaticImportState.value, expandedBlockIds: [...expanded] };
			}
			function guidedStaticImportVisibleBlocks(region) {
				if (!region || typeof guidedStaticImportApi.flattenBlocks !== 'function') return [];
				const expanded = new Set(guidedStaticImportState.value.expandedBlockIds);
				const allBlocks = guidedStaticImportApi.flattenBlocks(region.blocks || []);
				return allBlocks.map((block) => {
					let parentId = block.parentId;
					let depth = 0;
					let visible = true;
					while (parentId) {
						if (!expanded.has(parentId)) visible = false;
						const parent = allBlocks.find((item) => item.id === parentId);
						parentId = parent?.parentId || null;
						depth++;
					}
					return { block, depth, visible };
				}).filter((item) => item.visible).map((item) => ({ ...item.block, depth: item.depth }));
			}
			function guidedStaticImportGoBack() {
				const regions = guidedStaticImportState.value.analysis?.regions || [];
				const index = regions.findIndex((region) => region.id === guidedStaticImportState.value.selectedRegionId);
				if (index > 0) guidedStaticImportSelectRegion(regions[index - 1].id);
			}
			function guidedStaticImportCancel() {
				if (staticImportCompileController.value && typeof staticImportCompileController.value.abort === 'function') staticImportCompileController.value.abort();
				guidedStaticImportState.value = typeof guidedStaticImportApi.resetState === 'function'
					? guidedStaticImportApi.resetState(guidedStaticImportState.value)
					: { phase: 'idle', sourceFile: null, sourceHash: '', analysis: null, selectedRegionId: '', expandedBlockIds: [], regionMappings: {}, errors: [] };
				showGuidedStaticImportModal.value = false;
				staticImportBusy.value = false;
				nextTick(() => staticImportTrigger.value?.focus?.());
			}
			function guidedStaticImportSafeErrors(payload, fallbackCode = 'analyze-request-failed') {
				const source = Array.isArray(payload?.errors) && payload.errors.length ? payload.errors : [{ code: fallbackCode }];
				return source.map((item) => ({
					code: String(item?.code || fallbackCode).replace(/[^a-z0-9-]/gi, '').slice(0, 80) || fallbackCode,
					...(item?.regionId && /^region-[A-Za-z0-9_-]+$/.test(String(item.regionId)) ? { regionId: String(item.regionId) } : {}),
					...(item?.blockId && /^block-[A-Za-z0-9_-]+$/.test(String(item.blockId)) ? { blockId: String(item.blockId) } : {}),
				}));
			}
			function guidedStaticImportErrorMessage(error) {
				const messages = {
					'source-hash-mismatch': 'Source berubah sejak Analyze. Jalankan Re-analyze source.',
					'inactive-widget': 'Widget tidak lagi aktif pada catalog. Jalankan Re-analyze source.',
					'source-stale': 'File sumber sudah berubah atau tidak tersedia. Jalankan Re-analyze source.',
					'analyze-request-failed': 'Source tidak dapat dianalisis. Coba Re-analyze source.',
					'compile-request-failed': 'Mapping belum berhasil dikompilasi. Periksa pilihan widget.',
					'unknown-region': 'Region tidak ditemukan pada source terbaru.',
					'unknown-block': 'Block tidak ditemukan pada region pemiliknya.',
					'incompatible-widget': 'Widget tidak kompatibel dengan role block ini.',
					'unresolved-block': 'Masih ada block native yang belum dipetakan.',
					'browser-compiler-failed': 'Browser compiler gagal; mapping tetap dipertahankan.',
				};
				return messages[error?.code] || 'Import membutuhkan perhatian.';
			}
			async function guidedStaticImportReanalyze() {
				const file = staticImportSourceFile.value || guidedStaticImportState.value.sourceFile;
				if (!file || !PBC.staticImportUrl || staticImportBusy.value) return;
				const baseState = typeof guidedStaticImportApi.createState === 'function' ? guidedStaticImportApi.createState() : { phase: 'idle', sourceFile: null, sourceHash: '', analysis: null, selectedRegionId: '', expandedBlockIds: [], regionMappings: {}, errors: [] };
				guidedStaticImportState.value = { ...baseState, phase: 'analyzing', sourceFile: file };
				showGuidedStaticImportModal.value = true;
				staticImportBusy.value = true;
				const controller = typeof AbortController === 'function' ? new AbortController() : null;
				staticImportCompileController.value = controller;
				try {
					const data = new FormData();
					data.append('source', file);
					data.append('phase', 'analyze');
					data.append('framework', staticImportFramework.value);
					data.append('mode', 'compiled');
					if (staticImportBaseUrl.value.trim()) data.append('baseUrl', staticImportBaseUrl.value.trim());
					const response = await axios.post(PBC.staticImportUrl, data, {
						signal: controller?.signal,
						headers: { 'X-CSRF-TOKEN': PBC.csrfToken, 'X-Requested-With': 'XMLHttpRequest', Accept: 'application/json' },
					});
					const result = response.data || {};
					if (!Array.isArray(result.regions) || !result.sourceHash || !result.previewPayload) throw new Error('Analyze response does not contain a complete region payload.');
					guidedStaticImportState.value = typeof guidedStaticImportApi.setAnalysis === 'function'
						? guidedStaticImportApi.setAnalysis(guidedStaticImportState.value, result, window.PB_ELEMENTOR_V24_MODULE_CATALOG || {}, file)
						: { ...guidedStaticImportState.value, phase: 'mapping', sourceFile: file, sourceHash: String(result.sourceHash), analysis: result };
					staticImportReport.value = { ...(result.report || {}), detectedRegions: result.regions.length };
					staticImportPayload.value = null;
					showSaveToast('info', 'Source berhasil dianalisis ulang. Mapping lama sudah di-reset.');
				} catch (error) {
					if (error?.name === 'AbortError' || controller?.signal?.aborted) {
						guidedStaticImportState.value = typeof guidedStaticImportApi.resetState === 'function' ? guidedStaticImportApi.resetState(guidedStaticImportState.value) : guidedStaticImportState.value;
						showGuidedStaticImportModal.value = false;
					} else {
						guidedStaticImportState.value = typeof guidedStaticImportApi.setPhase === 'function'
							? guidedStaticImportApi.setPhase(guidedStaticImportState.value, 'failed', { errors: guidedStaticImportSafeErrors(error.response?.data, 'analyze-request-failed') })
							: { ...guidedStaticImportState.value, phase: 'failed', errors: guidedStaticImportSafeErrors(error.response?.data, 'analyze-request-failed') };
						showGuidedStaticImportModal.value = true;
						nextTick(() => guidedStaticImportDialog.value?.focus?.());
						showSaveToast('failed', 'Source tidak dapat dianalisis ulang.');
					}
				} finally {
					staticImportCompileController.value = null;
					staticImportBusy.value = false;
				}
			}
			async function guidedStaticImportCompileSelected() {
				if (!guidedStaticImportCanCompile.value) return;
				const current = guidedStaticImportState.value;
				const file = current.sourceFile || staticImportSourceFile.value;
				const sourceHash = String(current.sourceHash || '');
				if (!file || (typeof guidedStaticImportApi.sourceIsCurrent === 'function' && !guidedStaticImportApi.sourceIsCurrent(current, file, sourceHash))) {
					guidedStaticImportState.value = typeof guidedStaticImportApi.setPhase === 'function'
						? guidedStaticImportApi.setPhase(current, 'failed', { errors: [{ code: 'source-stale' }] })
						: { ...current, phase: 'failed', errors: [{ code: 'source-stale' }] };
					showSaveToast('failed', 'Source berubah atau tidak tersedia. Jalankan Re-analyze source terlebih dahulu.');
					return;
				}
				const mapping = typeof guidedStaticImportApi.buildCompileMapping === 'function'
					? guidedStaticImportApi.buildCompileMapping(current)
					: { version: 1, regions: [] };
				guidedStaticImportState.value = typeof guidedStaticImportApi.setPhase === 'function'
					? guidedStaticImportApi.setPhase(current, 'compiling', { errors: [] })
					: { ...current, phase: 'compiling', errors: [] };
				staticImportBusy.value = true;
				staticImportCompileError.value = '';
				staticImportCompileFallback.value = null;
				staticImportCompileResult.value = null;
				const controller = typeof AbortController === 'function' ? new AbortController() : null;
				staticImportCompileController.value = controller;
				let result = null;
				try {
					const data = new FormData();
					data.append('source', file);
					data.append('phase', 'compile');
					data.append('framework', staticImportFramework.value);
					data.append('mode', 'compiled');
					data.append('sourceHash', sourceHash);
					...
					if (!Array.isArray(result.layout) || !result.compilePayload) throw new Error('Compile response does not contain a complete layout payload.');
					showGuidedStaticImportModal.value = false;
					showStaticImportCompileModal.value = true;
					setStaticImportCompileStage('prepare');
					await hydrateCompiledStaticImport(result, controller);
					guidedStaticImportState.value = typeof guidedStaticImportApi.setPhase === 'function'
						? guidedStaticImportApi.setPhase(guidedStaticImportState.value, 'complete', { errors: [] })
						: { ...guidedStaticImportState.value, phase: 'complete', errors: [] };
					showSaveToast('success', 'Static page berhasil dikonversi ke Compiled Native tanpa dependency framework di output final.');
				} catch (error) {
					const cancelled = error?.code === 'compile-cancelled' || error?.name === 'AbortError' || controller?.signal?.aborted;
					if (cancelled) {
						closeStaticImportCompileModal(true);
						guidedStaticImportState.value = typeof guidedStaticImportApi.setPhase === 'function'
							? guidedStaticImportApi.setPhase(guidedStaticImportState.value, 'mapping', { errors: [] })
							: { ...guidedStaticImportState.value, phase: 'mapping', errors: [] };
						showGuidedStaticImportModal.value = true;
						nextTick(() => guidedStaticImportDialog.value?.focus?.());
						showSaveToast('info', 'Compiled Native dibatalkan; mapping tetap tersedia untuk dicoba kembali.');
					} else if (result?.exactFallback?.layout) {
						guidedStaticImportState.value = typeof guidedStaticImportApi.setPhase === 'function'
							? guidedStaticImportApi.setPhase(guidedStaticImportState.value, 'failed', { errors: [{ code: 'browser-compiler-failed' }] })
							: { ...guidedStaticImportState.value, phase: 'failed', errors: [{ code: 'browser-compiler-failed' }] };
						staticImportCompileError.value = error.response?.data?.message || error.message || 'Static import gagal.';
						staticImportCompileFallback.value = result.exactFallback;
						setStaticImportCompileStage('failed');
						showStaticImportCompileModal.value = true;
						showSaveToast('info', 'Compiled Native gagal. Pilih Exact Visual dari fallback tersanitasi atau kembali review mapping.');
					} else {
						const serverErrors = Array.isArray(error.response?.data?.errors) ? error.response.data.errors : [{ code: 'compile-request-failed' }];
						const safeErrors = serverErrors.map((item) => ({
							code: String(item?.code || 'compile-request-failed').replace(/[^a-z0-9-]/gi, '').slice(0, 80),
							...(item?.regionId ? { regionId: String(item.regionId).slice(0, 120) } : {}),
							...(item?.blockId ? { blockId: String(item.blockId).slice(0, 120) } : {}),
						}));
						guidedStaticImportState.value = typeof guidedStaticImportApi.setPhase === 'function'
							? guidedStaticImportApi.setPhase(guidedStaticImportState.value, 'failed', { errors: safeErrors })
							: { ...guidedStaticImportState.value, phase: 'failed', errors: safeErrors };
						showGuidedStaticImportModal.value = true;
						nextTick(() => guidedStaticImportDialog.value?.focus?.());
						showSaveToast('failed', 'Mapping belum berhasil dikompilasi. Periksa ringkasan error di wizard.');
					}
				} finally {
					staticImportCompileController.value = null;
					staticImportBusy.value = false;
			...
			const staticImportCompileStages = Object.freeze({
				prepare: { progress: 8, label: 'Reading source' },
				'load-framework': { progress: 24, label: 'Loading framework compiler' },
				compile: { progress: 42, label: 'Compiling framework CSS' },
				extract: { progress: 58, label: 'Extracting CSS' },
				rewrite: { progress: 72, label: 'Rewriting selectors' },
				'scan-sections': { progress: 78, label: 'Scanning source sections' },
				'measure-layout': { progress: 84, label: 'Measuring computed layout' },
				'map-native-settings': { progress: 88, label: 'Mapping native widget settings' },
				'emit-residual-css': { progress: 92, label: 'Emitting residual CSS' },
				validate: { progress: 96, label: 'Validating output' },
				cleanup: { progress: 98, label: 'Cleaning temporary compiler' },
				complete: { progress: 100, label: 'Completed' },
				failed: { progress: 100, label: 'Compilation failed' },
			});
			const staticImportCompileStageLabel = computed(() => staticImportCompileStages[staticImportCompileStage.value]?.label || 'Preparing compilation');
			const staticImportCompileIsActive = computed(() => showStaticImportCompileModal.value && staticImportCompileStage.value !== 'complete' && staticImportCompileStage.value !== 'failed');
			function setStaticImportCompileStage(stage) {
				const normalized = staticImportCompileStages[stage] ? stage : 'prepare';
				staticImportCompileStage.value = normalized;
				staticImportCompileProgress.value = staticImportCompileStages[normalized].progress;
				staticImportCompileMessage.value = staticImportCompileStages[normalized].label;
				nextTick(() => staticImportCompileDialog.value?.focus?.());
			}
			function closeStaticImportCompileModal(force = false) {
				if (staticImportCompileIsActive.value && !force) return;
				showStaticImportCompileModal.value = false;
				staticImportCompileFallback.value = null;
				staticImportCompileError.value = '';
				staticImportCompileStage.value = 'idle';
				staticImportCompileProgress.value = 0;
				staticImportCompileMessage.value = '';
			}
			function cancelStaticImportCompile() {
				if (!staticImportCompileIsActive.value) return;
				if (staticImportCompileController.value && typeof staticImportCompileController.value.abort === 'function') {
					staticImportCompileController.value.abort();
					return;
				}
				closeStaticImportCompileModal(true);
			}
			function handleStaticImportCompileEscape() {
				if (staticImportCompileIsActive.value) cancelStaticImportCompile();
				else closeStaticImportCompileModal();
			}
			function useExactStaticImportFallback() {
				const fallback = staticImportCompileFallback.value;
				if (!fallback || !Array.isArray(fallback.layout)) return;
				rootNodes.value = norm(fallback.layout);
				syncStaticImportDependencies(rootNodes.value);
				customCss.value = String(fallback.customCss || '');
				staticImportGeneratedCssStats.value = null;
				staticImportReport.value = fallback.report || null;
				staticImportPayload.value = fallback;
				if (fallback.pageName) pageName.value = String(fallback.pageName);
				saveState.value = 'dirty';
				closeStaticImportCompileModal();
				showSaveToast('info', 'Compiled Native gagal; Exact Visual sudah dipasang dari fallback tersanitasi. Periksa sebelum Save.');
			}
			function compiledSectionFallbackNode(section, currentNode, payload, compiledCss) {
				const marker = String(section?.marker || '').trim();
				if (!marker || typeof DOMParser !== 'function') return null;
				const sourceDocument = new DOMParser().parseFromString(String(payload?.html || ''), 'text/html');
				const sourceNode = Array.from(sourceDocument.querySelectorAll('[data-pb-import-node]'))
					.find((element) => element.getAttribute('data-pb-import-node') === marker);
				if (!sourceNode) return null;
				const safeCss = String(compiledCss || '').replace(/<\/style/gi, '<\\/style');
				const sourceHtml = String(sourceNode.outerHTML || '').replace(/<script\b[\s\S]*?<\/script\s*>/gi, '');
				const title = String(section.sourceId || section.tag || 'Imported section').trim() || 'Imported section';
				const srcdoc = '<!doctype html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>'
					+ safeCss + '</style></head><body class="pb-import-root">' + sourceHtml + '</body></html>';
				return {
					id: String(currentNode?.id || marker) + '-fallback',
					type: 'static_html',
					label: 'Static HTML Preview',
					labelSuffix: '',
					settings: { srcdoc, title, height: '1200px', importMode: 'compiled-fallback' },
				};
			}
			async function hydrateCompiledStaticImport(result, controller) {
				const compiler = window.PhoenixStaticImportCompiler;
				const nativeMapper = window.PhoenixStaticImportNative;
				const cssManager = window.PhoenixStaticImportCss;
				if (!compiler || typeof compiler.compile !== 'function') throw new Error('Compiled Native compiler helper belum tersedia.');
				if (!nativeMapper || typeof nativeMapper.applyComputedSnapshot !== 'function' || typeof nativeMapper.replaceFallbackSections !== 'function' || typeof compiler.scanComputedStyles !== 'function') throw new Error('Computed-style scanner helper belum tersedia.');
				if (!cssManager || typeof cssManager.replaceGeneratedStaticImportCss !== 'function') throw new Error('Generated CSS block manager belum tersedia.');
				if (!result.compilePayload || !Array.isArray(result.layout)) throw new Error('Compiled Native payload tidak lengkap.');
				const compiled = await compiler.compile(result.compilePayload, {
					signal: controller?.signal,
					onProgress: setStaticImportCompileStage,
				});
				const snapshot = await compiler.scanComputedStyles(result.compilePayload, {
					signal: controller?.signal,
					viewports: result.compilePayload.viewports,
					onProgress: setStaticImportCompileStage,
		});
		setStaticImportCompileStage('map-native-settings');
		nativeMapper.applyComputedSnapshot(result.layout, snapshot);
		const fallbackSections = Array.isArray(result.compilePayload.sections)
			? result.compilePayload.sections.filter((section) => section && section.fallback)
			: [];
		nativeMapper.replaceFallbackSections(result.layout, fallbackSections, (section, currentNode) => compiledSectionFallbackNode(section, currentNode, result.compilePayload, compiled.css) || currentNode);
		setStaticImportCompileStage('emit-residual-css');
		const ownedMarkers = typeof nativeMapper.collectNodes === 'function'
			? nativeMapper.collectNodes(result.layout).map((node) => node?.settings?.importNodeKey).filter(Boolean)
			: null;
		const residual = typeof compiler.filterResidualCss === 'function'
			? compiler.filterResidualCss(compiled.css, snapshot, ownedMarkers ? { ownedMarkers } : {})
			: { css: compiled.css, stats: {} };
				const mergedStats = {
					...(compiled.stats || {}),
					...(residual.stats || {}),
					sections: Number(snapshot.stats?.sections || 0),
					measuredNodes: Number(snapshot.stats?.measuredNodes || 0),
					fallbackSections: fallbackSections.length,
				};
				const merged = cssManager.replaceGeneratedStaticImportCss(customCss.value, residual.css, {
					sourceHash: result.compilePayload.sourceHash,
					stats: mergedStats,
				});
				if (!merged.valid) throw new Error('Generated CSS validation failed: ' + (merged.warnings || []).join(', '));
				setStaticImportCompileStage('validate');
				rootNodes.value = norm(result.layout);
				syncStaticImportDependencies(rootNodes.value);
				customCss.value = merged.css;
				staticImportGeneratedCssStats.value = {
					...mergedStats,
					generatedChars: String(residual.css || '').length,
				};
				const compileWarnings = [...(result.report?.warnings || []), ...(compiled.warnings || []), ...(merged.warnings || [])];
				staticImportCompileResult.value = { ...compiled, stats: mergedStats, warnings: compileWarnings, cssWarnings: merged.warnings || [] };
				staticImportReport.value = {
					...(result.report || {}),
					mappingReport: result.mappingReport || null,
					detectedRegions: Number(result.mappingReport?.regions || result.compilePayload?.regions?.length || 0),
					nativeRegions: Number(result.mappingReport?.nativeRegions || 0),
					exactRegions: Number(result.mappingReport?.exactRegions || 0),
					skippedRegions: Number(result.mappingReport?.skippedRegions || 0),
					mappedBlocks: Number(result.mappingReport?.mappedBlocks || 0),
					unmappedBlocks: Number(result.mappingReport?.unmappedBlocks || 0),
					compileStats: mergedStats,
					scanStats: { ...(snapshot.stats || {}), fallbackSections: fallbackSections.length },
					compileWarnings,
				};
				staticImportPayload.value = result;
				if (result.pageName) pageName.value = String(result.pageName);
				setStaticImportCompileStage('complete');
				saveState.value = 'dirty';
			}
			...
			async function importStaticFile(event) {
				...
				staticImportSourceFile.value = file;
				staticImportBusy.value = true;
				const compiledAttempt = staticImportMode.value === 'compiled';
				let result = null;
				let keepGuidedImportOpen = false;
				try {
					if (compiledAttempt) {
						const controller = typeof AbortController === 'function' ? new AbortController() : null;
						staticImportCompileController.value = controller;
						beginGuidedStaticImportAnalysis(file);
						const data = new FormData();
						data.append('source', file);
						data.append('phase', 'analyze');
						data.append('framework', staticImportFramework.value);
						...
						staticImportPayload.value = null;
						staticImportCompileController.value = null;
						keepGuidedImportOpen = true;
						showSaveToast('info', 'Source berhasil dianalisis. Periksa mapping per region sebelum Compile.');
						return;
					}
					...
					data.append('source', file);
					data.append('framework', staticImportFramework.value);
					data.append('mode', staticImportMode.value);
					if (staticImportBaseUrl.value.trim()) data.append('baseUrl', staticImportBaseUrl.value.trim());
					const response = await axios.post(PBC.staticImportUrl, data, {
						headers: {
							'X-CSRF-TOKEN': PBC.csrfToken,
					...
					const compileCancelled = compiledAttempt && (error?.code === 'compile-cancelled' || error?.name === 'AbortError' || staticImportCompileController.value?.signal?.aborted);
					if (compiledAttempt && compileCancelled) {
						closeStaticImportCompileModal(true);
						if (guidedStaticImportState.value.analysis) {
							guidedStaticImportState.value = typeof guidedStaticImportApi.setPhase === 'function'
								? guidedStaticImportApi.setPhase(guidedStaticImportState.value, 'mapping', { errors: [] })
								: { ...guidedStaticImportState.value, phase: 'mapping', errors: [] };
							showGuidedStaticImportModal.value = true;
						...
						showSaveToast('info', 'Compiled Native dibatalkan; layout dan Custom CSS sebelumnya dipertahankan.');
					} else if (compiledAttempt && result?.exactFallback?.layout) {
						staticImportCompileError.value = error.response?.data?.message || error.message || 'Static import gagal.';
						staticImportCompileFallback.value = result.exactFallback;
						setStaticImportCompileStage('failed');
						showStaticImportCompileModal.value = true;
						showSaveToast('info', 'Compiled Native gagal. Pilih Exact Visual dari fallback tersanitasi atau Cancel.');
					} else if (compiledAttempt) {
						const serverErrors = Array.isArray(error.response?.data?.errors) ? error.response.data.errors : [{ code: 'analyze-request-failed' }];
						const safeErrors = serverErrors.map((item) => ({
							code: String(item?.code || 'analyze-request-failed').replace(/[^a-z0-9-]/gi, '').slice(0, 80),
							...(item?.regionId ? { regionId: String(item.regionId).slice(0, 120) } : {}),
							...(item?.blockId ? { blockId: String(item.blockId).slice(0, 120) } : {}),
						}));
						guidedStaticImportState.value = typeof guidedStaticImportApi.setPhase === 'function'
						...
						showGuidedStaticImportModal.value = true;
						nextTick(() => guidedStaticImportDialog.value?.focus?.());
						showSaveToast('failed', 'Source tidak dapat dianalisis. Periksa error lalu coba lagi.');
					} else {
						showSaveToast('failed', error.response?.data?.message || error.message || 'Static import gagal.');
					}
				} finally {
					if (!keepGuidedImportOpen) {
						staticImportCompileController.value = null;
						staticImportBusy.value = false;
					}
			...
			function showStaticImportReport() {
				staticImportCopyConfirmation.value = false;
				staticImportCopyAcknowledged.value = false;
				showStaticImportReportModal.value = true;
			}

			function closeStaticImportReport() {
				showStaticImportReportModal.value = false;
				staticImportCopyConfirmation.value = false;
				staticImportCopyAcknowledged.value = false;
			}

			function requestCopyDetectedStaticScripts() {
				if (!staticImportSourceFile.value) {
					showSaveToast('info', 'Pilih ulang file sumber jika browser tidak lagi menyediakan file import sebelumnya.');
					return;
				}
				staticImportCopyConfirmation.value = true;
				staticImportCopyAcknowledged.value = false;
			}

			async function copyDetectedStaticScriptsToCustomJavaScript() {
				if (!staticImportCopyAcknowledged.value || staticImportCopyBusy.value || !staticImportSourceFile.value || !PBC.staticImportUrl) return;
				staticImportCopyBusy.value = true;
				try {
					const data = new FormData();
					data.append('source', staticImportSourceFile.value);
					data.append('framework', staticImportFramework.value);
					data.append('mode', staticImportMode.value);
					if (staticImportBaseUrl.value.trim()) data.append('baseUrl', staticImportBaseUrl.value.trim());
					data.append('includeScripts', '1');
					const response = await axios.post(PBC.staticImportUrl, data, {
						headers: {
							'X-CSRF-TOKEN': PBC.csrfToken,
							'X-Requested-With': 'XMLHttpRequest',
					...
					const candidates = response.data?.report?.sourceScripts?.inlineCode;
					if (!Array.isArray(candidates) || candidates.length === 0) {
						showSaveToast('info', 'Tidak ada inline source script yang dapat disalin.');
						return;
					}
					customJs.value = candidates.join('\n\n');
					customJsMode.value = 'disabled';
					customJsPublishAcknowledged.value = false;
					analyzeCustomJavaScript();
					showStaticImportReportModal.value = false;
					openCustomJsEditor();
					showSaveToast('info', candidates.length + ' inline source script disalin ke Custom JavaScript dalam mode Disabled. Periksa diagnostic sebelum Save.');
				} catch (error) {
					showSaveToast('failed', error.response?.data?.message || error.message || 'Source script tidak dapat disalin.');
				} finally {
					staticImportCopyBusy.value = false;
					staticImportCopyConfirmation.value = false;
					staticImportCopyAcknowledged.value = false;
				}
			}
			...
			const widgetEditorServices = {
				...
				gridColumnStyleControls: GridColumnStyleControls,
				linkControl: LinkControl,
				cssFilterControl: CssFilterControl,
				typographyControl: TypographyControl,
				widgetAdvancedControls: WidgetAdvancedControls,
				gridColumnTrackCells,
				selectGridColumnStyleTarget,
				clearGridColumnStyleTarget,
				gridColumnStyleTargetLabel,
				gridColumnStyleTargetIsCell,
				gridColumnStyleValue,
				setGridColumnStyleValue,
				resetSelectedGridColumnStyle,
				chooseGridColumnBgImage,
				clearGridColumnBgImage,
				draggable,
				tabsItemsForNode,
				tabsActiveItem,
				selectTabsItem,
			...
			return {
				appTitle, pageSettingsOpen, openPageSettings, closePageSettings, toolbox, elementSearch, filteredToolboxGroups, leftCollapsed, previewMode, rootNodes, loadWidget, loadWidgetSettings, warmWidgetModules, hasRegisteredWidget, widgetEditorServices, isCont, isGrid,
				toolClone, sidebarContGroup, sidebarGridGroup, sidebarWgtGroup, rootGroup,
				selectedId, hoveredId, settingsTab, imageBoxImageState, iconBoxIconState, selectedNode, selectedType, selectedSupportsRichTextModal, activeSettingsTabs, selectedNodeKind,
				...
				staticImportInput, staticImportSourceFile, staticImportBusy, staticImportReport, staticImportPayload, staticImportFramework, staticImportBaseUrl, staticImportMode, staticImportCompileController, staticImportCompileStage, staticImportCompileProgress, staticImportCompileMessage, staticImportCompileError, staticImportCompileFallback, staticImportCompileResult, staticImportCompileDialog, staticImportCompileStageLabel, staticImportCompileIsActive, showStaticImportCompileModal, staticImportGeneratedCssStats, showStaticImportReportModal, staticImportCopyConfirmation, staticImportCopyAcknowledged, staticImportCopyBusy, openStaticImport, importStaticFile, closeStaticImportCompileModal, cancelStaticImportCompile, handleStaticImportCompileEscape, useExactStaticImportFallback, showStaticImportReport, closeStaticImportReport, requestCopyDetectedStaticScripts, copyDetectedStaticScriptsToCustomJavaScript, downloadStaticImportJson,
				guidedStaticImportState, showGuidedStaticImportModal, guidedStaticImportDialog, staticImportTrigger, guidedStaticImportActiveRegion, guidedStaticImportPhaseLabel, guidedStaticImportPreviewSrcdoc, guidedStaticImportRegionValidation, guidedStaticImportCanCompile, guidedStaticImportCanReanalyze, staticImportReportMapping, staticImportReportDetectedRegions, staticImportReportNativeRegions, staticImportReportExactRegions, staticImportReportSkippedRegions, staticImportReportMappedBlocks, staticImportReportUnmappedBlocks, staticImportReportRelativeAssets, staticImportReportSourceScripts, openGuidedStaticImportModal, beginGuidedStaticImportAnalysis, guidedStaticImportSelectRegion, guidedStaticImportRegionStrategy, guidedStaticImportSetStrategy, guidedStaticImportSetBlockWidget, guidedStaticImportBlockWidget, guidedStaticImportBlockHasChildren, guidedStaticImportBlockExpanded, toggleGuidedStaticImportBlock, guidedStaticImportVisibleBlocks, guidedStaticImportGoBack, guidedStaticImportCancel, guidedStaticImportCompileSelected, guidedStaticImportReanalyze, guidedStaticImportErrorMessage,
				...
				customCssGotoLine, customCssSearchQuery, customCssActiveLine, customCssCharCount, customCssLineCount, customCssLineNumbers, customCssSummary, customCssHasGeneratedBlock, canvasCustomCss,
	...
	<header class="topbar">
		<div class="topbar-left">
			<div class="brand-lockup">
				...
				<div class="brand-copy"><strong>Phoenix</strong><span>Page Builder 2.4</span></div>
			</div>
		...
		<div class="topbar-right">
			...
			<button ref="staticImportTrigger" class="top-action" :disabled="staticImportBusy" @click="openStaticImport"><i class="bi bi-file-earmark-arrow-up me-1"></i>{{ staticImportBusy ? 'Importing' : 'Import Static' }}</button>
			<select v-model="staticImportMode" class="pb-select pb-import-mode" aria-label="Static import mode" title="Static import mode"><option value="exact">Exact Visual</option><option value="native">Editable Native</option><option value="compiled">Compiled Native</option></select>
			...
			<input v-model="staticImportBaseUrl" class="pb-input pb-import-base-url" type="url" aria-label="Asset Base URL" title="Optional base URL for relative images and links" placeholder="Asset Base URL (optional)">
	...
	<main class="workspace" :class="{ 'left-collapsed': leftCollapsed, 'preview-mode': previewMode }">
		<aside class="side-panel left-panel pb-panel left">
			...
			<template v-else>
				...
				<div class="pb-section v24-properties-section">
		...
		<component v-if="canvasCustomCss" :is="'style'">{{ canvasCustomCss }}</component>
			...
			<div class="pb-css-editor-panel" :class="{ fullscreen: cssEditorFullscreen }">
				...
				<div class="pb-css-editor-body">
					<div class="pb-css-editor-hint">
						...
						<div><span>Tulis CSS kustom di sini. Gunakan <code>.class-name</code> atau <code>#id-name</code> dari field CSS ID / CSS Class pada widget, column, atau container.</span><span v-if="customCssHasGeneratedBlock" class="pb-css-editor-generated-badge">Generated CSS · Compiled Native</span></div>
					</div>
...
#my-section {
  ...
  padding: 60px 0;
	...
	<teleport to="body">
		<div v-if="showGuidedStaticImportModal" ref="guidedStaticImportDialog" class="pb-guided-static-import-modal" role="dialog" aria-modal="true" aria-labelledby="pb-guided-static-import-title" tabindex="-1" @keydown.esc="guidedStaticImportCancel">
			<div class="pb-guided-static-import-panel">
				<div class="pb-guided-static-import-header">
					<div><strong id="pb-guided-static-import-title">Guided Compiled Native import</strong><span>{{ guidedStaticImportPhaseLabel }}</span></div>
					<div class="pb-guided-static-import-progress-label">{{ guidedStaticImportState.analysis?.regions?.length || 0 }} regions</div>
				</div>
				<div v-if="guidedStaticImportState.phase==='analyzing'" class="pb-guided-static-import-analyzing" aria-live="polite">
					<div class="pb-guided-static-import-spinner" aria-hidden="true"><i class="fas fa-circle-notch fa-spin"></i></div>
					<strong>Scanning source structure...</strong>
					<p>Detecting Header, Sections, Footer, block hierarchy, and widget recommendations. Canvas remains unchanged.</p>
				</div>
				<div v-else class="pb-guided-static-import-review">
					<div v-if="guidedStaticImportState.errors?.length" class="pb-guided-static-import-error-summary" role="alert" aria-live="assertive">
						<div><strong>Import needs attention</strong><button v-if="guidedStaticImportCanReanalyze" type="button" class="pb-guided-static-import-reanalyze" @click="guidedStaticImportReanalyze">Re-analyze source</button></div>
						<div v-for="error in guidedStaticImportState.errors" :key="'guided-error-' + error.code + '-' + (error.regionId || '') + '-' + (error.blockId || '')"><i class="fas fa-exclamation-circle" aria-hidden="true"></i><span>{{ guidedStaticImportErrorMessage(error) }}<small v-if="error.regionId"> · {{ error.regionId }}</small><small v-if="error.blockId"> · {{ error.blockId }}</small></span></div>
					</div>
					<div class="pb-guided-static-import-workspace">
					<aside class="pb-guided-static-import-regions" aria-label="Detected regions">
						<div class="pb-guided-static-import-section-heading"><strong>Detected regions</strong><span>{{ guidedStaticImportState.analysis?.regions?.length || 0 }}</span></div>
						<div class="pb-guided-static-import-region-list">
							<button v-for="region in (guidedStaticImportState.analysis?.regions || [])" :key="region.id" type="button" class="pb-guided-static-import-region" :class="{ active: guidedStaticImportActiveRegion?.id===region.id }" :aria-selected="guidedStaticImportActiveRegion?.id===region.id" @click="guidedStaticImportSelectRegion(region.id)">
								<span class="pb-guided-static-import-region-title"><strong>{{ region.label }}</strong><em>{{ region.kind }}</em></span>
								<span class="pb-guided-static-import-region-meta"><span>{{ Math.round((region.confidence || 0) * 100) }}% confidence</span><span v-if="region.warnings?.length" class="is-warning"><i class="fas fa-exclamation-triangle" aria-hidden="true"></i>{{ region.warnings.length }} warning{{ region.warnings.length > 1 ? 's' : '' }}</span></span>
							</button>
						</div>
					</aside>
					<section class="pb-guided-static-import-preview" aria-label="Selected region preview">
						<div class="pb-guided-static-import-section-heading"><strong>Source preview</strong><span>{{ guidedStaticImportActiveRegion?.label || 'Select a region' }}</span></div>
						<div class="pb-guided-static-import-preview-frame">
							<iframe v-if="guidedStaticImportPreviewSrcdoc" :srcdoc="guidedStaticImportPreviewSrcdoc" sandbox="" :title="'Preview of ' + (guidedStaticImportActiveRegion?.label || 'selected region')"></iframe>
							<div v-else class="pb-guided-static-import-empty">Preview unavailable for this region.</div>
						</div>
					</section>
					<section class="pb-guided-static-import-detail" aria-label="Region mapping details">
						<div class="pb-guided-static-import-section-heading"><strong>Mapping</strong><span>{{ guidedStaticImportActiveRegion?.sourceId || guidedStaticImportActiveRegion?.kind || 'Region' }}</span></div>
						<label class="pb-guided-static-import-field"><span>Region strategy</span><select :value="guidedStaticImportRegionStrategy(guidedStaticImportActiveRegion)" :aria-label="'Strategy for ' + (guidedStaticImportActiveRegion?.label || 'region')" @change="guidedStaticImportSetStrategy(guidedStaticImportActiveRegion?.id, $event.target.value)"><option value="auto_native">Use recommendations</option><option value="guided_native">Review mapping</option><option value="exact_visual">Exact Visual</option><option value="skip">Skip region</option></select></label>
						<div v-if="guidedStaticImportRegionValidation.errors?.length" class="pb-guided-static-import-validation" role="alert" aria-live="polite"><strong>Review required</strong><div v-for="error in guidedStaticImportRegionValidation.errors" :key="error.code + '-' + (error.blockId || '')"><i class="fas fa-exclamation-circle" aria-hidden="true"></i><span>{{ guidedStaticImportErrorMessage(error) }}<small v-if="error.blockId"> · {{ error.blockId }}</small></span></div></div>
						<div class="pb-guided-static-import-tree" role="tree" aria-label="Widget mapping tree">
							<div v-for="block in guidedStaticImportVisibleBlocks(guidedStaticImportActiveRegion)" :key="block.id" class="pb-guided-static-import-tree-row" role="treeitem" :style="{ paddingLeft: (block.depth * 16 + 8) + 'px' }">
								<button v-if="guidedStaticImportBlockHasChildren(block)" type="button" class="pb-guided-static-import-tree-toggle" :aria-expanded="guidedStaticImportBlockExpanded(block.id)" :aria-label="(guidedStaticImportBlockExpanded(block.id) ? 'Collapse ' : 'Expand ') + (block.label || block.role)" @click="toggleGuidedStaticImportBlock(block.id)"><i class="fas" :class="guidedStaticImportBlockExpanded(block.id) ? 'fa-chevron-down' : 'fa-chevron-right'" aria-hidden="true"></i></button><span v-else class="pb-guided-static-import-tree-spacer" aria-hidden="true"></span>
								<div class="pb-guided-static-import-tree-copy"><strong>{{ block.label || block.role }}</strong><small>{{ block.role }} · {{ block.tag }}</small></div>
								<select v-if="guidedStaticImportRegionStrategy(guidedStaticImportActiveRegion)==='guided_native' && block.widgetOptions?.length" :value="guidedStaticImportBlockWidget(guidedStaticImportActiveRegion.id, block)" :aria-label="'Widget for ' + (block.label || block.role)" @change="guidedStaticImportSetBlockWidget(guidedStaticImportActiveRegion.id, block.id, $event.target.value)"><option v-for="option in block.widgetOptions" :key="option.type" :value="option.type">{{ option.label }}</option></select>
								<span v-else class="pb-guided-static-import-widget-badge">{{ guidedStaticImportBlockWidget(guidedStaticImportActiveRegion?.id, block) || 'Unmapped' }}</span>
							</div>
							<div v-if="!guidedStaticImportVisibleBlocks(guidedStaticImportActiveRegion).length" class="pb-guided-static-import-empty">No editable blocks in this region.</div>
						</div>
					</section>
					</div>
				</div>
				<div class="pb-guided-static-import-footer">
					<button type="button" class="pb-guided-static-import-btn secondary" :disabled="!guidedStaticImportActiveRegion || guidedStaticImportActiveRegion.order===0" @click="guidedStaticImportGoBack">Back</button>
					<div class="pb-guided-static-import-footer-actions"><button type="button" class="pb-guided-static-import-btn danger" @click="guidedStaticImportCancel">Cancel</button><button type="button" class="pb-guided-static-import-btn primary" :disabled="!guidedStaticImportCanCompile" @click="guidedStaticImportCompileSelected">{{ guidedStaticImportState.phase==='failed' ? 'Retry compile' : 'Compile selected regions' }}</button></div>
				</div>
			</div>
		</div>
	</teleport>
	...
	<teleport to="body">
		<div v-if="showStaticImportCompileModal" ref="staticImportCompileDialog" class="pb-static-import-compile-modal" role="dialog" aria-modal="true" aria-labelledby="pb-static-import-compile-title" tabindex="-1" @click.self="closeStaticImportCompileModal" @keydown.esc="handleStaticImportCompileEscape">
			<div class="pb-static-import-compile-panel">
				<div class="pb-static-import-compile-header">
					<div><strong id="pb-static-import-compile-title">Compiled Native import</strong><span>{{ staticImportCompileStageLabel }}</span></div>
					<div class="pb-static-import-compile-percent">{{ staticImportCompileProgress }}%</div>
				</div>
				<div class="pb-static-import-compile-body" aria-live="polite">
					<div class="pb-static-import-compile-progress" role="progressbar" :aria-valuenow="staticImportCompileProgress" aria-valuemin="0" aria-valuemax="100"><span :style="{ width: staticImportCompileProgress + '%' }"></span></div>
					<div class="pb-static-import-compile-stage-list">
						<span :class="{ active: staticImportCompileStage==='prepare' }">Reading source</span>
						<span :class="{ active: staticImportCompileStage==='load-framework' }">Detecting framework / loading assets</span>
						<span :class="{ active: staticImportCompileStage==='compile' }">Compiling framework CSS</span>
						<span :class="{ active: staticImportCompileStage==='extract' }">Extracting CSS</span>
						<span :class="{ active: staticImportCompileStage==='rewrite' }">Rewriting selectors</span>
						<span :class="{ active: staticImportCompileStage==='scan-sections' }">Scanning source sections</span>
						<span :class="{ active: staticImportCompileStage==='measure-layout' }">Measuring computed layout</span>
						<span :class="{ active: staticImportCompileStage==='map-native-settings' }">Mapping native widget settings</span>
						<span :class="{ active: staticImportCompileStage==='emit-residual-css' }">Emitting residual CSS</span>
						<span :class="{ active: staticImportCompileStage==='validate' }">Validating output</span>
						<span :class="{ active: staticImportCompileStage==='cleanup' }">Cleaning temporary compiler</span>
					</div>
					<div v-if="staticImportCompileError" class="pb-static-import-compile-error" role="alert"><i class="fas fa-exclamation-triangle"></i><span>{{ staticImportCompileError }}</span></div>
					<div v-if="staticImportCompileResult" class="pb-static-import-compile-stats">
						<span>{{ staticImportCompileResult.stats?.sourceClasses || 0 }} source classes</span><span>{{ staticImportCompileResult.stats?.generatedRules || 0 }} generated rules</span><span>{{ staticImportCompileResult.stats?.rewrittenRules || 0 }} rewritten rules</span><span>{{ staticImportCompileResult.stats?.measuredNodes || 0 }} measured nodes</span><span>{{ staticImportCompileResult.stats?.fallbackSections || 0 }} fallback sections</span>
					</div>
					<div v-if="staticImportCompileResult?.warnings?.length" class="pb-static-import-compile-warnings"><div v-for="warning in staticImportCompileResult.warnings" :key="warning"><i class="fas fa-info-circle"></i><span>{{ warning }}</span></div></div>
					<div v-if="staticImportCompileFallback" class="pb-static-import-compile-fallback"><i class="fas fa-shield-alt"></i><span>Compiler gagal. Exact Visual menggunakan payload yang sudah tersanitasi dan tidak menjalankan source script.</span></div>
				</div>
				<div class="pb-static-import-compile-footer">
					<button v-if="staticImportCompileIsActive" type="button" class="pb-css-editor-btn danger" @click="cancelStaticImportCompile">Cancel</button>
					<button v-if="staticImportCompileFallback" type="button" class="pb-css-editor-btn primary" @click="useExactStaticImportFallback">Use Exact Visual</button>
					<button v-if="!staticImportCompileIsActive" type="button" class="pb-css-editor-btn primary" @click="closeStaticImportCompileModal">Close</button>
				</div>
			</div>
		</div>
	</teleport>
	...
	<teleport to="body">
		<div v-if="showStaticImportReportModal" class="pb-static-import-report-modal" role="dialog" aria-modal="true" aria-labelledby="pb-static-import-report-title" @click.self="closeStaticImportReport" @keydown.esc="closeStaticImportReport">
			<div class="pb-static-import-report-panel">
				<div class="pb-static-import-report-header">
					<div><strong id="pb-static-import-report-title">Static import report</strong><span>Source behavior remains disabled until you explicitly copy it.</span></div>
					<button type="button" class="pb-static-import-report-close" title="Close report" @click="closeStaticImportReport"><i class="fas fa-times"></i></button>
				</div>
				<div class="pb-static-import-report-body" aria-live="polite">
					<div class="pb-static-import-report-stats">
						<div><strong>{{ staticImportReportDetectedRegions }}</strong><span>Detected regions</span></div>
						<div><strong>{{ staticImportReportNativeRegions }}</strong><span>Native regions</span></div>
						<div><strong>{{ staticImportReportExactRegions }}</strong><span>Exact regions</span></div>
						<div><strong>{{ staticImportReportSkippedRegions }}</strong><span>Skipped regions</span></div>
						<div><strong>{{ staticImportReportMappedBlocks }}</strong><span>Mapped blocks</span></div>
						<div><strong>{{ staticImportReportUnmappedBlocks }}</strong><span>Unmapped blocks</span></div>
						<div><strong>{{ staticImportReportRelativeAssets }}</strong><span>Relative assets</span></div>
						<div><strong>{{ staticImportReportSourceScripts }}</strong><span>Source scripts</span></div>
						<div><strong>{{ staticImportReport?.mappedNodes || 0 }}</strong><span>Mapped nodes</span></div>
						<div><strong>{{ staticImportReport?.placeholderNodes || 0 }}</strong><span>Placeholders</span></div>
						<div><strong>{{ staticImportReport?.warnings?.length || 0 }}</strong><span>Warnings</span></div>
					</div>
					<div v-if="staticImportReport?.sourceScripts" class="pb-static-import-script-report">
						<div class="pb-static-import-report-section-title"><i class="bi bi-filetype-js"></i><span>Source scripts detected</span></div>
						<p>{{ staticImportReport.sourceScripts.total }} total · {{ staticImportReport.sourceScripts.inline }} inline · {{ staticImportReport.sourceScripts.external }} external.</p>
						<p v-if="staticImportReport.sourceScripts.eventSelectors?.length">Detected selectors: <code>{{ staticImportReport.sourceScripts.eventSelectors.join(', ') }}</code></p>
						<div v-if="staticImportReport.sourceScripts.warnings?.length" class="pb-static-import-report-warnings">
							<div v-for="warning in staticImportReport.sourceScripts.warnings" :key="warning"><i class="fas fa-exclamation-triangle"></i><span>{{ warning }}</span></div>
						</div>
						<button v-if="staticImportReport.sourceScripts.inline" type="button" class="pb-static-import-copy-btn" @click="requestCopyDetectedStaticScripts"><i class="fas fa-copy"></i>Copy detected scripts to Custom JavaScript</button>
						<div v-if="staticImportCopyConfirmation" class="pb-static-import-copy-confirm">
							<label><input type="checkbox" v-model="staticImportCopyAcknowledged"> <span>Salin hanya inline JavaScript ke editor terpisah; kode tidak akan dijalankan otomatis.</span></label>
							<button type="button" class="pb-static-import-copy-confirm-btn" :disabled="!staticImportCopyAcknowledged || staticImportCopyBusy" @click="copyDetectedStaticScriptsToCustomJavaScript">{{ staticImportCopyBusy ? 'Copying...' : 'Confirm copy' }}</button>
						</div>
					</div>
					<div v-else class="pb-static-import-report-empty"><i class="bi bi-shield-check"></i><span>Tidak ada source script dalam import ini.</span></div>
				</div>
				<div class="pb-static-import-report-footer"><button type="button" class="pb-css-editor-btn primary" @click="closeStaticImportReport">Close</button></div>
			</div>
		</div>
	</teleport>
			...
			<div class="pb-js-editor-panel" :class="{ fullscreen: jsEditorFullscreen }">
				...
				<div class="pb-js-editor-body">
					<div class="pb-js-editor-hint">
						...
						<span>Kode dianalisis tanpa dijalankan di editor. Compile atau re-import tidak otomatis menimpa Custom JavaScript page ini.</span>
					</div>
					<div class="pb-js-editor-toolbar">
						...
						<label class="pb-js-editor-mode-field">
							...
							<select v-model="customJsMode" @change="onCustomJsModeChange">
								...
								<option value="exact_sandbox">Exact sandbox — isolated iframe</option>
					...
					<div class="pb-js-editor-code-shell">
						...
						<textarea ref="customJsEditorTextarea" class="pb-js-editor-textarea" v-model="customJs" placeholder="document.querySelector('[data-pb-import-node]')?.classList.add('is-ready');" aria-label="Custom JavaScript code" @input="analyzeCustomJavaScript" @keydown.tab.prevent="handleCustomJsTab" @scroll="syncCustomJsEditorScroll" spellcheck="false" autocomplete="off"></textarea>
					</div>
					...
					<div v-if="customJsMode === 'exact_sandbox'" class="pb-js-editor-policy-note is-sandbox"><i class="fas fa-lock"></i><span>Kode hanya boleh berjalan di iframe Exact Visual yang terisolasi. Parent editor tidak menerima script ini.</span></div>
	...
	<teleport to="body">
		<div v-if="showTextEditorModal && selectedSupportsRichTextModal && selectedNode" class="pb-rte-editor-modal" @click.self="closeTextEditorModal">
	...
	<teleport to="body">
		<div v-if="modal.visible" class="pb-modal-backdrop" @click.self="closeModal">
			<div class="pb-modal">
				...
				<template v-if="modal.layoutType==='container_flex'||isCont(modal.layoutType)">
				...
				<template v-else-if="isGrid(modal.layoutType)||modal.layoutType==='container_grid'">
							<div class="pb-preset-grid">

<template>
	<div class="el-widget-image pb-image" :class="hoverAnimationClass" :style="rootStyle" data-basic-image :data-pb-import-node="importNodeKey">
		<figure class="pb-image__figure">
			<a v-if="linkUrl" class="pb-image__link" :href="linkUrl" :target="linkTarget" :rel="linkRel" v-bind="safeCustomAttributes" :data-basic-image-lightbox="usesLightbox ? 'true' : null" data-pb-interactive="true" @click.stop="onImageClick">
				<img v-if="imageUrl" class="pb-image__img" :src="imageUrl" :alt="alt" :style="imageStyle" :data-pb-import-node="importNodeKey">
				<div v-else class="pb-image__empty" role="img" aria-label="Choose an image"><i class="far fa-image" aria-hidden="true"></i></div>
			</a>
			<template v-else><img v-if="imageUrl" class="pb-image__img" :src="imageUrl" :alt="alt" :style="imageStyle" :data-pb-import-node="importNodeKey"><div v-else class="pb-image__empty" role="img" aria-label="Choose an image"><i class="far fa-image" aria-hidden="true"></i></div></template>
...
export default{
	...
	computed:{
		...
		importNodeKey(){const value=String(this.settings.importNodeKey||'').trim();return/^import-node-[A-Za-z0-9_-]+$/.test(value)?value:null;},

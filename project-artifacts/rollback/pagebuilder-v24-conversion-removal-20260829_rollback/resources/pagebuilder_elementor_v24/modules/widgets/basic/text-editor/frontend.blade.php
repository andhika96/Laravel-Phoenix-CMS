@php
	...
	$allowCompiledMarkers = preg_match('/^import-node-[A-Za-z0-9_-]+$/', trim((string) ($settings['importNodeKey'] ?? ''))) === 1;
	$allowedAttributes = ['A' => ['href', 'title', 'target', 'rel'], '*' => $allowCompiledMarkers ? ['class', 'id', 'data-pb-import-node'] : ['class']];
		$sanitizeNode = static function (\DOMNode $parent) use (&$sanitizeNode, $allowedTags, $allowedAttributes): void {
			foreach (iterator_to_array($parent->childNodes) as $child) {
				...
				foreach (iterator_to_array($child->attributes) as $attribute) {
					...
					if ($name === 'data-pb-import-node' && (!$allowCompiledMarkers || !preg_match('/^import-node-[A-Za-z0-9_-]+$/', trim($value)))) {
						$child->removeAttribute($attribute->name);
						continue;
					}
					if ($name === 'id' && (!$allowCompiledMarkers || !preg_match('/^[A-Za-z][A-Za-z0-9_-]*$/', trim($value)))) {
						$child->removeAttribute($attribute->name);
						continue;
					}

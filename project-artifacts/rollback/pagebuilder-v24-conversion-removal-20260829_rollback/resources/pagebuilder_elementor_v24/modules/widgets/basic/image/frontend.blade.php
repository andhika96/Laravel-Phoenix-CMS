@php
	...
	if ($imageUrl !== '') {
		$imageUrl = app(\App\Support\PageBuilderElementorV24\ImageRenditionResolver::class)->resolve(
			$imageUrl,
			$imageResolution,
	...
	<figure class="pb-image__figure">
		@if($linkUrl !== '')
			<a class="pb-image__link" href="{{ $linkUrl }}" @if($linkTarget !== '') target="{{ $linkTarget }}" @endif @if($linkRel !== '') rel="{{ $linkRel }}" @endif @if($usesLightbox) data-basic-image-lightbox="true" @endif @foreach($linkAttributes as $attributeName => $attributeValue) {{ $attributeName }}="{{ e($attributeValue) }}" @endforeach>
				@if($imageUrl !== '')<img class="pb-image__img" src="{{ $imageUrl }}" alt="{{ $imageAlt }}" style="{{ $imageStyle }}" @if(isset($advanced['attributes']['data-pb-import-node'])) data-pb-import-node="{{ e($advanced['attributes']['data-pb-import-node']) }}" @endif>@else<div class="pb-image__empty" role="img" aria-label="Choose an image"><i class="far fa-image" aria-hidden="true"></i></div>@endif
			</a>
		@elseif($imageUrl !== '')
			<img class="pb-image__img" src="{{ $imageUrl }}" alt="{{ $imageAlt }}" style="{{ $imageStyle }}" @if(isset($advanced['attributes']['data-pb-import-node'])) data-pb-import-node="{{ e($advanced['attributes']['data-pb-import-node']) }}" @endif>
		@else

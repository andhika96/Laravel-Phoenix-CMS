<template>
	<component :is="rootTag" :id="domId" :class="rootClass" :style="containerStyle" v-bind="customAttrs">
		<div v-if="shouldRenderBackgroundVideo" class="pb-container-background-media pb-container-background-video" aria-hidden="true">
			<iframe v-if="backgroundVideoEmbedSrc" :src="backgroundVideoEmbedSrc" title="Background video" tabindex="-1" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen></iframe>
			<video v-else-if="backgroundVideoSource" :src="backgroundVideoSource" :poster="settings.bgVideoFallback || null" autoplay muted playsinline :loop="!isTruthy(settings.bgVideoPlayOnce)" @loadedmetadata="syncBackgroundVideo" @timeupdate="enforceBackgroundVideoEnd"></video>
		</div>
		<div v-if="shouldRenderBackgroundSlideshow" class="pb-container-background-media pb-container-background-slideshow" :class="'pb-container-background-slideshow--' + slideshowTransition" aria-hidden="true">
			<span v-for="(image, index) in slideshowImages" :key="image.id || image.url || index" class="pb-container-background-slideshow__slide" :class="{ 'is-active': index === slideshowIndex }" :style="backgroundSlideshowStyle(image, index)"></span>
		</div>
...
export default {
	...
	computed: {
		...
		rootClass() {
			...
			if (this.hasBackgroundMedia) classes.push('pb-has-background-media');
		...
		backgroundType() {
			return String(this.settings.bgType || 'none').toLowerCase();
		},
		slideshowImages() {
			return Array.isArray(this.settings.bgSlideshowImages)
				? this.settings.bgSlideshowImages.filter((image) => image && String(image.url || '').trim())
				: [];
		},
		slideshowTransition() {
			const transition = String(this.settings.bgSlideshowTransition || 'fade').toLowerCase();
			return ['fade', 'slide-right', 'slide-left', 'slide-up', 'slide-down'].includes(transition) ? transition : 'fade';
		},
		slideshowSize() {
			const size = this.responsiveSetting('bgSlideshowSize', this.settings.bgSlideshowSize || 'cover');
			return size === 'default' ? 'cover' : (size || 'cover');
		},
		slideshowPosition() {
			return this.responsiveSetting('bgSlideshowPosition', this.settings.bgSlideshowPosition || 'center center') || 'center center';
		},
		shouldRenderBackgroundVideo() {
			return this.backgroundType === 'video'
				&& Boolean(String(this.settings.bgVideoLink || '').trim())
				&& (this.responsiveDevice !== 'mobile' || this.isTruthy(this.settings.bgVideoPlayOnMobile));
		},
		shouldRenderBackgroundSlideshow() {
			return this.backgroundType === 'slideshow' && this.slideshowImages.length > 0;
		},
		hasBackgroundMedia() {
			return this.shouldRenderBackgroundVideo || this.shouldRenderBackgroundSlideshow;
		},
		backgroundVideoEmbedSrc() {
			return this.videoEmbedUrl(this.settings.bgVideoLink);
		},
		backgroundVideoSource() {
			return this.backgroundVideoEmbedSrc ? '' : String(this.settings.bgVideoLink || '').trim();
		},
		customAttrs() {
			const attrs = {};
			...
			if (this.importNodeKey) attrs['data-pb-import-node'] = this.importNodeKey;
			if (this.rootTag === 'a') {
		...
		containerStyle() {
			...
			if ((this.hasAnyShapeDivider(s) || this.hasBackgroundMedia) && (!style.position || style.position === 'static')) {
	...
	methods: {
		...
		shapeDividerSvgData(type, negative = false) {
			const shapes = window.PB_ELEMENTOR_V24_SHAPES || {};
		...
		backgroundLayer(settings, prefix, suffix = '') {
			...
			if (prefix === 'bg' && suffix === '' && type === 'video') {
				const fallback = String(settings.bgVideoFallback || '').trim();
				if (!fallback) return null;
				return { image: 'url("' + fallback + '")', size: 'cover', position: 'center center', repeat: 'no-repeat', attachment: 'scroll', blendMode: 'normal' };
			}
			if (prefix === 'bg' && suffix === '' && type === 'slideshow') {
				const firstImage = this.slideshowImages[0] && String(this.slideshowImages[0].url || '').trim();
				if (!firstImage) return null;
				return { image: 'url("' + firstImage + '")', size: this.slideshowSize, position: this.slideshowPosition, repeat: 'no-repeat', attachment: 'scroll', blendMode: 'normal' };
			}
			if (type === 'color') {
		...
		videoEmbedUrl(value) {
			...
			const start = Math.max(0, Number(this.settings.bgVideoStart) || 0);
			const end = Math.max(0, Number(this.settings.bgVideoEnd) || 0);
			const playOnce = this.isTruthy(this.settings.bgVideoPlayOnce);
			if (youtube) {
				const params = new URLSearchParams({ autoplay: '1', mute: '1', controls: '0', playsinline: '1', rel: '0' });
				if (start) params.set('start', String(start));
				if (end > start) params.set('end', String(end));
				if (!playOnce) { params.set('loop', '1'); params.set('playlist', youtube[1]); }
				return 'https://www.' + (this.isTruthy(this.settings.bgVideoPrivacy) ? 'youtube-nocookie.com' : 'youtube.com') + '/embed/' + youtube[1] + '?' + params.toString();
			}
			const vimeo = url.match(/vimeo\.com\/(?:video\/)?(\d+)/i);
			if (vimeo) {
				const params = new URLSearchParams({ autoplay: '1', muted: '1', background: '1', loop: playOnce ? '0' : '1', title: '0', byline: '0', portrait: '0' });
				if (start) params.set('#t', String(start) + 's');
				if (this.isTruthy(this.settings.bgVideoPrivacy)) params.set('dnt', '1');
				return 'https://player.vimeo.com/video/' + vimeo[1] + '?' + params.toString();
			}
			return '';
		},
		syncBackgroundVideo(event) {
			const video = event && event.target;
			if (!video) return;
			const start = Math.max(0, Number(this.settings.bgVideoStart) || 0);
			if (start && Number.isFinite(video.duration) && start < video.duration) video.currentTime = start;
			const play = video.play && video.play();
			if (play && typeof play.catch === 'function') play.catch(() => {});
		},
		enforceBackgroundVideoEnd(event) {
			const video = event && event.target;
			const end = Math.max(0, Number(this.settings.bgVideoEnd) || 0);
			if (!video || !end || video.currentTime < end) return;
			if (this.isTruthy(this.settings.bgVideoPlayOnce)) { video.pause(); return; }
			video.currentTime = Math.max(0, Number(this.settings.bgVideoStart) || 0);
		},
		backgroundSlideshowStyle(image, index) {
			const active = index === this.slideshowIndex;
			const nextIndex = (this.slideshowIndex + 1) % Math.max(1, this.slideshowImages.length);
			const duration = Math.max(100, Number(this.settings.bgSlideshowTransitionDuration) || 500);
			const transition = this.slideshowTransition;
			let transform = 'translate3d(0, 0, 0)';
			if (!active && transition === 'slide-right') transform = 'translate3d(-100%, 0, 0)';
			if (!active && transition === 'slide-left') transform = 'translate3d(100%, 0, 0)';
			if (!active && transition === 'slide-up') transform = 'translate3d(0, 100%, 0)';
			if (!active && transition === 'slide-down') transform = 'translate3d(0, -100%, 0)';
			if (active && this.isTruthy(this.settings.bgSlideshowKenBurns)) transform += ' scale(1.08)';
			return {
				backgroundImage: (!this.isTruthy(this.settings.bgSlideshowLazyload) || active || index === nextIndex) ? 'url("' + String(image.url).replace(/"/g, '\\"') + '")' : 'none',
				backgroundSize: this.slideshowSize,
				backgroundPosition: this.slideshowPosition,
				opacity: active ? 1 : 0,
				transform,
		...
		syncBackgroundMedia() {
			this.clearSlideshowTimer();
			if (!this.shouldRenderBackgroundSlideshow || this.slideshowImages.length < 2) { this.slideshowIndex = 0; return; }
			if (this.slideshowIndex >= this.slideshowImages.length) this.slideshowIndex = 0;
			const duration = Math.max(1000, Number(this.settings.bgSlideshowDuration) || 5000);
			this.slideshowTimer = window.setInterval(() => {
				if (this.slideshowIndex >= this.slideshowImages.length - 1) {
					if (!this.isTruthy(this.settings.bgSlideshowInfiniteLoop)) { this.clearSlideshowTimer(); return; }

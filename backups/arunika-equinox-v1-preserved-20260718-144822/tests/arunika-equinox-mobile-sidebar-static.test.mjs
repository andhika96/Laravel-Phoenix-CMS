...
test('Arunika Equinox removes the desktop frame on mobile without changing the drawer width', () => {
  assert.match(
    css,
  ...
  assert.match(
    css,
    /@media \(max-width:\s*768px\)[\s\S]*?\.ph-theme-arunika-equinox\s+\.ph-sidebar,\s*\.ph-theme-arunika-equinox\s+\.ph-sidebar\.ph-expanded\s*\{[^}]*width:\s*256px;[^}]*background:\s*var\(--ph-equinox-sidebar-gradient\);/s,
  );
  assert.match(
    css,

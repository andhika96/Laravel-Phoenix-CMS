...
test('Arunika V3 profile summary stays clean while Logout keeps its divider', () => {
  assert.doesNotMatch(
    layout,
  ...
  const sectionRule = css.match(
    /\.ph-theme-arunika-v3\s+\.ph-profile-menu-section\s*\{([^}]*)\}/s,
  );
  ...
  assert.match(
    css,
    /\.ph-theme-arunika-v3\s+\.ph-profile-menu-footer\s*\{[^}]*border-top:\s*1px solid var\(--ph-v3-border\);/s,
  );
});

test('Arunika V3 profile menu rows keep hover surfaces visually separated', () => {
  assert.match(
    css,
    /\.ph-theme-arunika-v3\s+\.ph-profile-menu-section\s*>\s*\.dropdown-item\s*\{[^}]*margin:\s*3px 0;/s,
  );
  assert.match(
    css,
    /\.ph-theme-arunika-v3\s+\.ph-profile-menu-section\s*>\s*\.dropdown-item:first-child\s*\{[^}]*margin-top:\s*0;/s,
  );
  assert.match(
    css,
    /\.ph-theme-arunika-v3\s+\.ph-profile-menu-section\s*>\s*\.dropdown-item:last-child\s*\{[^}]*margin-bottom:\s*0;/s,
  );
});

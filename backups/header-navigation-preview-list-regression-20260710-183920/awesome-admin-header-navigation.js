const editor = document.getElementById('headerNavigationEditor');
		...
		async function loadPreviewMenus(showError = true) {
			...
			try {
				...
				if (showError) {
					alert('Menu endpoint belum bisa dibaca, jadi preview tetap memakai sample menu.');
				}
			} finally {
				button.disabled = false;
		...
		document.getElementById('loadMenu').addEventListener('click', () => loadPreviewMenus());

		updateAll();
		loadPreviewMenus(false);

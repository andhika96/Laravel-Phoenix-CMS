...
class Awesome_Admin_Menu_FE_Controller extends Controller
{
	...
	{
		return view('awesome_admin.menu.frontend.awesome_admin_menu_fe');
	}
	...
	public function create_submenu(MenuRequest $request)
	{
		...
		try
		{
			if ($request->validated())
			{
				...
				$role = Sub_Menu_FE_JSON::create(['parent_code' => $request->input('parent_code'), 'parent_name' => $getParentName]);
	...
	{
		return view('awesome_admin.menu.frontend.awesome_admin_menu_submenu_fe');
	}

	public function submenu_detail(string $idOrSlug)
	{
		$getDetailMenu = Sub_Menu_FE_JSON::find($idOrSlug);

		if ( ! $getDetailMenu)
		{			
			$getDetailMenu = Sub_Menu_FE_JSON::where('parent_code', $idOrSlug)->first();
		}

		if ($getDetailMenu)
		{
			return view('awesome_admin.menu.frontend.awesome_admin_menu_submenu_detail_fe', ['data' => $getDetailMenu, 'idOrSlug' => $idOrSlug]);
		}
	...
	public function parentmenu()
	{
		...
		return view('awesome_admin.menu.frontend.awesome_admin_parent_menu_fe', ['categorymenu' => $getListCategoryMenu]);
	}

	public function categorymenu()
	{
		return view('awesome_admin.menu.frontend.awesome_admin_category_menu_fe');
	}
	...
	public function update_parentmenu(Request $request)
	{
		...
		try 
		{
			$getDetailMenuParentJSON 	= Parent_Menu_FE_JSON::find(1);
	...
	public function update_submenu_detail(string $idOrSlug)
	{
		...
		try 
		{
			$getDetailMenuParentJSON 	= Sub_Menu_FE_JSON::where('parent_code', $idOrSlug)->first();
			$getJSONDecodeMenuVars 		= json_decode(request()->input('menu_vars'), true);
	...
	public function update_categorymenu(Request $request)
	{
		...
		try 
		{
			$getDetailMenuParentJSON 	= Category_Menu_FE_JSON::find(1);
			$getJSONDecodeMenuVars 		= json_decode($request->input('menu_vars'), true);
	...
	public function delete_parentmenu(Request $request)
	{
		...
		try 
		{
			...
			Parent_Menu_FE_Dropdown_Config::where('menu_page', 'awesome_admin')->where('parent_code', $request->input('menu_code'))->delete();
	...
	public function delete_submenu(Request $request)
	{
		...
		try
		{
			$detail_data = Sub_Menu_FE_JSON::where('id', $request->route('idOrSlug'))->first();
	...
	{
		$menu_categorylist = Category_Menu_FE_JSON::where('menu_page', 'awesome_admin')->get();
	...
	{
		$menu_parentlist 	= Parent_Menu_FE_JSON::where('menu_page', 'awesome_admin')->get();
		$dropdownConfigs 	= Parent_Menu_FE_Dropdown_Config::where('menu_page', 'awesome_admin')->get()->keyBy('parent_code');
		$menu_vars_decode 	= [];
		$menu_vars_decode2 	= [];
		...
		if (is_array($menu_vars_decode) && count($menu_vars_decode) > 0)
		{
			...
			{
				$menu_vars_decode2[$key] = $value;
				$parentCode = isset($menu_vars_decode2[$key]['parent_code']) ? $menu_vars_decode2[$key]['parent_code'] : '';
				$dropdownConfig = ($parentCode !== '' && isset($dropdownConfigs[$parentCode])) ? $dropdownConfigs[$parentCode] : null;
				...
				$menu_vars_decode2[$key]['dropdown_config'] = $this->formatDropdownConfig($dropdownConfig);
				$menu_vars_decode2[$key]['dropdown_type'] = $menu_vars_decode2[$key]['dropdown_config']['dropdown_type'];
			}
		}
		else
		{
	...
	public function listDataParentMenuDropdown(Request $request)
	{
		$menuPage 	= $request->input('menu_page', 'awesome_admin');
		$parentCode = $request->route('parentCode');
		$parentMenu = $this->findFrontendParentMenuByCode($parentCode, $menuPage);
		...
		$dropdownConfig = Parent_Menu_FE_Dropdown_Config::where('menu_page', $menuPage)->where('parent_code', $parentCode)->first();
		...
		[
			...
			[
				...
				'submenus' 			=> $this->getFrontendSubmenuVars($parentCode),
				'dropdown_config' 	=> $this->formatDropdownConfig($dropdownConfig)
			]
		]);
	}

	public function update_parentmenu_dropdown(Request $request)
	{
		DB::beginTransaction();
		...
		try
		{
			...
			if ( ! is_string($parentCode) || trim($parentCode) == '')
			{
				throw new \Exception(t('Parent menu code is required'));
			}
			...
			if ( ! $parentMenu)
			{
				throw new \Exception(t('Parent menu not found'));
			}
			...
			[
				'dropdown_type' => $normalized['dropdown_type'],
				'mega_layout' => $normalized['mega_layout'],
	...
	protected function formatDropdownConfig($dropdownConfig = null): array
	{
		if ($dropdownConfig)
		{
	...
	protected function getFrontendParentMenuVars(string $menuPage = 'awesome_admin'): array
	{
		...
		if ( ! $parentMenu || $parentMenu->menu_vars === 'null')
		{
			return [];
		}

		$decoded = json_decode($parentMenu->menu_vars, true);

		return is_array($decoded) ? $decoded : [];
	}

	protected function findFrontendParentMenuByCode(string $parentCode, string $menuPage = 'awesome_admin'): ?array
	{
		foreach ($this->getFrontendParentMenuVars($menuPage) as $value)
		{
			if (isset($value['parent_code']) && $value['parent_code'] === $parentCode)
			{
				if (isset($value['parent_icon_path']) && $value['parent_icon_path'] !== '')
				{
					$value['parent_icon_url'] = url($this->getImageURL($value['parent_icon_path']));
				}
				else
				{
					$value['parent_icon_url'] = '';
					$value['parent_icon_path'] = '';
				}
	...
	protected function getFrontendSubmenuVars(string $parentCode): array
	{
		$submenuList = Sub_Menu_FE_JSON::where('parent_code', $parentCode)->first();

		if ( ! $submenuList || $submenuList->menu_vars === 'null')
		{
			return [];
		}

		$decoded = json_decode($submenuList->menu_vars, true);
		...
		{
			if (isset($decoded[$key]['submenu_icon_path']) && $decoded[$key]['submenu_icon_path'] !== '')
			{
				$decoded[$key]['submenu_icon_url'] = url($this->getImageURL($decoded[$key]['submenu_icon_path'], 'icons/sub_menu'));
			}
			else
			{
	...
	{
		$menulist = Sub_Menu_FE_JSON::where('parent_code', $request->route('idOrSlug'))->get();
	...
	{
		$parentMenu = Parent_Menu_FE_JSON::where('menu_page', 'awesome_admin')->first();
	...
	{
		$subMenu = Sub_Menu_FE_JSON::where('parent_code', $request->input('parent_code'))->first();

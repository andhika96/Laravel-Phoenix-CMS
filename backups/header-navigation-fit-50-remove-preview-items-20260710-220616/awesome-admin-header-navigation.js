const editor = document.getElementById('headerNavigationEditor');
		...
		function updateMenuCount() {
			const count = Math.max(1, Number(controls.menuCount.value || 5));
		...
		function updatePreviewScale() {
			...
			const scale = Math.min(1, frameWidth / viewport.width, frameHeight / viewport.height);
		...
		[
			...
			'headerHeight', 'headerHeightUnit', 'menuCount'
		].forEach(id => {
		...
		async function loadPreviewMenus() {
			try {
				...
				controls.menuCount.value = Math.max(1, mapped.length);
			} catch (error) {

...
@section('title')
	{{ t('Manage Frontend Parent Menus') }}
@endsection

@section('content')
	<div id="ph-app-manage-parentmenu">
		<div class="mb-3">
			{{ Breadcrumbs::render('awesome_admin.menu.fe.parent_menu') }}
		</div>

		<div class="ph-content rounded p-3 mb-3">
			<div class="row g-3">
				<div class="col-md-6 d-flex align-items-center">
					<h4 class="mb-0">{{ t('Manage Frontend Parent Menus') }}</h4>
				</div>
			</div>
		</div>
		...
		<div class="row g-4">
			<div class="col-md-6">
				<div class="ph-content rounded p-4 ph-fetch-listdata-parentmenu" id="ph-fetch-listdata-parentmenu" data-url="{{ url('awesome_admin/menu/fe/listdata/parentmenu') }}" data-dropdown-detail-url="{{ url('awesome_admin/menu/fe/listdata/parentmenu/dropdown') }}" data-dropdown-update-url="{{ route('cms.admin.awesome_admin.menu.fe.update_parentmenu.dropdown') }}">
					...
					<div v-else class="ph-data-load-content" id="ph-form-manage-parentmenu" style="display: none">
						<form action="{{ route('cms.admin.awesome_admin.menu.fe.update_parentmenu') }}" method="post" auto-refresh="false" auto-lock-button="false" custom-button-value="Save" ref="formHTML" @submit.prevent="submitData">
												...
												<div class="row g-1">
													<div class="col-md-6 mb-3 mb-md-0 d-flex align-items-center">
														...
														<span class="ms-2">
															...
															@{{ element.parent_name }}
															<span v-if="element.dropdown_type !== undefined && element.dropdown_type !== '' && element.dropdown_type !== 'none'" class="badge rounded-pill text-bg-light border ms-2 fw-semibold">
																@{{ dropdownTypeLabel(element.dropdown_type) }}
															</span>
														</span>
													</div>

													<div class="col-md-6 d-flex align-items-center justify-content-end">
														<a href="javascript:void(0)" class="btn btn-link text-danger font-size-inherit" v-on:click="openDropdownModalParentMenu(element, index)">{{ t('Dropdown') }}</a> |
			...
			<div class="col-md-6">
				<div class="ph-content rounded p-4">
					...
					<div id="ph-submit-data-rp" class="ph-fetch-listdata-roles" data-url="{{ url('awesome_admin/menu/fe/listdata/roles') }}">
						<form action="{{ route('cms.admin.awesome_admin.menu.fe.checkParentMenuLink') }}" method="post" auto-refresh="false" auto-lock-button="false" custom-button-value="{{ t('Create') }}" ref="formHTMLCreateNewMenu" @submit.prevent="addParentMenu">
							<div class="ph-notice" v-cloak>
								<div aria-live="polite" aria-atomic="true" class="position-relative">
									<div class="toast-container position-fixed top-0 end-0 p-3">
										<div :class="'toast ph-notice-toast ph-callout-no-border '+responseStatusToast" role="alert" aria-live="assertive" aria-atomic="true" data-bs-delay="3000">
							...
							<div class="row g-3 ph-fetch-listdata-routes" data-url="{{ url('awesome_admin/menu/fe/listdata/routes') }}">
								...
								<div v-if="responseNewDataParentMenu.is_for_parent_menu == '' || responseNewDataParentMenu.is_for_parent_menu == 'single'" class="col-12 form-group">
									...
									<div v-else-if="responseNewDataParentMenu.parent_type == 'page'" class="col-12 form-group">
										...
										<select name="parent_link" class="form-select font-size-inherit" aria-label="Select Parent Link" v-on:change="selectParentLink($event)">
											<option value="">{{ t('Select') }}</option>
								...
								<div class="col-12 form-group">
									...
									<select name="parent_icon_type" class="form-select font-size-inherit" aria-label="Select Parent Menu Icon Type" v-on:change="selectParentMenuIconType($event)">
										<option value="">{{ t('Select') }}</option>
										<option value="upload_file">{{ t('Upload File') }}</option>
		...
		<!-- Modal Manage Dropdown Parent Menu -->
		<Teleport to="body">
			<div class="modal ph-modal-dropdown fade" id="modalDropdownParentMenu" tabindex="-1" aria-labelledby="modalDropdownParentMenuLabel" aria-hidden="true">
				<div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
					<div class="modal-content" id="ph-form-dropdown-parentmenu">
						<form action="{{ route('cms.admin.awesome_admin.menu.fe.update_parentmenu.dropdown') }}" method="post" auto-refresh="false" auto-lock-button="false" custom-button-value="{{ t('Save Dropdown') }}" ref="formHTMLDropdown" @submit.prevent="submitDataDropdown($event)">
							...
							<div class="modal-header px-4 py-3">
								<div class="d-flex align-items-center gap-3">
									<div class="ph-dropdown-modal-icon">
										<i class="fad fa-layer-group fa-fw"></i>
									</div>

									<div>
										<h5 class="modal-title mb-1" id="modalDropdownParentMenuLabel">{{ t('Manage Dropdown Menu') }}</h5>
										<div class="text-muted small">
											@{{ dropdownParentMenu.parent_name || 'Parent Menu' }}
										</div>
									</div>
								</div>

								<button type="button" class="btn-close" aria-label="Close" v-on:click="closeDropdownModalParentMenu"></button>
							</div>

							<div class="modal-body p-0">
								<div v-if="dropdownModalLoading" class="text-center p-5">
									<div class="spinner-border text-primary mb-3" role="status">
										<span class="sr-only"></span>
									</div>

									<div class="h6 mb-0">{{ t('Loading') }} ...</div>
								</div>
								...
								<div v-else class="row g-0 ph-dropdown-builder">
									<div class="col-lg-5 ph-dropdown-builder-sidebar">
										<div class="ph-dropdown-section">
											...
											<div class="btn-group w-100 ph-segmented-control" role="group" aria-label="Dropdown Type">
												...
												<label class="btn btn-outline-primary" for="dropdownTypeNone">{{ t('None') }}</label>

												<input type="radio" class="btn-check" id="dropdownTypeBootstrap" value="bootstrap" v-model="dropdownConfig.dropdown_type">
												<label class="btn btn-outline-primary" for="dropdownTypeBootstrap">Bootstrap 5</label>

												<input type="radio" class="btn-check" id="dropdownTypeMega" value="mega" v-model="dropdownConfig.dropdown_type">
												<label class="btn btn-outline-primary" for="dropdownTypeMega">{{ t('Mega Menu') }}</label>
											</div>
										</div>
										...
										<div v-if="dropdownConfig.dropdown_type !== 'none'" class="ph-dropdown-section">
											...
											<div class="row g-3">
												...
												<div class="col-md-6" v-if="dropdownConfig.dropdown_type == 'mega' && dropdownConfig.config_json.dropdown.width_mode == 'custom'">
												...
												<div class="col-md-6" v-if="dropdownConfig.dropdown_type == 'bootstrap' || dropdownConfig.config_json.dropdown.width_mode == 'custom'">
									...
									<div class="col-lg-7 ph-dropdown-preview">
										<div class="ph-dropdown-preview-header">
											<div>
												<div class="ph-dropdown-section-title mb-1">{{ t('Live Preview') }}</div>
												<div class="text-muted small">@{{ limitedDropdownSubmenus().length }} {{ t('submenu items') }}</div>
											</div>

											<span class="badge rounded-pill text-bg-light border">@{{ dropdownTypeLabel(dropdownConfig.dropdown_type) }}</span>
										</div>

										<div class="ph-dropdown-preview-canvas">
											<div class="ph-dropdown-preview-stage">
												...
												<div v-if="dropdownConfig.dropdown_type == 'none'" class="ph-dropdown-empty-state">
													<i class="fad fa-ban fa-fw mb-2"></i>
												...
												<div v-else :class="'ph-preview-dropdown-shell is-'+dropdownConfig.dropdown_type+' width-'+(dropdownConfig.config_json.dropdown.width_mode || 'container')" :style="dropdownPreviewShellStyle()">
													...
													<div v-if="dropdownConfig.dropdown_type == 'bootstrap'" class="ph-preview-bootstrap-dropdown" :style="bootstrapPreviewStyle()">
														<a v-for="(item, index) in limitedDropdownSubmenus()" href="javascript:void(0)" class="dropdown-item ph-preview-bootstrap-item">
															<span v-if="item.submenu_icon_url !== undefined && item.submenu_icon_url !== ''" class="ph-preview-bootstrap-icon-slot">
																<img :src="item.submenu_icon_url" alt="">
															</span>
															<span class="ph-preview-bootstrap-label">@{{ item.submenu_name || 'Submenu' }}</span>
														</a>

														<div v-if="limitedDropdownSubmenus().length < 1" class="dropdown-item text-muted">{{ t('No submenu') }}</div>
													</div>
													...
													<div v-else :class="'ph-preview-mega-dropdown ph-preview-mega-'+dropdownConfig.mega_layout">
														...
														<div class="ph-preview-mega-grid" :style="megaPreviewGridStyle()">
															...
															<div v-if="limitedDropdownSubmenus().length < 1" class="ph-dropdown-empty-state py-4">{{ t('No submenu') }}</div>
														</div>
													</div>
												</div>
											</div>
							...
							<div class="modal-footer px-4 py-3">
								<input type="hidden" name="menu_page" value="awesome_admin">
								<input type="hidden" name="parent_code" :value="dropdownParentMenu.parent_code">

								<button type="button" class="btn btn-secondary font-size-inherit" v-on:click="closeDropdownModalParentMenu">{{ t('Cancel') }}</button>
								<button type="submit" class="btn ph-btn-theme btn-submit-data font-size-inherit">{{ t('Save Dropdown') }}</button>
							</div>
						</form>
					</div>
				</div>
			...
			<div class="modal ph-modal-delete fade" id="modalDeleteParentMenu" tabindex="-1" aria-labelledby="modalDeleteParentMenuLabel" aria-hidden="true">
				<div class="modal-dialog ph-modal-dialog modal-dialog-centered">
					<div class="modal-content" id="ph-form-delete-parentmenu">
						<form action="{{ url('awesome_admin/menu/fe/delete_parentmenu') }}" method="post" auto-refresh="true" auto-lock-button="true" custom-button-value="{{ t('Yes, Delete') }}" ref="formHTMLdelete" @submit="submitDataDelete($event)">
...
@pushonce('js')
	<script src="{{ url('assets/js/vue3/manage_menu/fe/vueV3-manage-frontend-parent-menu-2026.js?v=').time() }}"></script>
@endpushonce

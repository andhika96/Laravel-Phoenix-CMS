...
- Earlier [P1] shell proportion mismatch: the desktop sidebar was `256px`, moving the canvas much farther right than the source. Fix: scoped Arunika V3 desktop expanded width is now `160px`; the existing mobile drawer retains `256px`, and collapsed desktop remains `76px`.
...
- Earlier [P2] density mismatch: the header was `60px`, search was `320 x 36px`, and the content top inset was `10px`. Fix: header is `52px`, desktop search is `220 x 32px`, and the content shell is `8px 8px 18px`.
...
- Fonts and typography: the existing Nunito family remains intact; sidebar labels are compacted to `12px`, profile name/role remain visible at `852px`, and truncation protects longer dynamic menu names.
- Spacing and layout rhythm: source-aligned `160px` sidebar, `52px` header, `220px` search, `8px` outer canvas gutter, `12px` canvas radius, and `8px` content top inset are verified from runtime DOM measurements.
...
- Desktop collapse changed the measured sidebar from `160px` to `76px` while the toggle remained in the header; reloading after expansion restored `160px`.
...
- Focused Arunika V3 Node regressions: `12 passed`, `0 failed`.

...
test('Arunika Equinox V2 recreates the edge-to-edge reference shell', () => {
  ...
  assert.match(
    css,
    /\.ph-theme-arunika-equinox\s+\.ph-header-profile-menu::before,\s*\.ph-theme-arunika-equinox\s+\.ph-header-profile-menu::after\s*\{[^}]*left:\s*50%\s*!important;[^}]*right:\s*auto\s*!important;[^}]*transform:\s*translateX\(-50%\);/s,
  );
  assert.match(
    css,

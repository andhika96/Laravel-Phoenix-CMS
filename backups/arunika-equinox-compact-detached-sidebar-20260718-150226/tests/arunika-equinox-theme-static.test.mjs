...
test('Arunika Equinox V2 recreates the edge-to-edge reference shell', () => {
  ...
  assert.match(
    css,
    /\.ph-theme-arunika-equinox\s+\.ph-app-shell\s*\{[^}]*width:\s*100%\s*!important;[^}]*height:\s*100%\s*!important;[^}]*margin:\s*0;[^}]*border-radius:\s*30px;[^}]*overflow:\s*hidden;/s,
  );
  assert.match(
    css,
    /\.ph-theme-arunika-equinox\s+\.ph-sidebar,\s*\.ph-theme-arunika-equinox\s+\.ph-sidebar\.ph-expanded\s*\{[^}]*background:\s*linear-gradient\(180deg,\s*var\(--ph-equinox-sidebar-start\)[^}]*border-right:\s*1px solid var\(--ph-equinox-border\);[^}]*border-radius:\s*0 30px 30px 0;/s,
  );
  assert.match(
    css,
    /\.ph-theme-arunika-equinox\s+\.ph-layout-right\s*\{[^}]*margin:\s*0;[^}]*background:\s*var\(--ph-equinox-main-gradient\);/s,
  );
  assert.match(
    css,
    /\.ph-theme-arunika-equinox\s+\.ph-top-bar\s*\{[^}]*height:\s*88px;[^}]*padding:\s*0 40px 0 42px;[^}]*background:\s*transparent;[^}]*border-bottom:\s*0;/s,
  );
  assert.match(
    css,
    /\.ph-theme-arunika-equinox\s+\.ph-search-container\s*\{[^}]*width:\s*min\(360px,\s*34vw\);[^}]*display:\s*flex\s*!important;[^}]*border:\s*1px solid var\(--ph-equinox-border\);[^}]*border-radius:\s*999px;/s,
  );
  assert.match(
    css,
    /\.ph-theme-arunika-equinox\s+\.ph-header-user-card\s*\{[^}]*display:\s*grid;[^}]*grid-template-columns:\s*40px minmax\(0,\s*1fr\);[^}]*background:\s*transparent;/s,
  );
});

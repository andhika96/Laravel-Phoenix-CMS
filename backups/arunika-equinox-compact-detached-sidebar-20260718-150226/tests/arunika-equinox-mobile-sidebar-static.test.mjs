...
test('Arunika Equinox V2 preserves the reference shell as an accessible mobile drawer', () => {
  ...
  assert.match(
    css,
    /@media \(max-width:\s*768px\)[\s\S]*?\.ph-theme-arunika-equinox\s+\.ph-sidebar,\s*\.ph-theme-arunika-equinox\s+\.ph-sidebar\.ph-expanded\s*\{[^}]*width:\s*min\(280px,\s*calc\(100vw - 42px\)\);[^}]*background:\s*linear-gradient\(180deg,\s*var\(--ph-equinox-sidebar-start\)/s,
  );
  assert.match(
    css,

...
const concepts = [
    ...
    {
        code: 'arunika_canvas',
        displayName: 'Arunika Canvas',
        preview: 'arunika-canvas-theme-preview.png',
    },
];
...
test('the database identity migration is reversible and collision-safe', () => {
    const migrationPath = path.join(
        root,
        'database/migrations/2026_07_17_131400_rename_arunika_theme_identifiers.php',
    );

    assert.equal(existsSync(migrationPath), true, 'Missing the Arunika identity migration');
    ...
    for (const concept of concepts) {
        assert.match(migration, new RegExp(`'to_code'\\s*=>\\s*'${concept.code}'`));
        assert.match(migration, new RegExp(`'to_name'\\s*=>\\s*'${concept.displayName}'`));
    }
});

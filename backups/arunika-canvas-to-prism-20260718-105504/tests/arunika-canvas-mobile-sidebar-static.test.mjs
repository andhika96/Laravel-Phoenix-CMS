...
assert.match(
	...
	'mobile Canvas drawer toggles should not broadcast a synthetic layout resize',
);
assert.match(
	css,
...
assert.match(
	css,
	/\.ph-theme-arunika-canvas \.ph-mobile-sidebar-trigger\s*\{[^}]*padding:\s*0;[^}]*border:\s*0;[^}]*border-radius:\s*0;[^}]*background:\s*transparent;[^}]*box-shadow:\s*none;/s,
	'mobile Canvas navigation should expose only the icon without a button shell',
);
assert.match(
	css,
	/\.ph-theme-arunika-canvas \.ph-sidebar:not\(\.ph-expanded\) \+ \.ph-layout-right \.ph-mobile-sidebar-trigger \.ph-sidebar-toggle-chevron\s*\{[^}]*transform:\s*rotate\(180deg\);/s,
	'closed mobile Canvas navigation should show the desktop panel chevron as an arrow right',
);
assert.match(
	css,

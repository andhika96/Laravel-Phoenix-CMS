...
const layout = readFileSync(
  path.join(root, 'resources/views/themes/arunika_prism/cms/cms_layout.blade.php'),
  'utf8',
);
const css = readFileSync(
  path.join(root, 'public/assets/css/themes/arunika_prism/arunika_prism.css'),
  'utf8',
);

test('Arunika Prism keeps notifications mounted but hidden before the admin shortcut', () => {
  ...
  assert.ok(headerActionsIndex >= 0, 'Missing Prism header actions container.');
  assert.ok(hiddenNotificationIndex > headerActionsIndex, 'Notification must use the Prism hidden wrapper.');
...
test('Arunika Prism renders an admin-only user-secret shortcut to Awesome Admin', () => {
  assert.match(
    layout,
...
test('Arunika Prism header actions use proportional button and glyph sizes', () => {
  assert.match(
    css,
    /\.ph-theme-arunika-prism\s+\.ph-btn-action-icon\s*\{[^}]*width:\s*34px;[^}]*height:\s*34px;[^}]*font-size:\s*16px;/s,
  );
  assert.match(
    css,
    /\.ph-theme-arunika-prism\s+\.ph-header-awesome-admin\s+i,\s*\.ph-theme-arunika-prism\s+\.ph-header-notification\s+#cmsNotifBell\s+i\s*\{[^}]*font-size:\s*16px;/s,
  );
  assert.match(
    css,
    /\.ph-theme-arunika-prism\s+\.ph-header-notification\.is-hidden\s*\{[^}]*display:\s*none\s*!important;/s,
  );
});

test('Arunika Prism profile dropdown preserves dynamic account actions in the reference card order', () => {
...
test('Arunika Prism profile dropdown wires the Dark Mode switch and collapsible theme colors', () => {
  assert.match(
    layout,
...
test('Arunika Prism profile dropdown uses the compact reference card styling', () => {
  assert.match(
    css,
    /\.ph-theme-arunika-prism\s+\.ph-header-profile-menu\s*\{[^}]*width:\s*240px;[^}]*min-width:\s*240px;[^}]*padding:\s*6px;[^}]*border:\s*1px solid var\(--ph-prism-border\);[^}]*border-radius:\s*14px;[^}]*box-shadow:/s,
  );
  assert.match(
    css,
    /\.ph-theme-arunika-prism\s+\.ph-profile-menu-user\s*\{[^}]*min-height:\s*58px;[^}]*padding:\s*8px;[^}]*border-radius:\s*10px;/s,
  );
  assert.match(
    css,
    /\.ph-theme-arunika-prism\s+\.ph-header-profile-menu\s+\.dropdown-item\s*\{[^}]*min-height:\s*40px;[^}]*padding:\s*0 10px;[^}]*border-radius:\s*8px;/s,
  );
  assert.match(
    css,
    /\.ph-theme-arunika-prism\s+\.ph-profile-switch\s*\{[^}]*width:\s*32px;[^}]*height:\s*18px;/s,
  );
  assert.match(
    css,
    /\.ph-theme-arunika-prism\s+\.ph-profile-theme-toggle\.is-dark\s+\.ph-profile-switch\s*\{[^}]*background:\s*var\(--ph-theme-primary\);/s,
  );
  assert.match(
    css,
    /\.ph-theme-arunika-prism\s+\.ph-profile-color-toggle:not\(\.collapsed\)\s+\.ph-profile-color-chevron\s*\{[^}]*transform:\s*rotate\(90deg\);/s,
  );
});

test('Arunika Prism profile summary stays clean while Logout keeps its divider', () => {
  assert.doesNotMatch(
    layout,
  ...
  const sectionRule = css.match(
    /\.ph-theme-arunika-prism\s+\.ph-profile-menu-section\s*\{([^}]*)\}/s,
  );
  ...
  assert.match(
    css,
    /\.ph-theme-arunika-prism\s+\.ph-profile-menu-footer\s*\{[^}]*border-top:\s*1px solid var\(--ph-prism-border\);/s,
  );
});

test('Arunika Prism profile menu rows keep hover surfaces visually separated', () => {
  assert.match(
    css,
    /\.ph-theme-arunika-prism\s+\.ph-profile-menu-section\s*>\s*\.dropdown-item\s*\{[^}]*margin:\s*3px 0;/s,
  );
  assert.match(
    css,
    /\.ph-theme-arunika-prism\s+\.ph-profile-menu-section\s*>\s*\.dropdown-item:first-child\s*\{[^}]*margin-top:\s*0;/s,
  );
  assert.match(
    css,
    /\.ph-theme-arunika-prism\s+\.ph-profile-menu-section\s*>\s*\.dropdown-item:last-child\s*\{[^}]*margin-bottom:\s*0;/s,
  );
});

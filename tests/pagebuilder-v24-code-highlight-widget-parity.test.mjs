...
test('Code Highlight registers as a Pro widget with complete defaults and normalization', async () => {
  const definitionPath = join(rootDir, 'resources/pagebuilder_elementor_v24/modules/widgets/pro/code-highlight/definition.js');
  ...
  configureSingleModule(context, await source('resources/pagebuilder_elementor_v24/modules/widgets/pro/code-highlight/module.json'));
  vm.runInNewContext(await source('resources/pagebuilder_elementor_v24/modules/widgets/pro/code-highlight/definition.js'), context);

  const definition = context.window.PageBuilderElementorV24Widgets.get('code_highlight');
  ...
  const settingsSource = await source('resources/pagebuilder_elementor_v24/modules/widgets/pro/code-highlight/Settings.vue');
...
test('Code Highlight settings expose the complete Content, Style, and Advanced mapping', async () => {
  const component = await loadSfc('resources/pagebuilder_elementor_v24/modules/widgets/pro/code-highlight/Settings.vue');
...
test('Code Highlight canvas renders escaped syntax lines, highlight state, theme, and copy contract', async () => {
  const component = await loadSfc('resources/pagebuilder_elementor_v24/modules/widgets/pro/code-highlight/Canvas.vue');
...
test('Code Highlight is wired through the v2.4 registry, labels, Advanced gate, Blade, and runtime', async () => {
  const manifest = JSON.parse(await source('resources/pagebuilder_elementor_v24/modules/widgets/pro/code-highlight/module.json'));
  const blade = await source('resources/pagebuilder_elementor_v24/modules/widgets/pro/code-highlight/frontend.blade.php');
  const runtime = await source('resources/pagebuilder_elementor_v24/modules/widgets/pro/code-highlight/runtime.js');

  assert.deepEqual([manifest.type, manifest.label, manifest.category, manifest.icon], ['code_highlight', 'Code Highlight', 'pro', 'fas fa-code']);

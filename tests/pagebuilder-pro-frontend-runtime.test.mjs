...
await import("../public/js/pagebuilder_elementor/frontend-runtime.js");
const runtime = window.PageBuilderElementorRuntime;
...
test("actual Pro Form runtime waits for the server before success and redirect", async () => {
    ...
    const root = rootBase({
        "data-pro-config": JSON.stringify({
            ...
            submitUrl: "/pagebuilder-elementor/form/contact-page/form-contact",
        }),
    });
    ...
    assert.equal(fetchCalls[0].url, "/pagebuilder-elementor/form/contact-page/form-contact");

...
const containerSettings = fs.readFileSync(path.resolve('resources/pagebuilder_elementor_v24/modules/layout/container/Settings.vue'), 'utf8');
const capabilityPrelude = pageBuilderV24CapabilityPrelude();

function loadChildContainerHelpers() {
    const block = app.match(/\/\/ V24_CHILD_CONTAINER_HELPERS_START([\s\S]*?)\/\/ V24_CHILD_CONTAINER_HELPERS_END/)?.[1];
    assert.ok(block, 'child Container helpers should exist');
    const context = { structuredClone: globalThis.structuredClone };
    vm.runInNewContext(`${capabilityPrelude}\n${block}\nthis.api = { createLegacyContainerMigrationState, claimCanonicalNodeId, migrateLegacyContainerColumns, createChildContainerNode, presetChildContainers, appendChildContainer, gridSlotCount, reconcileGridColumns, gridColumnHasContent, isGridColumnLockedSequentially, moveLooseGridChildrenIntoColumns, restoreGridColumnsAsFlexChildren: typeof restoreGridColumnsAsFlexChildren === 'function' ? restoreGridColumnsAsFlexChildren : null, nodeHasNestedContent: typeof nodeHasNestedContent === 'function' ? nodeHasNestedContent : null, nestedOwnerBlocksAncestorCapture: typeof nestedOwnerBlocksAncestorCapture === 'function' ? nestedOwnerBlocksAncestorCapture : null, canMoveNodeIntoContainer, responsiveContainerWidth, applyAdjacentContainerWidths, canonicalLayoutForSave };`, context);
    return context.api;
}
...
test('layout content detection only requires confirmation when nested content exists', () => {
    const api = loadChildContainerHelpers();
    assert.equal(typeof api.nodeHasNestedContent, 'function');

    assert.equal(api.nodeHasNestedContent({ type: 'container', children: [] }), false);
    assert.equal(api.nodeHasNestedContent({ type: 'container', children: [{ id: 'heading-1', type: 'heading' }] }), true);
    assert.equal(api.nodeHasNestedContent({
        type: 'grid',
        columns: [{ id: 'cell-1', children: [] }, { id: 'cell-2', children: [{ id: 'button-1', type: 'button' }] }],
    }), true);
});

test('Flexbox Child Containers sidebar exposes selectable rows with a delete action', () => {
    const childSection = containerSettings.match(/<details class="pb-collapsible"[^>]*>\s*<summary>Child Containers<\/summary>[\s\S]*?<\/details>/)?.[0] || '';

    assert.match(childSection, /v-for="\(child, index\) in editor\.childContainersFor\(node\)"/);
    assert.match(childSection, /editor\.selectNode\(child, \{ revealPanel: true \}\)/);
    assert.match(childSection, /editor\.removeNode\(child\.id\)/);
    assert.match(childSection, /fa-trash/);
});
...
test('Flexbox child Containers unwrap one level into conceptual Grid cells', () => {
    const api = loadChildContainerHelpers();
    const emptyColumn = {
        ...containerFactory(),
        id: 'flex-left',
        settings: { ...containerFactory().settings, containerWidth: '50%', cssClass: 'left-column' },
    };
    const nestedContainer = {
        ...containerFactory(),
        id: 'nested-container',
        children: [{ id: 'button-1', type: 'button', settings: {} }],
    };
    const filledColumn = {
        ...containerFactory(),
        id: 'flex-right',
        settings: { ...containerFactory().settings, containerWidth: '50%', cssClass: 'right-column' },
        children: [
            { id: 'heading-1', type: 'heading', settings: {} },
            nestedContainer,
        ],
    };
    const parent = {
        ...
        children: [emptyColumn, filledColumn],
        columns: [{ id: 'cell-left', children: [] }, { id: 'cell-right', children: [] }],
    };

    assert.equal(api.moveLooseGridChildrenIntoColumns(parent), true);

    assert.equal(parent.children.length, 0);
    assert.deepEqual(Array.from(parent.columns[0].children, (child) => child.id), []);
    assert.deepEqual(Array.from(parent.columns[1].children, (child) => child.id), ['heading-1', 'nested-container']);
    assert.equal(parent.columns[0].flexSourceContainer.id, 'flex-left');
    assert.equal(parent.columns[0].flexSourceContainer.settings.cssClass, 'left-column');
    assert.equal(parent.columns[1].flexSourceContainer.id, 'flex-right');
    assert.equal(parent.columns[1].flexSourceContainer.settings.cssClass, 'right-column');
    assert.equal(Object.hasOwn(parent.columns[1].flexSourceContainer, 'children'), false);
    assert.equal(parent.columns[1].children[1].children[0].id, 'button-1');
});

test('Grid to Flexbox restores the unwrapped child Container identities and settings', () => {
    const api = loadChildContainerHelpers();
    assert.equal(typeof api.restoreGridColumnsAsFlexChildren, 'function');
    const parent = {
        id: 'parent',
        type: 'container',
        ...
        columns: [{
            ...
            flexSourceContainer: {
                id: 'flex-left',
                type: 'container',
                settings: { displayType: 'flex', containerWidth: '50%', cssClass: 'left-column' },
            },
            children: [{ id: 'heading-1', type: 'heading', settings: {} }],
        }],
    };

    assert.equal(api.restoreGridColumnsAsFlexChildren(parent, containerFactory), true);

    assert.equal(Object.hasOwn(parent, 'columns'), false);
    assert.deepEqual(Array.from(parent.children, (child) => child.id), ['flex-left']);
    assert.equal(parent.children[0].settings.cssClass, 'left-column');
    assert.equal(parent.children[0].settings.containerWidth, '50%');
    assert.equal(parent.children[0].children[0].id, 'heading-1');
    assert.equal(Object.hasOwn(parent.children[0], 'flexSourceContainer'), false);
});
...
test('cycle prevention traverses Tabs and Accordion nested item trees', () => {
    const api = loadChildContainerHelpers();
    const tree = {
        id: 'parent',
        type: 'container',
        children: [{
            id: 'tabs',
            type: 'tabs',
            tabItems: [{ id: 'tab-1', children: [{
                id: 'accordion',
                type: 'accordion',
                accordionItems: [{ id: 'item-1', children: [{ id: 'deep-target', type: 'container', children: [] }] }],
            }] }],
        }],
    };

    assert.equal(api.canMoveNodeIntoContainer(tree, 'deep-target'), false);
    assert.equal(api.canMoveNodeIntoContainer(tree, 'outside'), true);
});

test('nested Tabs and Accordion drags do not get captured by an ancestor while the pointer is inside their owner', () => {
    const api = loadChildContainerHelpers();
    assert.equal(typeof api.nestedOwnerBlocksAncestorCapture, 'function');
    const owner = { getBoundingClientRect: () => ({ left: 100, right: 500, top: 100, bottom: 500 }) };
    const from = { closest: () => owner };
    const ancestor = { contains: (candidate) => candidate === owner };
    const siblingNestedTarget = { contains: () => false };

    assert.equal(api.nestedOwnerBlocksAncestorCapture(from, ancestor, { clientX: 300, clientY: 300 }), true);
    assert.equal(api.nestedOwnerBlocksAncestorCapture(from, ancestor, { clientX: 700, clientY: 300 }), false);
    assert.equal(api.nestedOwnerBlocksAncestorCapture(from, siblingNestedTarget, { clientX: 300, clientY: 300 }), false);
});
...
test('Container edge resizer uses the dedicated child Container selector', () => {
    const css = readPageBuilderV24EditorStyles();
...
test('toolbox and existing widgets can enter canonical child Container dropzones', () => {
    ...
    const isCont = (type) => ['container', 'container_fluid'].includes(type);
    const isGrid = (type) => ['grid', 'row_grid'].includes(type);
    const groupFor = (children = []) => new Function('isFormLayoutDrag', 'isCont', 'isGrid', contGroupBody)
        .call({ node: { children } }, isFormLayoutDrag, isCont, isGrid);
    const accepts = (group, groupName, nestedTarget = true, nodeType = '') => typeof group.put === 'function'
        ? group.put(
            ...
            { dataset: nodeType ? { nodeType } : {} },
        )
        : group.put === true;

    const leafGroup = groupFor();
    const structuralParentGroup = groupFor([
        { id: 'column-1', type: 'container' },
        { id: 'column-2', type: 'container' },
    ]);

    assert.equal(leafGroup.name, 'pb-container');
    assert.equal(accepts(leafGroup, 'pb-col'), true, 'leaf child Containers must accept sidebar widget clones');
    ...
    assert.equal(accepts(structuralParentGroup, 'pb-root', true, 'heading'), false, 'root widgets must not move beside Flexbox child Containers');
    assert.equal(accepts(structuralParentGroup, 'pb-root'), true, 'sidebar layout tools remain valid structural children');
    assert.equal(accepts(structuralParentGroup, 'pb-container', true, 'container'), true, 'nested Containers remain valid structural children');
    assert.equal(accepts(structuralParentGroup, 'pb-col', true, 'grid'), true, 'Grid layout nodes remain valid structural children');
...
test('nested Tabs and Accordion nodes restore native drag only while Sortable chooses them', () => {
    const css = readPageBuilderV24EditorStyles();
    const rule = css.match(/\.pb-preview \.pb-dropzone > \.pb-node\[draggable="true"\]\s*\{([\s\S]*?)\}/)?.[1] || '';
    assert.match(rule, /-webkit-user-drag:\s*element/);
...
test('empty child hints and nested layout toolbars remain centered and collision-free', () => {
    ...
    for (const selector of ['.pb-ghost', '.sortable-ghost', '.sortable-chosen']) {
        assert.match(css, new RegExp(`\\.pb-dropzone-container-children\\s*>\\s*\\${selector}\\s*\\{[\\s\\S]*?position:\\s*relative;[\\s\\S]*?z-index:\\s*1;`));
    }

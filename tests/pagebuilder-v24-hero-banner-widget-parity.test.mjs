...
const definition = read('resources/pagebuilder_elementor_v24/modules/widgets/pro/hero-banner/definition.js');
const settings = read('resources/pagebuilder_elementor_v24/modules/widgets/pro/hero-banner/Settings.vue');
const settingsMarkup = settings.split('<style scoped>')[0];
const canvas = read('resources/pagebuilder_elementor_v24/modules/widgets/pro/hero-banner/Canvas.vue');
const blade = read('resources/pagebuilder_elementor_v24/modules/widgets/pro/hero-banner/frontend.blade.php');
const manifest = JSON.parse(read('resources/pagebuilder_elementor_v24/modules/widgets/pro/hero-banner/module.json'));
const runtime = read('resources/pagebuilder_elementor_v24/modules/widgets/pro/hero-banner/runtime.js');
const coreRuntime = read('public/js/pagebuilder_elementor_v24/frontend-runtime.js');
...
const prototypeCss = read('project-artifacts/mockups/pagebuilder-v24-responsive-hero-prototype/src/styles.css');
const prototype = read('public/mockups/pagebuilder-editor-redesign-prototype-v2.4.html');
...
vm.runInNewContext(definition, {
    window: {
        ...
        PageBuilderElementorV24Widgets: {
            ...
            register(widget) { registrations.push(widget); },
        },
    },
});
...
assert.equal(manifest.type, 'hero_banner');
assert.equal(manifest.label, 'Hero Banner');
assert.equal(manifest.icon, 'fas fa-image');
...
assert.ok((settings.match(/class="pb-seg-btn"/g) || []).length >= 6, 'Hero segmented controls must reuse the v2.4 button class');
...
assert.match(editorCss, /\.side-panel\.pb-panel\.left \.v24-properties-section :is\(\.pb-media-field\.has-action, \.pb-input-with-action, \.pb-hero-color\) > \.pb-input\s*\{[^}]*border-top-right-radius:\s*0 !important;[^}]*border-bottom-right-radius:\s*0 !important;/s);
...
assert.match(canvas, /PageBuilderElementorV24Runtime\?\.openMediaLightbox/);
...
assert.match(frontendV24Css, /\.pb-hero-banner\.is-natural-image/);
...
assert.ok(coreRuntime.includes('www\\.dailymotion\\.com\\/embed\\/video'));
assert.match(coreRuntime, /document\.createElement\(isNativeVideo \? 'video' : 'iframe'\)/);
...
assert.match(runtime, /runtimes\["hero_banner"\]\s*=\s*Object\.freeze\(\{\s*init,\s*initHeroBanner\s*\}\)/);

console.log('pagebuilder v2.4 hero banner widget parity test passed');

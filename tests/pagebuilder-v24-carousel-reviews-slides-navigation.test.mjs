...
async function definition(type) {
    ...
    configureSingleModule(context, await source(`resources/pagebuilder_elementor_v24/modules/widgets/pro/${type}/module.json`));
    vm.runInNewContext(await source(`resources/pagebuilder_elementor_v24/modules/widgets/pro/${type}/definition.js`), context);
    return context.window.PageBuilderElementorV24Widgets.get(type);
}

test("Canvas SSR render is warning-free", async () => {
    const component = await loadSfc("resources/pagebuilder_elementor_v24/modules/widgets/pro/carousel/Canvas.vue");
...
test("Canvas renders navigation visibility, pagination mode, and custom arrow icons", async () => {
    const carouselComponent = await loadSfc("resources/pagebuilder_elementor_v24/modules/widgets/pro/carousel/Canvas.vue");

    const arrowsOnly = await renderToString(Vue.createSSRApp(carouselComponent, {
        item: {
            id: "carousel-arrows",
            type: "carousel",
            settings: {
    ...
    const fraction = await renderToString(Vue.createSSRApp(carouselComponent, {
        item: {
    ...
    const reviewsComponent = await loadSfc("resources/pagebuilder_elementor_v24/modules/widgets/pro/reviews/Canvas.vue");
    const reviewsNone = await renderToString(Vue.createSSRApp(reviewsComponent, {
        item: {
            id: "reviews-none",
            type: "reviews",
            settings: {
    ...
    const slidesComponent = await loadSfc("resources/pagebuilder_elementor_v24/modules/widgets/pro/slides/Canvas.vue");
    const slidesDotsOnly = await renderToString(Vue.createSSRApp(slidesComponent, {
        item: {
            id: "slides-dots",
            type: "slides",
            settings: {
                slidesName: "QA Slides",
    ...
    assert.match(slidesDotsOnly, /aria-label="QA Slides"/);
...
test("Settings and Blade renderer expose the same navigation controls", async () => {
    const settingsSources = await Promise.all(
        ["carousel", "reviews", "slides"].map((type) => source(`resources/pagebuilder_elementor_v24/modules/widgets/pro/${type}/Settings.vue`)),
    );
    const settingsSource = settingsSources.join("\n");
    const rendererSource = await source("resources/pagebuilder_elementor_v24/modules/widgets/pro/reviews/frontend.blade.php");

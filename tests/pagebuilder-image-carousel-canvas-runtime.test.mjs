...
const appPath = new URL('../public/js/pagebuilder_elementor/app.js', import.meta.url);
const frontendBladePath = new URL('../resources/views/pagebuilder_elementor/partials/render_image_carousel.blade.php', import.meta.url);
const editorCssPath = new URL('../public/assets/css/pagebuilder_elementor.css', import.meta.url);
const frontendCssPath = new URL('../public/assets/css/frontend_elementor.css', import.meta.url);
...
test('Image Carousel arrow controls match the Elementor icon picker and compact canvas rhythm', () => {
	const settings = fs.readFileSync(settingsPath, 'utf8');
	const app = fs.readFileSync(appPath, 'utf8');
	const canvas = fs.readFileSync(canvasPath, 'utf8');
	const blade = fs.readFileSync(frontendBladePath, 'utf8');
	const editorCss = fs.readFileSync(editorCssPath, 'utf8');
	const frontendCss = fs.readFileSync(frontendCssPath, 'utf8');

	assert.match(settings, /const IconPicker=/);
	assert.match(settings, /editor\.openImageCarouselArrowIconLibrary/);
	assert.match(settings, /title="Upload SVG"/);
	assert.match(settings, /title="Icon Library"/);
	assert.match(app, /previousArrowIconSource: 'library'/);
	assert.match(app, /openImageCarouselArrowIconLibrary/);
	assert.match(app, /chooseImageCarouselArrowSvg/);
	assert.match(canvas, /arrowIconSource\('previous'\)/);
	assert.match(canvas, /pb-image-carousel__arrow-svg/);
	assert.match(blade, /\$sanitizeSvg/);
	assert.match(blade, /previousArrow\['svg'\]/);
	assert.match(editorCss, /pb-image-carousel-icon-picker__button/);
...
test('image track supports horizontal pointer dragging without triggering native image drag', () => {
	const source = fs.readFileSync(canvasPath, 'utf8');
	const editorCss = fs.readFileSync(editorCssPath, 'utf8');

	assert.match(source, /pb-image-carousel__viewport[^>]*data-pb-interactive="true"/);
	assert.match(source, /@pointerdown\.stop="startDrag\(\$event\)"/);
	assert.match(source, /@pointermove\.stop="moveDrag\(\$event\)"/);
	assert.match(source, /@pointerup\.stop="endDrag\(\$event\)"/);
	assert.match(source, /@pointercancel\.stop="cancelDrag\(\$event\)"/);
	assert.match(source, /draggable="false"/);
	assert.match(editorCss, /\.pb-node\.pb-node-image_carousel \.pb-image-carousel__viewport[\s\S]*?pointer-events:\s*auto;/);
	...
	const context = canvasContext({
		...
		slidesToShow: '1', slidesToScroll: '1', navigation: 'arrows_dots',
		pauseOnInteraction: true,
	});
	context.startAutoplay = () => {};
	context.stopAutoplay = () => {};
	...
	const event = (x, overrides = {}) => ({
		button: 0, pointerId: 7, clientX: x, clientY: 120, currentTarget: pointerTarget,
		preventDefault() {}, stopPropagation() {}, ...overrides,
	});

	context.startDrag(event(400));
	assert.equal(context.dragState.pointerId, 7);
	assert.deepEqual(pointerTarget.captured, [7]);
	context.moveDrag(event(250));
	assert.equal(context.dragOffset, -150);
	context.endDrag(event(250));
	assert.equal(context.activeIndex, 1, 'dragging left should advance one slide');
	assert.equal(context.dragState, null);
	assert.equal(context.dragOffset, 0);
	assert.deepEqual(pointerTarget.released, [7]);
	...
	assert.equal(prevented, true, 'the click generated after a drag should not follow the image link');
	assert.equal(stopped, true, 'the suppressed click should not bubble into editor selection');
});
...
test('editor canvas explicitly enables carousel arrow and pagination click targets', () => {
	const source = fs.readFileSync(canvasPath, 'utf8');
	const editorCss = fs.readFileSync(editorCssPath, 'utf8');

	assert.match(source, /pb-image-carousel__arrow--previous[^>]*data-pb-interactive="true"[^>]*@click\.stop="previous\(true\)"/);
	assert.match(source, /pb-image-carousel__arrow--next[^>]*data-pb-interactive="true"[^>]*@click\.stop="next\(true\)"/);
	assert.match(source, /pb-image-carousel__dot[^>]*data-pb-interactive="true"[^>]*@click\.stop="goTo\(\(page-1\)\*slidesToScroll,true\)"/);
	assert.match(editorCss, /\.pb-node\.pb-node-image_carousel \[data-pb-interactive="true"\]\s*\{\s*pointer-events:\s*auto;/);
});

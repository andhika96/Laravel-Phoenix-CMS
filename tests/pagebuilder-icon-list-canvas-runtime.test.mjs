...
test('Icon List panel keeps its rhythm, responsive controls, and repeater actions compact', () => {
	const css = fs.readFileSync(editorCssPath, 'utf8');
	const bodyRule = css.match(/\.pb-panel\.left\s+\.pb-widget-settings--icon-list\s+\.pb-collapsible-body\s*\{([\s\S]*?)\}/);
	const labelRule = css.match(/\.pb-panel\.left\s+\.pb-widget-settings--icon-list\s+\.pb-label-row\.pb-label-row-device\s*\{([\s\S]*?)\}/);
	...
	assert.ok(labelRule, 'Icon List responsive labels should be scoped to the widget');
	assert.match(labelRule[1], /justify-content\s*:\s*space-between\s*;/);
	assert.ok(deviceRule, 'Icon List device menu should be scoped to the widget');
	assert.match(deviceRule[1], /margin-left\s*:\s*auto\s*;/);
	assert.ok(addButtonRule, 'Icon List add action should have a compact widget rule');

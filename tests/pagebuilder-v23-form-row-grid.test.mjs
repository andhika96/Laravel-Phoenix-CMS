...
const definitionPath = path.join(root, 'public/js/pagebuilder_elementor_v23/widgets/pro/form/definition.js');
const settingsPath = path.join(root, 'public/js/pagebuilder_elementor_v23/widgets/pro/shared/Settings.vue');
const canvasPath = path.join(root, 'public/js/pagebuilder_elementor_v23/widgets/pro/shared/Canvas.vue');
const appPath = path.join(root, 'public/js/pagebuilder_elementor_v23/app.js');
const runtimePath = path.join(root, 'public/js/pagebuilder_elementor_v23/frontend-runtime.js');
const bladePath = path.join(root, 'resources/views/pagebuilder_elementor_v23/partials/render_pro_widget.blade.php');
const editorShellPath = path.join(root, 'resources/views/pagebuilder_elementor_v23/editor_shell.blade.php');
const routesPath = path.join(root, 'routes/experimentalFeaturesWebv2.php');

function loadDefinition() {
    const source = fs.readFileSync(definitionPath, 'utf8');
    const registry = { definition: null, register(definition) { this.definition = definition; } };
    const context = vm.createContext({
        console,
        Math,
        window: {
            PageBuilderElementorV23Widgets: registry,
    ...
    assert.ok(context.window.PageBuilderElementorV23FormRowGrid, 'Form row grid helpers should be exposed');
    return { definition: registry.definition, api: context.window.PageBuilderElementorV23FormRowGrid };
}
...
test('Custom Messages exposes four display modes and previews the selected state only in its Form owner', () => {
    ...
    assert.equal(dispatched[0].type, 'pagebuilder:v23-form-message-preview');

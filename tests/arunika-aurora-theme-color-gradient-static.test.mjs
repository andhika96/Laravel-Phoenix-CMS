...
test('keeps the Arunika Aurora picker limited to colors and reuses existing persistence', () =>
{
	...
	assert.match(themeScript, /function applyMainColor\(color\)/);
	assert.match(themeScript, /style\.setProperty\('--ph-theme-primary', isCoolGray \? coolGrayInteractiveColor : color\)/);
	assert.match(themeScript, /style\.setProperty\('--ph-theme-surface-tint', color\)/);
	assert.match(themeScript, /localStorage\.setItem\('theme-color', color\)/);
});

test('derives light mode shell gradients from the selected surface color and keeps interactive hover separate', () =>
{
	const lightTheme = shellStyles.match(/\[data-bs-theme=light\][\s\S]*?(?=\[data-bs-theme=dark\])/u)?.[0] ?? '';

	assert.match(shellStyles, /--ph-shell-hover:\s*var\(--ph-theme-hover-surface, color-mix\(in srgb, var\(--ph-theme-primary\), white \d+%\)\);/u);
	assert.match(lightTheme, /--ph-sidebar-surface:[^;]*color-mix\(in srgb, var\(--ph-theme-surface-tint\), transparent \d+%\)[^;]*;/u);
	assert.match(lightTheme, /--ph-header-surface:[^;]*color-mix\(in srgb, var\(--ph-theme-surface-tint\), transparent \d+%\)[^;]*;/u);
});

test('derives dark mode sidebar and header gradients from the selected surface color', () =>
{
	const darkTheme = shellStyles.match(/\[data-bs-theme=dark\][\s\S]*?(?=html,)/u)?.[0] ?? '';

	assert.match(darkTheme, /--ph-sidebar-surface:[^;]*color-mix\(in srgb, var\(--ph-theme-surface-tint\), transparent \d+%\)[^;]*;/u);
	assert.match(darkTheme, /--ph-header-surface:[^;]*color-mix\(in srgb, var\(--ph-theme-surface-tint\), transparent \d+%\)[^;]*;/u);
});

...
const layout = fs.readFileSync(path.join(projectRoot, 'resources/views/themes/arunika_v2/cms/cms_layout.blade.php'), 'utf8');
const stylesheet = fs.readFileSync(path.join(projectRoot, 'public/assets/css/themes/arunika_v2/arunika_v2.css'), 'utf8');
const themeScript = fs.readFileSync(path.join(projectRoot, 'public/assets/js/themes/arunika_v2/arunika_v2.js'), 'utf8');
const shellStyles = stylesheet.slice(stylesheet.indexOf('/* ARUNIKA V2 - WEIHU REFERENCE PANEL SHELL */'));

test('places the Arunika V2 color palette immediately before the theme mode toggle', () =>
{
...
test('keeps the Arunika V2 picker limited to colors and reuses existing persistence', () =>
{

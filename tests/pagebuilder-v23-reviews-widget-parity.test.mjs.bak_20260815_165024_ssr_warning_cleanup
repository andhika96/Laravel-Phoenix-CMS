...
test('Reviews registers as a Pro widget with Elementor carousel defaults', async () => {
  ...
  vm.runInNewContext(await source('public/js/pagebuilder_elementor_v23/widgets/pro/reviews/definition.js'), context);

  const definition = context.window.PageBuilderElementorV23Widgets.get('reviews');

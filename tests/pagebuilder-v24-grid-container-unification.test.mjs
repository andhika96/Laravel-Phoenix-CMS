...
const sharedAdvanced = readFileSync(resolve(root, 'resources/pagebuilder_elementor_v24/shared/AdvancedControls.vue'), 'utf8');
const containerCanvas = readFileSync(resolve(root, 'resources/pagebuilder_elementor_v24/modules/layout/container/Canvas.vue'), 'utf8');
const renderNode = readFileSync(resolve(root, 'resources/views/pagebuilder_elementor_v24/partials/render_node.blade.php'), 'utf8');
const containerBlade = readFileSync(resolve(root, 'resources/pagebuilder_elementor_v24/modules/layout/container/frontend.blade.php'), 'utf8');
...
test('Grid uses conceptual columns while Flexbox uses selectable child Containers', () => {
    ...
    assert.match(app, /function onContainerDisplayTypeChange\(node\) \{[\s\S]*?if \(dt === 'flex'\)[\s\S]*?restoreGridColumnsAsFlexChildren\([\s\S]*?if \(dt === 'grid'\)[\s\S]*?ensureNodeGridColumns\(node\);/);
...
test('Container Grid exposes the official Elementor control groups without legacy-only fields', () => {
    assert.match(containerSettings, /:is="editor\.widgetAdvancedControls"/);
    const completeSettingsContract = `${containerSettings}\n${sharedAdvanced}`;
    for (const label of [
        ...
        assert.match(completeSettingsContract, new RegExp(label.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')), `${label} should be present`);
    }

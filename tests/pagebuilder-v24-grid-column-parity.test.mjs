import assert from 'node:assert/strict';
import fs from 'node:fs';
import path from 'node:path';
import test from 'node:test';
import vm from 'node:vm';
import { fileURLToPath } from 'node:url';
import {
    pageBuilderV24CapabilityPrelude,
    readPageBuilderV24EditorStyles,
} from './helpers/pagebuilder-v24-module-source.mjs';

const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const appPath = path.join(root, 'public/js/pagebuilder_elementor_v24/app.js');
const settingsPath = path.join(root, 'resources/pagebuilder_elementor_v24/modules/layout/container/Settings.vue');
const gridSettingsPath = path.join(root, 'resources/pagebuilder_elementor_v24/modules/layout/grid/Settings.vue');
const rowGridSettingsPath = path.join(root, 'resources/pagebuilder_elementor_v24/modules/layout/row-grid/Settings.vue');
const cssPath = path.join(root, 'public/assets/css/pagebuilder_elementor_v24.css');
const app = fs.readFileSync(appPath, 'utf8');
const containerSettings = fs.readFileSync(settingsPath, 'utf8');
const gridSettings = fs.readFileSync(gridSettingsPath, 'utf8');
const rowGridSettings = fs.readFileSync(rowGridSettingsPath, 'utf8');
const css = readPageBuilderV24EditorStyles(root);

test('Grid spacing controls use the established connected responsive four-side form', () => {
    for (const source of [gridSettings, rowGridSettings]) {
        assert.equal((source.match(/<div class="pb-responsive-switch">/g) || []).length, 0, 'Grid Layout should use one responsive select control');
        assert.match(source, /Grid Layout[\s\S]{0,260}<div class="pb-control-device-wrap">[\s\S]*?openControlResponsiveMenu\(node\.type \+ '-grid-layout'\)/);
        assert.match(source, /isControlResponsiveMenuOpen\(node\.type \+ '-grid-layout'\)/);
        assert.match(source, /applyResponsiveDevice\(node\.type \+ '-grid-layout', device\.value\)/);
        assert.match(source, /editor\.responsiveDeviceIcon\(\)/);
        assert.doesNotMatch(source, /Spacing[\s\S]{0,180}<div class="pb-responsive-switch">/);
        assert.equal((source.match(/pb-form-group pb-spacing-control-group/g) || []).length, 2);
        assert.ok((source.match(/pb-four-sides pb-four-sides-with-link mt-1/g) || []).length >= 2);
        assert.ok((source.match(/pb-side-link-cell/g) || []).length >= 2);
        assert.match(source, /pb-mini-unit pb-edge-unit-select/);
        assert.match(source, /editor\.openControlResponsiveMenu\(node\.type \+ '-padding'\)/);
        assert.match(source, /editor\.openControlResponsiveMenu\(node\.type \+ '-margin'\)/);
    }

    assert.match(css, /:is\(\.pb-layout-settings, \.pb-grid-settings\) \.pb-spacing-control-group/);
    assert.match(css, /:is\(\.pb-layout-settings, \.pb-grid-settings\) \.pb-edge-unit-select/);
    assert.match(css, /\.pb-panel\.left \.pb-grid-settings \.pb-device-btn\s*\{[^}]*justify-content:\s*center;[^}]*gap:\s*0;/);
});

function extractBlock(source, startIndex) {
    const open = source.indexOf('{', startIndex);
    assert.notEqual(open, -1, 'function or method opening brace should exist');
    let depth = 0;
    let quote = '';
    let escaped = false;
    for (let index = open; index < source.length; index += 1) {
        const char = source[index];
        if (quote) {
            if (escaped) escaped = false;
            else if (char === '\\') escaped = true;
            else if (char === quote) quote = '';
            continue;
        }
        if (char === '"' || char === "'" || char === '`') {
            quote = char;
            continue;
        }
        if (char === '{') depth += 1;
        if (char === '}') {
            depth -= 1;
            if (depth === 0) return source.slice(open + 1, index);
        }
    }
    assert.fail('function or method closing brace should exist');
}

function namedFunction(name) {
    const start = app.indexOf(`function ${name}(`);
    assert.notEqual(start, -1, `${name} should exist`);
    const body = extractBlock(app, start);
    const signatureStart = app.indexOf('(', start);
    const signatureEnd = app.indexOf(')', signatureStart);
    return `function ${name}${app.slice(signatureStart, signatureEnd + 1)} {${body}}`;
}

function loadGridHelpers() {
    const names = [
        'gridSlotCount',
        'gridColumnStyleDefaults',
        'normalizeGridColumnStyle',
        'reconcileGridColumns',
        'gridColumnHasContent',
        'isGridColumnLockedSequentially',
        'canonicalLayoutForSave',
    ];
    const context = vm.createContext({ structuredClone, console });
    vm.runInContext(`${pageBuilderV24CapabilityPrelude(root)}\n${names.map(namedFunction).join('\n')}\nthis.api = { ${names.join(', ')} };`, context);
    return context.api;
}

function loadGridRemovalHelpers() {
    const names = ['gridColumnStyleDefaults', 'normalizeGridColumnStyle', 'gridColumnTrackRemovalImpact', 'removeGridColumnTrack'];
    const context = vm.createContext({ console });
    vm.runInContext(`${pageBuilderV24CapabilityPrelude(root)}\n${names.map(namedFunction).join('\n')}\nthis.api = { ${names.join(', ')} };`, context);
    return context.api;
}

function methodBody(name) {
    const start = app.indexOf(`\n\t\t\t${name}()`);
    assert.notEqual(start, -1, `${name} BuilderNode method should exist`);
    return extractBlock(app, start);
}

function loadOnAddCol(isLocked) {
    const context = vm.createContext({
        isGridColumnLockedSequentially: isLocked,
        isGrid: () => false,
        ensureNodeGridColumns: () => {},
        isCont: () => false,
        seedResponsiveSettings: () => {},
        selectedId: { value: '' },
    });
    vm.runInContext(`${namedFunction('onAddCol')}\nthis.onAddCol = onAddCol;`, context);
    return { onAddCol: context.onAddCol, selectedId: context.selectedId };
}

test('Grid Columns and Rows create conceptual slots without creating child Containers', () => {
    const api = loadGridHelpers();
    const firstWidget = { id: 'heading-1', type: 'heading' };
    const node = {
        children: [],
        columns: [
            { id: 'cell-1', children: [firstWidget] },
            { id: 'cell-2', children: [] },
        ],
    };
    let nextId = 2;

    const slots = api.reconcileGridColumns(node, 3, 2, () => `cell-${++nextId}`);

    assert.equal(api.gridSlotCount(3, 2), 6);
    assert.equal(slots.length, 6);
    assert.equal(node.children.length, 0);
    assert.equal(slots[0].id, 'cell-1');
    assert.equal(slots[0].children[0], firstWidget);
    assert.ok(slots.every((slot) => Array.isArray(slot.children)));
    assert.ok(slots.every((slot) => !Object.hasOwn(slot, 'type')));
});

test('Grid slot reduction preserves overflow content in the final remaining slot', () => {
    const api = loadGridHelpers();
    const node = {
        columns: Array.from({ length: 6 }, (_, index) => ({
            id: `cell-${index + 1}`,
            children: index === 5 ? [{ id: 'button-1', type: 'button' }] : [],
        })),
    };

    const slots = api.reconcileGridColumns(node, 2, 1, () => 'unused');

    assert.equal(slots.length, 2);
    assert.deepEqual(Array.from(slots[1].children, (child) => child.id), ['button-1']);
});

test('deleting a Grid column track removes the matching cell from every row', () => {
    const api = loadGridRemovalHelpers();
    const node = {
        type: 'grid',
        settings: { columns: 3, gridRows: '2' },
        columns: [
            { id: 'r1-c1', children: [] },
            { id: 'r1-c2', children: [{ id: 'heading-1', type: 'heading' }] },
            { id: 'r1-c3', children: [] },
            { id: 'r2-c1', children: [] },
            { id: 'r2-c2', children: [{ id: 'button-1', type: 'button' }] },
            { id: 'r2-c3', children: [] },
        ],
    };

    const impact = api.gridColumnTrackRemovalImpact(node, 1);
    assert.deepEqual(JSON.parse(JSON.stringify(impact)), {
        canRemove: true,
        columnCount: 3,
        rowCount: 2,
        trackIndex: 1,
        removedIndexes: [1, 4],
        itemCount: 2,
    });

    assert.equal(api.removeGridColumnTrack(node, 1), true);
    assert.equal(node.settings.columns, 2);
    assert.deepEqual(Array.from(node.columns, (column) => column.id), ['r1-c1', 'r1-c3', 'r2-c1', 'r2-c3']);
});

test('Grid column deletion updates Container Grid settings and protects the final track', () => {
    const api = loadGridRemovalHelpers();
    const node = {
        type: 'container',
        settings: { displayType: 'grid', gridColumns: 2, gridRows: '1' },
        columns: [{ id: 'left', children: [] }, { id: 'right', children: [] }],
    };

    assert.equal(api.removeGridColumnTrack(node, 0), true);
    assert.equal(node.settings.gridColumns, 1);
    assert.deepEqual(Array.from(node.columns, (column) => column.id), ['right']);
    assert.equal(api.removeGridColumnTrack(node, 0), false);
    assert.deepEqual(Array.from(node.columns, (column) => column.id), ['right']);
});

test('only the first/current empty Grid column is unlocked', () => {
    const api = loadGridHelpers();
    const node = {
        columns: [
            { id: 'cell-1', children: [] },
            { id: 'cell-2', children: [] },
            { id: 'cell-3', children: [] },
        ],
    };

    assert.equal(api.isGridColumnLockedSequentially(node, 0), false);
    assert.equal(api.isGridColumnLockedSequentially(node, 1), true);
    assert.equal(api.isGridColumnLockedSequentially(node, 2), true);

    node.columns[0].children.push({ id: 'container-1', type: 'container' });
    assert.equal(api.isGridColumnLockedSequentially(node, 1), false);
    assert.equal(api.isGridColumnLockedSequentially(node, 2), true);

    node.columns[1].children.push({ id: 'grid-1', type: 'grid' });
    assert.equal(api.isGridColumnLockedSequentially(node, 2), false);
});

test('Grid column Sortable accepts Widget, Container, and Grid sources but rejects a locked target', () => {
    const body = methodBody('colGroup');
    const isFormLayoutDrag = (value) => String(
        typeof value === 'string'
            ? value
            : value?.options?.group?.name || value?.dataset?.formLayoutGroup || '',
    ).startsWith('pb-form-grid:');
    let lockArguments = [];
    const unlocked = new Function('isFormLayoutDrag', body).call({
        getTargetColumnIndexFromDropzone: () => 0,
        isSequentialColumnLocked: (...args) => {
            lockArguments = args;
            return false;
        },
    }, isFormLayoutDrag);
    const locked = new Function('isFormLayoutDrag', body).call({
        getTargetColumnIndexFromDropzone: () => 2,
        isSequentialColumnLocked: () => true,
    }, isFormLayoutDrag);
    const target = { el: { dataset: { colIndex: '0' } } };

    for (const [group, nodeType] of [['pb-col', 'heading'], ['pb-root', 'container'], ['pb-container', 'grid']]) {
        const from = { options: { group: { name: group } } };
        assert.equal(unlocked.put(target, from, { dataset: { nodeType } }), true, `${nodeType} should enter the current Grid column`);
        assert.deepEqual(lockArguments, [0], 'pre-drop lock checks must use the intact source column state');
        assert.equal(locked.put(target, from, { dataset: { nodeType } }), false, `${nodeType} should not skip an earlier empty Grid column`);
    }
    assert.equal(unlocked.put(target, { options: { group: { name: 'pb-form-grid:form-1' } } }, { dataset: { nodeType: 'form-field' } }), false, 'internal Form fields must not enter page Grid columns');
});

test('an existing Grid item can move into its next empty unlocked column', () => {
    const item = { id: 'heading-1', type: 'heading', settings: {} };
    const parentNode = { id: 'grid-1', columns: [] };
    const sourceChildren = [];
    const targetChildren = [item];
    const sourceList = sourceChildren;
    const { onAddCol, selectedId } = loadOnAddCol(() => true);
    const event = {
        newIndex: 0,
        oldIndex: 0,
        item: { _underlying_vm_: item },
        from: {
            dataset: { parentNodeId: 'grid-1', colIndex: '0' },
            __draggable_component__: { realList: sourceList },
        },
        to: { dataset: { parentNodeId: 'grid-1', colIndex: '1' } },
    };

    onAddCol(event, { id: 'cell-2', children: targetChildren }, 1, parentNode);

    assert.deepEqual(Array.from(targetChildren, (child) => child.id), ['heading-1']);
    assert.deepEqual(Array.from(sourceChildren, (child) => child.id), []);
    assert.equal(selectedId.value, 'heading-1');

    const foreignSource = [];
    const lockedTarget = [item];
    const foreignEvent = {
        ...event,
        from: {
            dataset: { parentNodeId: 'grid-2', colIndex: '0' },
            __draggable_component__: { realList: foreignSource },
        },
    };

    onAddCol(foreignEvent, { id: 'cell-2', children: lockedTarget }, 1, parentNode);

    assert.deepEqual(Array.from(lockedTarget), [], 'a genuinely locked cross-Grid target must still reject the move');
    assert.deepEqual(Array.from(foreignSource, (child) => child.id), ['heading-1']);
});

test('Grid columns persist on save while Flexbox legacy columns are removed', () => {
    const api = loadGridHelpers();
    const source = [
        {
            id: 'grid-container',
            type: 'container',
            settings: { displayType: 'grid' },
            children: [],
            columns: [{ id: 'cell-1', children: [{ id: 'nested-grid', type: 'grid', settings: {}, columns: [{ id: 'nested-cell', children: [] }] }] }],
        },
        {
            id: 'flex-container',
            type: 'container',
            settings: { displayType: 'flex' },
            children: [],
            columns: [{ id: 'legacy-flex-cell', children: [] }],
        },
    ];

    const saved = api.canonicalLayoutForSave(source);

    assert.equal(saved[0].columns.length, 1);
    assert.equal(saved[0].columns[0].children[0].columns.length, 1);
    assert.equal(Object.hasOwn(saved[1], 'columns'), false);
    assert.equal(Object.hasOwn(source[1], 'columns'), true, 'serialization must not mutate editor state');
});

test('Container settings expose Add Container only for Flexbox', () => {
    const childSection = containerSettings.match(/<details class="pb-collapsible"[^>]*>\s*<summary>Child Containers<\/summary>[\s\S]*?<\/details>/)?.[0] || '';
    assert.match(childSection, /v-if="[^\"]*displayType[^\"]*flex/);
    assert.match(childSection, /Add Container/);
    assert.doesNotMatch(childSection, /displayType==='grid'/);
});

test('Container Grid and standalone Grid settings expose guarded column deletion', () => {
    for (const source of [containerSettings, gridSettings, rowGridSettings]) {
        assert.match(source, /editor\.gridColumnTracks\(node\)/);
        assert.match(source, /editor\.requestGridColumnRemoval\(node, track\.index\)/);
        assert.match(source, /fa-trash/);
    }

    assert.match(app, /const deleteConfirmation = ref\(/);
    assert.match(app, /function confirmPendingDeletion\(\)/);
    assert.match(app, /role="dialog"/);
    assert.match(app, /aria-modal="true"/);
    assert.match(css, /\.pb-delete-confirm-modal\s*\{/);
});

test('Grid column rows stay read-only during Vue render', () => {
    const start = app.indexOf('function gridColumnTracks(');
    assert.notEqual(start, -1, 'gridColumnTracks should exist');
    const body = extractBlock(app, start);

    assert.doesNotMatch(body, /ensureNodeGridColumns\(/, 'render-time list building must not replace reactive Grid columns');
});

test('Container Grid and legacy Grid render column dropzones while Flexbox keeps child Containers', () => {
    assert.match(app, /v-if="\(node\.settings\?\.displayType \|\| 'flex'\) === 'grid'"[\s\S]*?v-for="\(col, ci\) in node\.columns"[\s\S]*?v-model="col\.children"/);
    assert.match(app, /v-else[\s\S]*?v-model="node\.children"[\s\S]*?pb-dropzone-container-children/);
    assert.match(app, /<template v-else-if="isGrid">[\s\S]*?v-for="\(col, ci\) in node\.columns"[\s\S]*?v-model="col\.children"/);
    assert.match(app, /onShowToolbox\(\{ type: 'column', nodeId: node\.id, colId: col\.id \}\)/);
});

test('restored Grid cells preserve original alignment and single Heading preview behavior', () => {
    assert.match(app, /contDropzoneStyle\(col\)\s*\{[\s\S]*?gridAlignItems[\s\S]*?justifyContent/);
    assert.match(app, /contGridNodeStyle\(col, childNode = null\)\s*\{[\s\S]*?gridJustifyItems[\s\S]*?--pb-widget-shell-justify[\s\S]*?gridAlignItems/);
    assert.match(app, /contChildNodeStyle\(col, childNode = null\)\s*\{[\s\S]*?contGridNodeStyle\(col, childNode\)/);
    assert.match(app, /v-if="\(node\.settings\?\.displayType \|\| 'flex'\) === 'grid'"[\s\S]*?:style="contDropzoneStyle\(col\)"[\s\S]*?has-single-heading[\s\S]*?:style="contChildNodeStyle\(col, element\)"/);
    assert.match(app, /<template v-else-if="isGrid">[\s\S]*?has-single-heading/);
});

test('empty Grid cell hint and Sortable ghost stay inside the target cell', () => {
    assert.match(css, /\.pb-grid-col-empty-hint\s*\{[\s\S]*?position:\s*absolute;[\s\S]*?inset:\s*0;[\s\S]*?align-items:\s*center;[\s\S]*?justify-content:\s*center;/);
    assert.match(css, /\.pb-dropzone-col\s*\{[\s\S]*?position:\s*relative;/);
    assert.match(css, /\.pb-dropzone-col\s*>\s*\.sortable-ghost,[\s\S]*?\.pb-dropzone-col\s*>\s*\.sortable-chosen\s*\{[\s\S]*?position:\s*relative;[\s\S]*?z-index:\s*1;/);
});

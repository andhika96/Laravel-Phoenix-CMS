...
test('Blockquote and Share Buttons definitions expose Pro defaults and strict normalization', async () => {
    ...
    configureModules(context, await Promise.all([
        source('resources/pagebuilder_elementor_v24/modules/widgets/pro/blockquote/module.json'),
        source('resources/pagebuilder_elementor_v24/modules/widgets/pro/share-buttons/module.json'),
    ]));
    const definitionPaths = [
        'resources/pagebuilder_elementor_v24/modules/widgets/pro/blockquote/definition.js',
        'resources/pagebuilder_elementor_v24/modules/widgets/pro/share-buttons/definition.js',
    ];
    ...
    const blockquote = context.window.PageBuilderElementorV24Widgets.get('blockquote');
    const share = context.window.PageBuilderElementorV24Widgets.get('share_buttons');
...
test('Blockquote and Share Buttons settings map Content, Style, and Advanced controls', async () => {
    const blockquoteComponent = await loadSfc('resources/pagebuilder_elementor_v24/modules/widgets/pro/blockquote/Settings.vue');
    const node = { type: 'blockquote', settings: blockquoteSettings };
    const blockquoteContent = await renderToString(Vue.createSSRApp(blockquoteComponent, { node, editor: editorFor('content') }));
    ...
    const blockquoteStyle = await renderToString(Vue.createSSRApp(blockquoteComponent, { node, editor: editorFor('style') }));
    ...
    const quotationStyle = await renderToString(Vue.createSSRApp(blockquoteComponent, { node: { type: 'blockquote', settings: { ...blockquoteSettings, skin: 'quotation' } }, editor: editorFor('style') }));
    for (const label of ['Quote Color', 'Quote Size', 'Gap']) assert.match(quotationStyle, new RegExp(label));
    assert.match(await renderToString(Vue.createSSRApp(blockquoteComponent, { node, editor: editorFor('advanced') })), /Advanced/);

    const shareComponent = await loadSfc('resources/pagebuilder_elementor_v24/modules/widgets/pro/share-buttons/Settings.vue');
    const shareNode = { type: 'share_buttons', settings: shareSettings };
    const shareContent = await renderToString(Vue.createSSRApp(shareComponent, { node: shareNode, editor: editorFor('content') }));
    ...
    const shareStyle = await renderToString(Vue.createSSRApp(shareComponent, { node: shareNode, editor: editorFor('style') }));
...
test('Blockquote and Share Buttons canvas expose safe content, responsive style, hover, and action hooks', async () => {
    const blockquoteComponent = await loadSfc('resources/pagebuilder_elementor_v24/modules/widgets/pro/blockquote/Canvas.vue');
    const blockquoteHtml = await renderToString(Vue.createSSRApp(blockquoteComponent, {
    ...
    const shareComponent = await loadSfc('resources/pagebuilder_elementor_v24/modules/widgets/pro/share-buttons/Canvas.vue');
    const shareHtml = await renderToString(Vue.createSSRApp(shareComponent, {
...
test('Blockquote and Share Buttons are wired through manifests, Blade, and owned runtime', async () => {
    const runtime = await source('resources/pagebuilder_elementor_v24/modules/widgets/pro/share-buttons/runtime.js');
    for (const [type, slug, label, icon, marker] of [
        ['blockquote', 'blockquote', 'Blockquote', 'fas fa-quote-left', 'pb-pro-blockquote'],
        ['share_buttons', 'share-buttons', 'Share Buttons', 'fas fa-share-alt', 'pb-pro-share-buttons'],
    ]) {
        const manifest = JSON.parse(await source(`resources/pagebuilder_elementor_v24/modules/widgets/pro/${slug}/module.json`));
        const blade = await source(`resources/pagebuilder_elementor_v24/modules/widgets/pro/${slug}/frontend.blade.php`);
        assert.equal(manifest.type, type);
        assert.equal(manifest.label, label);
        assert.equal(manifest.icon, icon);
        assert.match(blade, new RegExp(marker));
    }

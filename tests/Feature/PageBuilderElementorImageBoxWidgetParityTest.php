...
class PageBuilderElementorImageBoxWidgetParityTest extends TestCase
{
    ...
    public function test_title_html_tag_uses_automatic_visual_scale_until_typography_size_is_customized(): void
    {
        ...
        $this->assertStringNotContainsString('H1-H6 use automatic heading scale until Typography Size is edited.', $settings);
    ...
    public function test_style_colors_use_only_the_local_coloris_picker(): void
    {
        $settings = file_get_contents(public_path('js/pagebuilder_elementor/widgets/general/image-box/Settings.vue'));

        $this->assertIsString($settings);
        $this->assertStringNotContainsString('type="color"', $settings);
        $this->assertStringNotContainsString('class="pb-color-swatch"', $settings);

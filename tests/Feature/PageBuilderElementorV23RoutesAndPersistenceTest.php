...
class PageBuilderElementorV23RoutesAndPersistenceTest extends TestCase
{
    ...
    public function test_public_pages_route_renders_only_published_v23_pages(): void
    {
        $this->insertPage('public-v23-page', 'Public V23 Page', Page_Builder::EDITOR_VERSION_V23, json_encode([
            ['id' => 'public-heading', 'type' => 'heading', 'settings' => ['text' => 'Public V23 Content']],
        ], JSON_THROW_ON_ERROR));
        $this->insertPage('draft-v23-page', 'Draft V23 Page', Page_Builder::EDITOR_VERSION_V23, '[]', 'draft');

        $html = $this->get('/pages/public-v23-page')
            ->assertOk()
            ->getContent();

        $this->assertStringContainsString('<title>Public V23 Page</title>', $html);
        $this->assertStringContainsString('Public V23 Content', $html);
        $this->assertStringContainsString('assets/css/frontend_elementor_v23.css', $html);
        $this->assertStringContainsString('js/pagebuilder_elementor_v23/frontend-runtime.js', $html);
        $this->get('/pages/draft-v23-page')->assertNotFound();

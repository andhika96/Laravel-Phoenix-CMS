...
class PageBuilderElementorV24FormDatasetTest extends TestCase
{
    ...
    {
        $this->getJson('/pagebuilder-elementor/v2.4/datasets')
            ->assertOk()
            ->assertJsonPath('data', []);

        $created = $this->postJson('/pagebuilder-elementor/v2.4/datasets', [
        ...
        $this->putJson('/pagebuilder-elementor/v2.4/datasets/'.$datasetId, [
        ...
        $this->assertSame('Updated Locations', DB::table('pagebuilder_elementor_v24_form_datasets')->value('name'));
    }

    public function test_dataset_api_rejects_invalid_hierarchy_without_persisting_it(): void
    {
        $this->postJson('/pagebuilder-elementor/v2.4/datasets', [
            'name' => 'Broken Dataset',
            'nodes' => [
    ...
    public function test_delete_removes_dataset_and_disconnects_only_owned_v24_page_fields(): void
    {
        $now = now();
        $datasetId = DB::table('pagebuilder_elementor_v24_form_datasets')->insertGetId([
            'user_id' => 1,
        ...
        DB::table('page_builder')->insert([
            ['user_id' => 1, 'uri' => 'owned-v24', 'page_name' => 'Owned V24', 'vars' => json_encode($v24Layout), 'editor_version' => '2.4', 'status' => 'publish', 'created_at' => $now, 'updated_at' => $now],
            ...
            ['user_id' => 2, 'uri' => 'other-v24', 'page_name' => 'Other V24', 'vars' => json_encode($legacyLayout), 'editor_version' => '2.4', 'status' => 'publish', 'created_at' => $now, 'updated_at' => $now],
        ]);

        $this->deleteJson('/pagebuilder-elementor/v2.4/datasets/'.$datasetId)
            ->assertOk()
            ->assertJsonPath('data.id', $datasetId)
            ->assertJsonPath('data.disconnectedFields', 2);

        $this->assertDatabaseMissing('pagebuilder_elementor_v24_form_datasets', ['id' => $datasetId]);

        $owned = json_decode((string) DB::table('page_builder')->where('uri', 'owned-v24')->value('vars'), true, flags: JSON_THROW_ON_ERROR);
        ...
        $this->assertStringContainsString('"datasetMode":"dataset"', (string) DB::table('page_builder')->where('uri', 'other-v24')->value('vars'));
    }

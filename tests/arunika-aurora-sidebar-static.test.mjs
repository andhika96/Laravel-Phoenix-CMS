...
const cmsLayout = readFileSync('resources/views/themes/arunika_aurora/cms/cms_layout.blade.php', 'utf8');
const menuBlade = readFileSync('resources/views/themes/arunika_aurora/components/menu.blade.php', 'utf8');
const css = readFileSync('public/assets/css/themes/arunika_aurora/arunika_aurora.css', 'utf8');
const js = readFileSync('public/assets/js/themes/arunika_aurora/arunika_aurora.js', 'utf8');
...
assert.match(css, /\.ph-nav-category\s*\{[\s\S]*?background:\s*transparent;/, 'collapsed category initials should not render as gray divider strips');
...
assert.match(css, /\.ph-layout-right\s*\{[\s\S]*?flex:\s*1 1 0;[\s\S]*?overflow-x:\s*hidden !important;/, 'layout right should not create horizontal overflow when the sidebar expands');
assert.match(css, /\.ph-scrollable-content\s*\{[\s\S]*?overflow-x:\s*hidden !important;/, 'dashboard content scroller should stay vertical-only');
...
assert.match(js, /function notifyLayoutResize/, 'JS should notify charts after sidebar width changes');
assert.match(js, /window\.echarts\.getInstanceByDom/, 'ECharts instances should be resized after sidebar toggle');
assert.match(js, /setTimeout\(runResize,\s*280\)/, 'layout resize should run after the sidebar transition');

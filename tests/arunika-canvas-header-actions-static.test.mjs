...
const layout = readFileSync(
  path.join(root, 'resources/views/themes/arunika_canvas/cms/cms_layout.blade.php'),
  'utf8',
);
const css = readFileSync(
  path.join(root, 'public/assets/css/themes/arunika_canvas/arunika_canvas.css'),
  'utf8',
);

test('Arunika Canvas keeps notifications mounted but hidden before the admin shortcut', () => {
  ...
  assert.ok(headerActionsIndex >= 0, 'Missing Canvas header actions container.');
  assert.ok(hiddenNotificationIndex > headerActionsIndex, 'Notification must use the Canvas hidden wrapper.');
...
test('Arunika Canvas renders an admin-only user-secret shortcut to Awesome Admin', () => {
  assert.match(
    layout,
...
test('Arunika Canvas header actions use proportional button and glyph sizes', () => {
  assert.match(
    css,
    /\.ph-theme-arunika-canvas\s+\.ph-btn-action-icon\s*\{[^}]*width:\s*34px;[^}]*height:\s*34px;[^}]*font-size:\s*16px;/s,
  );
  assert.match(
    css,
    /\.ph-theme-arunika-canvas\s+\.ph-header-awesome-admin\s+i,\s*\.ph-theme-arunika-canvas\s+\.ph-header-notification\s+#cmsNotifBell\s+i\s*\{[^}]*font-size:\s*16px;/s,
  );
  assert.match(
    css,
    /\.ph-theme-arunika-canvas\s+\.ph-header-notification\.is-hidden\s*\{[^}]*display:\s*none\s*!important;/s,
  );
});

test('Arunika Canvas profile dropdown preserves dynamic account actions in the reference card order', () => {
...
test('Arunika Canvas profile dropdown wires the Dark Mode switch and collapsible theme colors', () => {
  assert.match(
    layout,
...
test('Arunika Canvas profile dropdown uses the compact reference card styling', () => {
  assert.match(
    css,
    /\.ph-theme-arunika-canvas\s+\.ph-header-profile-menu\s*\{[^}]*width:\s*240px;[^}]*min-width:\s*240px;[^}]*padding:\s*6px;[^}]*border:\s*1px solid var\(--ph-canvas-border\);[^}]*border-radius:\s*14px;[^}]*box-shadow:/s,
  );
  assert.match(
    css,
    /\.ph-theme-arunika-canvas\s+\.ph-profile-menu-user\s*\{[^}]*min-height:\s*58px;[^}]*padding:\s*8px;[^}]*border-radius:\s*10px;/s,
  );
  assert.match(
    css,
    /\.ph-theme-arunika-canvas\s+\.ph-header-profile-menu\s+\.dropdown-item\s*\{[^}]*min-height:\s*40px;[^}]*padding:\s*0 10px;[^}]*border-radius:\s*8px;/s,
  );
  assert.match(
    css,
    /\.ph-theme-arunika-canvas\s+\.ph-profile-switch\s*\{[^}]*width:\s*32px;[^}]*height:\s*18px;/s,
  );
  assert.match(
    css,
    /\.ph-theme-arunika-canvas\s+\.ph-profile-theme-toggle\.is-dark\s+\.ph-profile-switch\s*\{[^}]*background:\s*var\(--ph-theme-primary\);/s,
  );
  assert.match(
    css,
    /\.ph-theme-arunika-canvas\s+\.ph-profile-color-toggle:not\(\.collapsed\)\s+\.ph-profile-color-chevron\s*\{[^}]*transform:\s*rotate\(90deg\);/s,
  );
});

test('Arunika Canvas profile summary stays clean while Logout keeps its divider', () => {
  assert.doesNotMatch(
    layout,
    /ph-profile-menu-chevron/,
    'The profile summary must not render a trailing arrow.',
  );

  const sectionRule = css.match(
    /\.ph-theme-arunika-canvas\s+\.ph-profile-menu-section\s*\{([^}]*)\}/s,
  );

  assert.ok(sectionRule, 'Missing profile menu section rule.');
  assert.doesNotMatch(
    sectionRule[1],
    /border-top:/,
    'The first action section must not draw a divider below the profile summary.',
  );
  assert.match(
    css,
    /\.ph-theme-arunika-canvas\s+\.ph-profile-menu-footer\s*\{[^}]*border-top:\s*1px solid var\(--ph-canvas-border\);/s,
  );
});

test('Arunika Canvas profile menu rows keep hover surfaces visually separated', () => {
  assert.match(
    css,
    /\.ph-theme-arunika-canvas\s+\.ph-profile-menu-section\s*>\s*\.dropdown-item\s*\{[^}]*margin:\s*3px 0;/s,
  );
  assert.match(
    css,
    /\.ph-theme-arunika-canvas\s+\.ph-profile-menu-section\s*>\s*\.dropdown-item:first-child\s*\{[^}]*margin-top:\s*0;/s,
  );
  assert.match(
    css,
    /\.ph-theme-arunika-canvas\s+\.ph-profile-menu-section\s*>\s*\.dropdown-item:last-child\s*\{[^}]*margin-bottom:\s*0;/s,
  );
});

...
test('context menu closes from capture-phase clicks outside the shared popup', () => {
  const helperBlock = app.match(/\/\/ V24_CONTEXT_TARGET_HELPERS_START([\s\S]*?)\/\/ V24_CONTEXT_TARGET_HELPERS_END/)?.[1];
...
test('external clipboard accepts only versioned Phoenix v2.4 node payloads', () => {
  const helperBlock = app.match(/\/\/ V24_CONTEXT_CLIPBOARD_HELPERS_START([\s\S]*?)\/\/ V24_CONTEXT_CLIPBOARD_HELPERS_END/)?.[1];
  ...
  assert.equal(context.helpers.parseContextClipboardPayload('{"source":"phoenix-pagebuilder-v24","version":1,"node":{"type":"heading"}}').reason, 'invalid');

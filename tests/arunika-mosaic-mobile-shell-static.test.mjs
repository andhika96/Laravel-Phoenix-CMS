...
assert.match(
	layout,
	/class="ph-mobile-sidebar-close"(?:(?!<\/button>)[\s\S])*?event\.stopPropagation\(\); toggleSidebar\(\)(?:(?!<\/button>)[\s\S])*?<i class="fas fa-bars" aria-hidden="true"><\/i>/,
	'the open drawer should reuse the header hamburger icon in its brand header',
);
...
assert.match(
	css,
	/@media \(max-width: 768px\)[\s\S]*?\.ph-sidebar\.ph-expanded \.ph-sidebar-logo-container\s*\{[^}]*padding-left:\s*12px;[^}]*padding-right:\s*50px;/s,
	'the expanded mobile brand should shift left to preserve breathing room before the close control',
);
assert.match(
	css,
	/@media \(max-width: 768px\)[\s\S]*?\.ph-mobile-sidebar-close\s*\{[^}]*right:\s*8px;/s,
	'the mobile close control should sit close to the drawer edge without crowding the site name',
);
assert.match(
	css,
	/\.ph-mobile-sidebar-close\s*\{[^}]*font-size:\s*1\.2rem;/s,
	'the drawer hamburger should match the header navigation icon scale',
);

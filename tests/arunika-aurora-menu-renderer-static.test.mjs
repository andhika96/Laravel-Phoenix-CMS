import assert from 'node:assert/strict';
import { readFileSync } from 'node:fs';

const menuBlade = readFileSync(
    'resources/views/themes/arunika_aurora/components/menu.blade.php',
    'utf8',
);

const menuV2Start = menuBlade.indexOf('function menu_v2()');
const categorizedStart = menuBlade.indexOf('// Menu list with category', menuV2Start);
const uncategorizedStart = menuBlade.indexOf('// Menu list without category', categorizedStart);

assert.notEqual(menuV2Start, -1, 'menu_v2 renderer should exist');
assert.notEqual(categorizedStart, -1, 'categorized menu renderer should exist');
assert.notEqual(uncategorizedStart, -1, 'uncategorized menu renderer should exist');

const categorizedRenderer = menuBlade.slice(categorizedStart, uncategorizedStart);
const menuV2Renderer = menuBlade.slice(menuV2Start);
const safeSubmenuLoop = /foreach\s*\(\$value1\['parent_submenu'\]\['list'\]\s+as\s+\$key\d+\s*=>\s*\$value2\)/g;
const shadowingSubmenuLoop = /foreach\s*\(\$value1\['parent_submenu'\]\['list'\]\s+as\s+\$key1\s*=>\s*\$value1\)/;
const semanticSubmenuLabel = /<span class="ph-submenu-label">'\.\$value2\['submenu_name'\]\.'<\/span>/g;

assert.doesNotMatch(
    categorizedRenderer,
    shadowingSubmenuLoop,
    'categorized submenu loops must not overwrite the parent menu variable',
);
assert.equal(
    categorizedRenderer.match(safeSubmenuLoop)?.length ?? 0,
    2,
    'categorized collapsed and floating submenu loops should preserve the parent menu variable',
);
assert.equal(
    menuV2Renderer.match(semanticSubmenuLabel)?.length ?? 0,
    4,
    'categorized and uncategorized expanded/floating submenu labels should use a semantic label class',
);

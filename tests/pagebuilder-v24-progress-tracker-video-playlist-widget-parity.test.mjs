...
import { configureModules } from './helpers/pagebuilder-v24-module-test.mjs';
...
test('Progress Tracker and Video Playlist definitions expose Pro defaults and normalization', async () => {
    const registrySource = await source('public/js/pagebuilder_elementor_v24/widget-registry.js');
    const context = { window: {} };
    vm.runInNewContext(registrySource, context);
    configureModules(context, await Promise.all([
        source('resources/pagebuilder_elementor_v24/modules/widgets/pro/progress-tracker/module.json'),
        source('resources/pagebuilder_elementor_v24/modules/widgets/pro/video-playlist/module.json'),
    ]));

    const definitionPaths = [
        'resources/pagebuilder_elementor_v24/modules/widgets/pro/progress-tracker/definition.js',
        'resources/pagebuilder_elementor_v24/modules/widgets/pro/video-playlist/definition.js',
    ];
    ...
    const progress = context.window.PageBuilderElementorV24Widgets.get('progress_tracker');
    const playlist = context.window.PageBuilderElementorV24Widgets.get('video_playlist');
...
test('new Pro settings map Content, Style, responsive units, and Advanced controls', async () => {
    const progressComponent = await loadSfc('resources/pagebuilder_elementor_v24/modules/widgets/pro/progress-tracker/Settings.vue');
    const progressContent = await renderToString(Vue.createSSRApp(progressComponent, { node: { type: 'progress_tracker', settings: progressSettings }, editor: editorFor('content') }));
    ...
    const progressStyle = await renderToString(Vue.createSSRApp(progressComponent, { node: { type: 'progress_tracker', settings: progressSettings }, editor: editorFor('style') }));
    ...
    const playlistComponent = await loadSfc('resources/pagebuilder_elementor_v24/modules/widgets/pro/video-playlist/Settings.vue');
    const playlistContent = await renderToString(Vue.createSSRApp(playlistComponent, { node: { type: 'video_playlist', settings: playlistSettings }, editor: editorFor('content') }));
    ...
    const playlistStyle = await renderToString(Vue.createSSRApp(playlistComponent, { node: { type: 'video_playlist', settings: playlistSettings }, editor: editorFor('style') }));
    ...
    assert.match(await renderToString(Vue.createSSRApp(progressComponent, { node: { type: 'progress_tracker', settings: progressSettings }, editor: editorFor('advanced') })), /Advanced/);
    assert.match(await renderToString(Vue.createSSRApp(playlistComponent, { node: { type: 'video_playlist', settings: playlistSettings }, editor: editorFor('advanced') })), /Advanced/);
});

test('new Pro canvases render visible, safe, and interactive markup', async () => {
    const progressComponent = await loadSfc('resources/pagebuilder_elementor_v24/modules/widgets/pro/progress-tracker/Canvas.vue');
    const progressHtml = await renderToString(Vue.createSSRApp(progressComponent, { item: { id: 'progress-test', type: 'progress_tracker', settings: { ...progressSettings, trackerType: 'circular' } }, responsiveDevice: 'desktop' }));
    ...
    const playlistComponent = await loadSfc('resources/pagebuilder_elementor_v24/modules/widgets/pro/video-playlist/Canvas.vue');
    const playlistHtml = await renderToString(Vue.createSSRApp(playlistComponent, { item: { id: 'playlist-test', type: 'video_playlist', settings: { ...playlistSettings, items: [{ ...playlistSettings.items[0], title: '<script>alert(1)<\/script>' }] } }, responsiveDevice: 'desktop' }));
...
test('Progress Tracker keeps root direction separate from progress indicator alignment', async () => {
    const component = await loadSfc('resources/pagebuilder_elementor_v24/modules/widgets/pro/progress-tracker/Canvas.vue');
    const html = await renderCanvas(component, {
        item: {
            id: 'progress-alignment-test',
    ...
    assert.match(html, /pb-pro-progress-tracker--horizontal align-center/);
    assert.match(html, /pb-pro-progress-tracker__indicator[^>]*style="[^"]*margin-left:auto;[^"]*margin-right:0/);
});

test('Video Playlist renders a functional dropdown contract and honors section background type', async () => {
    const component = await loadSfc('resources/pagebuilder_elementor_v24/modules/widgets/pro/video-playlist/Canvas.vue');
    const gradientHtml = await renderCanvas(component, {
        item: {
            id: 'playlist-dropdown-test',
            type: 'video_playlist',
            settings: {
                ...playlistSettings,
                dropdownAlignment: 'right',
                ...
                items: [{ id: 'section-1', type: 'section', title: 'Section', titleTag: 'h4', sectionContent: 'Content' }],
            },
        },
        responsiveDevice: 'desktop',
    });

    assert.match(gradientHtml, /data-playlist-dropdown-toggle/);
    assert.match(gradientHtml, /data-pb-interactive="true"/);
    assert.match(gradientHtml, /aria-expanded="true"/);
    assert.match(gradientHtml, /fa-chevron-down/);
    assert.match(gradientHtml, /justify-content:flex-end/);
    assert.match(gradientHtml, /--video-playlist-dropdown-color:#112233/);
    assert.match(gradientHtml, /--video-playlist-dropdown-hover:#223344/);
    assert.match(gradientHtml, /--video-playlist-dropdown-active:#334455/);
    assert.match(gradientHtml, /linear-gradient\(120deg, #ffeedd, #ddeeff\)/);

    const noneHtml = await renderCanvas(component, {
        item: {
            id: 'playlist-section-none-test',
            type: 'video_playlist',
            settings: {
                ...playlistSettings,
                sectionBackgroundType: 'none',
                items: [{ id: 'section-1', type: 'section', title: 'Section', titleTag: 'h4', sectionContent: 'Content' }],
            },
        },
        responsiveDevice: 'desktop',
    });
    assert.match(noneHtml, /pb-pro-video-playlist__section[^>]*style="[^"]*background:transparent/);
});

test('new widgets are wired through registry, labels, Blade renderer, and frontend runtime', async () => {
    const progressManifest = JSON.parse(await source('resources/pagebuilder_elementor_v24/modules/widgets/pro/progress-tracker/module.json'));
    const playlistManifest = JSON.parse(await source('resources/pagebuilder_elementor_v24/modules/widgets/pro/video-playlist/module.json'));
    const progressCanvas = await source('resources/pagebuilder_elementor_v24/modules/widgets/pro/progress-tracker/Canvas.vue');
    const playlistCanvas = await source('resources/pagebuilder_elementor_v24/modules/widgets/pro/video-playlist/Canvas.vue');
    const progressBlade = await source('resources/pagebuilder_elementor_v24/modules/widgets/pro/progress-tracker/frontend.blade.php');
    const playlistBlade = await source('resources/pagebuilder_elementor_v24/modules/widgets/pro/video-playlist/frontend.blade.php');
    const progressRuntime = await source('resources/pagebuilder_elementor_v24/modules/widgets/pro/progress-tracker/runtime.js');
    const playlistRuntime = await source('resources/pagebuilder_elementor_v24/modules/widgets/pro/video-playlist/runtime.js');

    assert.deepEqual([progressManifest.type, progressManifest.label, progressManifest.icon], ['progress_tracker', 'Progress Tracker', 'fas fa-tasks']);
    assert.deepEqual([playlistManifest.type, playlistManifest.label, playlistManifest.icon], ['video_playlist', 'Video Playlist', 'fas fa-list']);
    assert.match(progressCanvas, /pb-pro-progress-tracker/);
    assert.match(playlistCanvas, /pb-pro-video-playlist/);
    assert.match(progressBlade, /data-progress-tracker/);
    assert.match(playlistBlade, /data-video-playlist/);
    assert.match(progressRuntime, /initProProgressTracker/);
    assert.match(playlistRuntime, /initProVideoPlaylist/);
    assert.match(progressRuntime, /data-progress-tracker/);
    assert.match(playlistRuntime, /data-video-playlist/);
});
